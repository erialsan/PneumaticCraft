...
public class WandTriggerRegistry {
     ...
     * @param modid   a unique identifier. It is best to register your own triggers using your mod id to avoid conflicts
     *                with mods that register triggers for the same block
     */
     ...
     * @return
     */
    public static boolean performTrigger(World world, ItemStack wand, EntityPlayer player, int x, int y, int z,
        int side, Block block, int meta) {
     ...
     */
    public static boolean performTrigger(World world, ItemStack wand, EntityPlayer player, int x, int y, int z,
        int side, Block block, int meta, String modid) {
        if (!triggers.containsKey(modid)) return false;
        HashMap<List, List> temp = triggers.get(modid);
        List l = temp.get(Arrays.asList(block, meta));
        if (l == null) l = temp.get(Arrays.asList(block, -1));
