...
public class ItemFocusBasic extends Item {
    ...
    public void addFocusInformation(ItemStack focusstack, EntityPlayer player, List list, boolean par4) {
        ...
        for (Short id : map.keySet()) {
            list.add(
                EnumChatFormatting.DARK_PURPLE + FocusUpgradeType.types[id].getLocalizedName()
    ...
    /**
     * Does the focus have ornamentation like the focus of the nine hells. Ornamentation is a standard icon rendered in
     * a cross around the focus
     */
    ...
    public short[] getAppliedUpgrades(ItemStack focusstack) {
        ...
        if (nbttaglist == null) {
            ...
            for (int j = 0; j < nbttaglist.tagCount(); ++j) {
                if (j >= 5) break;
                l[j] = nbttaglist.getCompoundTagAt(j)
                    .getShort("id");
            }
     ...
     * "trees"
     * that make certain upgrades available only when others are unlocked first, when certain research is completed, or
     * similar logic.
     * 
     */
    ...
    private void setFocusUpgradeTagList(ItemStack focusstack, short[] upgrades) {
        if (!focusstack.hasTagCompound()) focusstack.setTagCompound(new NBTTagCompound());
