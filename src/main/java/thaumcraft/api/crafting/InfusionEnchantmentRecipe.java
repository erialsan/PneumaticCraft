...
public class InfusionEnchantmentRecipe {
    ...
    public InfusionEnchantmentRecipe(String research, Enchantment input, int inst, AspectList aspects2,
        ItemStack[] recipe) {
    ...
    public boolean matches(ArrayList<ItemStack> input, ItemStack central, World world, EntityPlayer player) {
        ...
        if (!enchantment.canApply(central) || !central.getItem()
            .isItemTool(central)) {
            return false;
        }

        Map map1 = EnchantmentHelper.getEnchantments(central);
        Iterator iterator = map1.keySet()
            .iterator();
    ...
    protected boolean areItemStacksEqual(ItemStack stack0, ItemStack stack1, boolean fuzzy) {
        ...
        if (fuzzy) {
            ...
            if (od != -1) {
                ItemStack[] ores = OreDictionary.getOres(od)
                    .toArray(new ItemStack[] {});
                if (ThaumcraftApiHelper.containsMatch(false, new ItemStack[] { stack1 }, ores)) return true;
            }
        }
        return stack0.getItem() != stack1.getItem() ? false
            : (stack0.getItemDamage() != stack1.getItemDamage() ? false : stack0.stackSize <= stack0.getMaxStackSize());
    }
