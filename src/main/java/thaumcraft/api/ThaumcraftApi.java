 ...
 *
 *
 *         IMPORTANT: If you are adding your own aspects to items it is a good idea to do it AFTER Thaumcraft adds its
 *         aspects, otherwise odd things may happen.
 *
 */
public class ThaumcraftApi {
    ...
    public static ArmorMaterial armorMatThaumium = EnumHelper
        .addArmorMaterial("THAUMIUM", 25, new int[] { 2, 6, 5, 2 }, 25);
    public static ArmorMaterial armorMatSpecial = EnumHelper
        .addArmorMaterial("SPECIAL", 25, new int[] { 1, 3, 2, 1 }, 25);
    public static ArmorMaterial armorMatThaumiumFortress = EnumHelper
        .addArmorMaterial("FORTRESS", 40, new int[] { 3, 7, 6, 3 }, 25);
    public static ArmorMaterial armorMatVoid = EnumHelper.addArmorMaterial("VOID", 10, new int[] { 3, 7, 6, 3 }, 10);
    public static ArmorMaterial armorMatVoidFortress = EnumHelper
        .addArmorMaterial("VOIDFORTRESS", 18, new int[] { 4, 8, 7, 4 }, 10);
     ...
     *                   to differentiate between mobs. <br>
    ...
    public static void addSmeltingBonus(ItemStack in, ItemStack out) {
        smeltingBonus
            .put(Arrays.asList(in.getItem(), in.getItemDamage()), new ItemStack(out.getItem(), 0, out.getItemDamage()));
    }
     ...
     */
    public static ShapedArcaneRecipe addArcaneCraftingRecipe(String research, ItemStack result, AspectList aspects,
        Object... recipe) {
     ...
     * @param result   the recipe output
     * @param aspects  the vis cost per aspect
     * @param recipe   The recipe. Format is exactly the same as vanilla shapeless recipes. Input itemstacks are NBT
     *                 sensitive.
     */
    public static ShapelessArcaneRecipe addShapelessArcaneCraftingRecipe(String research, ItemStack result,
    ...
    /**
     * @param research    the research key required for this recipe to work. Leave blank if it will work without
     *                    research
     * @param result      the recipe output. It can either be an itemstack or an nbt compound tag that will be added to
     *                    the central item
     ...
     * 
     */
    public static InfusionRecipe addInfusionCraftingRecipe(String research, Object result, int instability,
     ...
     * 
     */
    public static InfusionEnchantmentRecipe addInfusionEnchantmentRecipe(String research, Enchantment enchantment,
        int instability, AspectList aspects, ItemStack[] recipe) {
        InfusionEnchantmentRecipe r = new InfusionEnchantmentRecipe(
            research,
            enchantment,
            instability,
            aspects,
    ...
    public static CrucibleRecipe getCrucibleRecipe(ItemStack stack) {
        for (Object r : getCraftingRecipes()) {
            if (r instanceof CrucibleRecipe) {
                if (((CrucibleRecipe) r).getRecipeOutput()
                    .isItemEqual(stack)) return (CrucibleRecipe) r;
            }
        }
        return null;
    }
     ...
     * <p>
     * <i>ThaumcraftApi.registerObjectTag(new ItemStack(Blocks.cobblestone), (new AspectList()).add(Aspect.ENTROPY,
     * 1).add(Aspect.EARTH, 1));</i>
     * 
     * @param item    the item passed. Pass OreDictionary.WILDCARD_VALUE if all damage values of this item/block should
     *                have the same aspects
     ...
     * Used to assign apsects to the given item/block. Here is an example of the declaration for cobblestone:
     * <p>
     * <i>ThaumcraftApi.registerObjectTag(new ItemStack(Blocks.cobblestone), new int[]{0,1}, (new
     * AspectList()).add(Aspect.ENTROPY, 1).add(Aspect.EARTH, 1));</i>
     * 
     * @param item
     * @param meta    A range of meta values if you wish to lump several item meta's together as being the "same" item
     *                (i.e. stair orientations)
     * @param aspects A ObjectTags object of the associated aspects
     */
     ...
     * <p>
     * <i>ThaumcraftApi.registerComplexObjectTag(new ItemStack(Blocks.cobblestone), (new
     ...
     * 
     * @param item,   pass OreDictionary.WILDCARD_VALUE to meta if all damage values of this item/block should have the
     *                same aspects
     * @param aspects A ObjectTags object of the associated aspects
     */
     ...
     * As for standard crops, you need to create and ItemStack that tells the golem what block id
     * and metadata represents the crop when fully grown. Sending a metadata of [OreDictionary.WILDCARD_VALUE] will mean
     * the actualy md won't get
     ...
     * "[ore item/block id],[ore item/block metadata],[cluster item/block id],[cluster item/block metadata],[chance
     * modifier float]"
     * 
     * NOTE: The chance modifier is a multiplier applied to the default chance for that cluster to be produced (default
     * 27.5% for a pickaxe of the core)
     * 
     * Example for vanilla iron ore to produce one of my own native iron clusters (assuming default id's) at double the
     * default chance:
     ...
     * [Entity] is in a similar format to what is used for mob spawners and such (see EntityList.class for vanilla
     * examples).
     ...
     * You can give 0 or negative numbers to allow champions to spawn with a very low chance only in particularly
     * dangerous places.
