...
public class Api {
    ...
    public static void registerMod(String modname, String... keyDescriptions) {
        try {
            Class.forName("modwarriors.notenoughkeys.keys.KeyHelper")
    ...
    public static boolean isKeyBindingPressed(KeyBinding binding) {
        try {
            return (Boolean) Class.forName("modwarriors.notenoughkeys.keys.KeyHelper")
                .getMethod("isKeyBindingPressed", KeyBinding.class)
                .invoke(null, binding);
        } catch (Exception e) {
            e.printStackTrace();
        }
