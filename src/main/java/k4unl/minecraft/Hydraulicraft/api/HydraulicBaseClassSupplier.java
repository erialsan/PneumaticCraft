    ...
    public static IBaseClass getBaseClass(TileEntity target, PressureTier pTier, int maxStorage) {
        ...
        try {
            if (baseHandlerConstructor == null)
                baseHandlerConstructor = Class.forName("k4unl.minecraft.Hydraulicraft.tileEntities.TileHydraulicBase")
                    .getConstructor(PressureTier.class, int.class);
            baseClassEntity = (IBaseClass) baseHandlerConstructor.newInstance(pTier, maxStorage);
            baseClassEntity.init(target);
        } catch (Exception e) {
            System.err.println(
                "[Hydraulicraft API] An error has occured whilst trying to get a base class. Here's a stacktrace:");
            e.printStackTrace();
        }
    ...
    public static IBaseClass getBaseClass(TMultiPart target, PressureTier pTier, int maxStorage) {
        ...
        try {
            if (multipartConstructor == null)
                multipartConstructor = Class.forName("k4unl.minecraft.Hydraulicraft.tileEntities.TileHydraulicBase")
                    .getConstructor(PressureTier.class, int.class);
