...
public class DroneAIPlace extends DroneAIBlockInteraction {
    ...
    protected boolean isValidPosition(ChunkPosition pos) {
        if (drone.getWorld()
            .isAirBlock(pos.chunkPosX, pos.chunkPosY, pos.chunkPosZ)) {
            ...
            for (int i = 0; i < drone.getInventory()
                ...
                if (droneStack != null && droneStack.getItem() instanceof ItemBlock) {
                    if (widget.isItemValidForFilters(droneStack)) {
                        if (((ItemBlock) droneStack.getItem()).field_150939_a.canPlaceBlockOnSide(
                            drone.getWorld(),
                            pos.chunkPosX,
                            pos.chunkPosY,
                            pos.chunkPosZ,
                            ProgWidgetPlace.getDirForSides(((ISidedWidget) widget).getSides())
                                .ordinal())) {
                            if (drone instanceof EntityDrone) {
                                EntityDrone entity = (EntityDrone) drone;
                                entity.setPosition(entity.posX, entity.posY + 200, entity.posZ);// Teleport the drone to
                                                                                                // make sure it isn't in
                                                                                                // the way of placing a
                                                                                                // block.
                            }
                            if (drone.getWorld()
                                .canPlaceEntityOnSide(
                                    ((ItemBlock) droneStack.getItem()).field_150939_a,
                                    pos.chunkPosX,
                                    pos.chunkPosY,
                                    pos.chunkPosZ,
                                    false,
        ...
        if (drone.getPathNavigator()
            ...
            for (int i = 0; i < drone.getInventory()
                ...
                ItemStack droneStack = drone.getInventory()
                    .getStackInSlot(i);
                if (droneStack != null && droneStack.getItem() instanceof ItemBlock
                    && ((ItemBlock) droneStack.getItem()).field_150939_a.canPlaceBlockOnSide(
                        drone.getWorld(),
                        pos.chunkPosX,
                        pos.chunkPosY,
                        pos.chunkPosZ,
                    ...
                    if (widget.isItemValidForFilters(droneStack)) {
                        if (drone instanceof EntityDrone) {
                            EntityDrone entity = (EntityDrone) drone;
                            entity.setPosition(entity.posX, entity.posY + 200, entity.posZ);// Teleport the drone to
                                                                                            // make sure it isn't in the
                                                                                            // way of placing a block.
                        }
                        if (drone.getWorld()
                            .canPlaceEntityOnSide(
                                ((ItemBlock) droneStack.getItem()).field_150939_a,
                            ...
                            int meta = droneStack.getItem()
                                .getMetadata(droneStack.getItemDamage());
                            int newMeta = block.onBlockPlaced(
                                drone.getWorld(),
                                pos.chunkPosX,
                                pos.chunkPosY,
                                pos.chunkPosZ,
