...
public abstract class DroneAIBlockInteraction<Widget extends ProgWidgetAreaItemBase> extends EntityAIBase {
    ...
    private final List<ChunkPosition> blacklist = new ArrayList<ChunkPosition>();// a list of position which weren't
                                                                                 // allowed to be digged in the past.
    private int curY;
    ...
    protected boolean searching; // true while the drone is searching for a coordinate, false if traveling/processing a
                                 // coordinate.
    ...
    public boolean continueExecuting() {
        ...
        if (searching) {
            ...
            while (curPos == null && curY != lastSuccessfulY && order != ProgWidgetDigAndPlace.EnumOrder.CLOSEST
                || firstRun) {
                firstRun = false;
                while (!shouldAbort() && searchIndex < area.size()) {
                    ...
                    if (isYValid(pos.chunkPosY) && !blacklist.contains(pos)
                        && (!respectClaims() || !DroneClaimManager.getInstance(drone.getWorld())
                            .isClaimed(pos))) {
                        ...
                        if (isValidPosition(pos)) {
                            ...
                            if (moveToPositions()) {
                                if (moveIntoBlock()) {
                                    if (drone.getPathNavigator()
                                        .moveToXYZ(curPos.chunkPosX, curPos.chunkPosY + 0.5, curPos.chunkPosZ)) {
                                        searching = false;
                                        totalActions++;
                                        if (respectClaims()) DroneClaimManager.getInstance(drone.getWorld())
                                            .claim(pos);
                                        blacklist.clear();// clear the list for next time (maybe the blocks/rights have
                                                          // changed by the time there will be dug again).
                                        return true;
                                    }
                                } else {
                                    for (ForgeDirection dir : ForgeDirection.VALID_DIRECTIONS) {
                                        if (drone.getPathNavigator()
                                            .moveToXYZ(
                                                curPos.chunkPosX + dir.offsetX,
                                                curPos.chunkPosY + dir.offsetY + 0.5,
                                                curPos.chunkPosZ + dir.offsetZ)) {
                                            searching = false;
                                            totalActions++;
                                            if (respectClaims()) DroneClaimManager.getInstance(drone.getWorld())
                                                .claim(pos);
                                            blacklist.clear();// clear the list for next time (maybe the blocks/rights
                                                              // have changed by the time there will be dug again).
                                            return true;
                                        }
                                    }
                                }
            ...
            double dist = curPos != null ? PneumaticCraftUtils.distBetween(
                curPos.chunkPosX + 0.5,
                curPos.chunkPosY + 0.5,
                curPos.chunkPosZ + 0.5,
                dronePos.xCoord,
                dronePos.yCoord,
                dronePos.zCoord) : 0;
            if (curPos != null) {
    ...
    protected void indicateToListeningPlayers(ChunkPosition pos) {
        for (EntityPlayer player : (List<EntityPlayer>) drone.getWorld().playerEntities) {
            if (player.getCurrentArmor(3) != null && player.getCurrentArmor(3)
                .getItem() == Itemss.pneumaticHelmet
                && ItemPneumaticArmor.getUpgrades(ItemMachineUpgrade.UPGRADE_ENTITY_TRACKER, player.getCurrentArmor(3))
                    > 0
                && ((IPressurizable) Itemss.pneumaticHelmet).getPressure(player.getCurrentArmor(3)) > 0) {
