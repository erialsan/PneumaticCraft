...
public class EntityPathNavigateDrone extends PathNavigate implements IPathNavigator {
    ...
    private PathEntity getPathEntityToEntity(EntityDrone par1Entity, Entity par2Entity, float par3, boolean par4,
         ...
         * PathEntity pathentity = new PathFinderDrone(par1Entity, chunkcache, par4, par5, pathThroughLiquid,
         * par7).createEntityPathTo(par1Entity, par2Entity, par3);
         * return pathentity;
         */
        return getEntityPathToXYZ(
            par1Entity,
            (int) Math.floor(par2Entity.posX),
            (int) Math.floor(par2Entity.posY),
            (int) Math.floor(par2Entity.posZ),
            par3,
            par4,
            par5,
            par6,
    ...
    public PathEntity getEntityPathToXYZ(EntityDrone par1Entity, int par2, int par3, int par4, float par5, boolean par6,
        ...
        if (!forceTeleport || l == par2 && i1 == par3 && j1 == par4) {
            ...
            pathentity = new PathFinderDrone(par1Entity, chunkcache, par6, par7, pathThroughLiquid, par9)
                .createEntityPathTo(par1Entity, par2, par3, par4, par5);
            if (pathentity != null) {
                PathPoint finalPoint = pathentity.getFinalPathPoint();
                if (finalPoint == null || finalPoint.xCoord != par2
                    || finalPoint.yCoord != par3
                    || finalPoint.zCoord != par4) pathentity = null;
            }
        }
        ...
        if (isGoingToTeleport()) {
            if (teleportCounter == 0 || teleportCounter == 60) {
                NetworkHandler.sendToAllAround(
                    new PacketPlaySound(
                        Sounds.HUD_INIT,
                        pathfindingEntity.posX,
                        pathfindingEntity.posY,
                        pathfindingEntity.posZ,
                        0.1F,
                        teleportCounter == 0 ? 0.7F : 1F,
                        true),
                    pathfindingEntity.worldObj);
            }
            ...
            if (++teleportCounter > TELEPORT_TICKS) {
                ...
                teleportCounter = -1;
                setPath(null, 0);
                pathfindingEntity.getMoveHelper()
                    .setMoveTo(telX, telY, telZ, pathfindingEntity.getSpeed());
    ...
    public void teleport() {
        ...
        for (int l = 0; l < short1; ++l) {
            ...
            double d7 = pathfindingEntity.posX + (telX + 0.5 - pathfindingEntity.posX) * d6
                + (rand.nextDouble() - 0.5D) * width * 2.0D;
            double d8 = pathfindingEntity.posY + (telY - pathfindingEntity.posY) * d6 + rand.nextDouble() * height;
            double d9 = pathfindingEntity.posZ + (telZ + 0.5 - pathfindingEntity.posZ) * d6
                + (rand.nextDouble() - 0.5D) * width * 2.0D;
            NetworkHandler
                .sendToAllAround(new PacketSpawnParticle("portal", d7, d8, d9, f, f1, f2), pathfindingEntity.worldObj);
        }

        pathfindingEntity.worldObj.playSoundEffect(
            pathfindingEntity.posX,
            pathfindingEntity.posY,
            pathfindingEntity.posZ,
            "mob.endermen.portal",
    ...
    @Override
    public boolean isDirectPathBetweenPoints(Vec3 p_75493_1_, Vec3 p_75493_2_, int p_75493_3_, int p_75493_4_,
        int p_75493_5_) {
        return false;
    }
}
