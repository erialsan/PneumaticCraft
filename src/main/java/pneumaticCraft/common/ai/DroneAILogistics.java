...
public class DroneAILogistics extends EntityAIBase {
    ...
    public boolean shouldExecute() {
        ...
        for (int x = minX; x < maxX + 16; x += 16) {
            for (int z = minZ; z < maxZ + 16; z += 16) {
                Chunk chunk = drone.getWorld()
                    .getChunkFromBlockCoords(x, z);
                Map<ChunkPosition, ISemiBlock> map = SemiBlockManager.getInstance(drone.getWorld())
                    .getSemiBlocks()
                    .get(chunk);
                if (map != null) {
    ...
    private boolean doLogistics() {
        ItemStack item = drone.getInventory()
            .getStackInSlot(0);
        FluidStack fluid = drone.getTank()
            .getFluid();
    ...
    private boolean clearAIAndProvideAgain() {
        ...
        if (curTask.isStillValid(
            drone.getInventory()
                .getStackInSlot(0) != null ? drone.getInventory()
                    .getStackInSlot(0)
                    : drone.getTank()
                        .getFluid())
            && execute(curTask)) {
    ...
    public boolean execute(LogisticsTask task) {
        if (drone.getInventory()
            ...
            curAI = new DroneEntityAIInventoryExport(
                drone,
                new FakeWidgetLogistics(task.requester.getPos(), task.transportingItem));
        } else if (drone.getTank()
            .getFluidAmount() > 0) {
                if (!isPosPathfindable(task.requester.getPos())) return false;
                curAI = new DroneAILiquidExport(
                    drone,
                    new FakeWidgetLogistics(task.requester.getPos(), task.transportingFluid.stack));
            } else if (task.transportingItem != null) {
                if (!isPosPathfindable(task.provider.getPos())) return false;
                curAI = new DroneEntityAIInventoryImport(
                    drone,
                    new FakeWidgetLogistics(task.provider.getPos(), task.transportingItem));
            } else {
                if (!isPosPathfindable(task.provider.getPos())) return false;
                curAI = new DroneAILiquidImport(
                    drone,
                    new FakeWidgetLogistics(task.provider.getPos(), task.transportingFluid.stack));
            }
    ...
    private boolean isPosPathfindable(ChunkPosition pos) {
        for (ForgeDirection d : ForgeDirection.VALID_DIRECTIONS) {
            if (drone.isBlockValidPathfindBlock(
                pos.chunkPosX + d.offsetX,
                pos.chunkPosY + d.offsetY,
                pos.chunkPosZ + d.offsetZ)) return true;
        }
        return false;
    }

    private static class FakeWidgetLogistics extends ProgWidgetAreaItemBase
        implements ISidedWidget, ICountWidget, ILiquidFiltered {
