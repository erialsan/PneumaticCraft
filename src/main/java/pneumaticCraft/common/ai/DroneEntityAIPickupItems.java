...
public class DroneEntityAIPickupItems extends EntityAIBase {
            ...
            if (itemPickupWidget.isItemValidForFilters(stack)) {
                for (int i = 0; i < drone.getInventory()
                    ...
                    ItemStack droneStack = drone.getInventory()
                        .getStackInSlot(i);
                    if (droneStack == null
                        || droneStack.isItemEqual(stack) && droneStack.stackSize < droneStack.getMaxStackSize()) {
    ...
    public boolean continueExecuting() {
        ...
        if (Vec3.createVectorHelper(curPickingUpEntity.posX, curPickingUpEntity.posY, curPickingUpEntity.posZ)
            .distanceTo(drone.getPosition()) < 1.5) {
            ItemStack stack = curPickingUpEntity.getEntityItem();
            if (itemPickupWidget.isItemValidForFilters(stack)) {
                new EventHandlerPneumaticCraft()
                    .onPlayerPickup(new EntityItemPickupEvent(drone.getFakePlayer(), curPickingUpEntity));// not posting
                                                                                                          // the event
                                                                                                          // globally,
                                                                                                          // as I don't
                                                                                                          // have a way
                ...
                ItemStack remainder = PneumaticCraftUtils
                    .exportStackToInventory(drone.getInventory(), stack, ForgeDirection.UP);// side doesn't matter,
                                                                                            // drones aren't ISided.
                if (remainder == null) {
