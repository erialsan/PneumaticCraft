    ...
    private boolean fillTank(ChunkPosition pos, boolean simulate) {
        if (drone.getTank()
            ...
            if (te instanceof IFluidHandler) {
                ...
                FluidStack exportedFluid = drone.getTank()
            ...
            } else if (((ILiquidExport) widget).isPlacingFluidBlocks()
                && (!((ICountWidget) widget).useCount() || getRemainingCount() >= 1000)) {
                    Block fluidBlock = drone.getTank()
                        .getFluid()
                        .getFluid()
                        .getBlock();
                    if (drone.getTank()
                        .getFluidAmount() >= 1000 && fluidBlock != null
                        && drone.getWorld()
                        ...
                        if (!simulate) {
                            ...
                            drone.getTank()
                                .drain(1000, true);
                            drone.getWorld()
                                .setBlock(pos.chunkPosX, pos.chunkPosY, pos.chunkPosZ, fluidBlock);
                        }
                        return true;
                    }
                }
