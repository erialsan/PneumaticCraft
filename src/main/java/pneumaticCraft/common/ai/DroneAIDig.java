...
public class DroneAIDig extends DroneAIBlockInteraction {
    ...
    private void swapBestItemToFirstSlot(Block block, ChunkPosition pos) {
        ...
        ItemStack oldCurrentStack = drone.getInventory()
            .getStackInSlot(0);
        for (int i = 0; i < drone.getInventory()
            ...
            drone.getInventory()
                .setInventorySlotContents(
                    ...
                    drone.getInventory()
                        .getStackInSlot(i));
            float softness = block.getPlayerRelativeBlockHardness(
                drone.getFakePlayer(),
                drone.getWorld(),
                pos.chunkPosX,
                pos.chunkPosY,
        ...
        drone.getInventory()
            .setInventorySlotContents(0, oldCurrentStack);
        if (bestSlot != 0) {
            ItemStack bestItem = drone.getInventory()
            ...
            drone.getInventory()
                .setInventorySlotContents(
                    ...
                    drone.getInventory()
                        .getStackInSlot(0));
            drone.getInventory()
                .setInventorySlotContents(0, bestItem);
        }
    }

    @Override
    protected boolean doBlockInteraction(ChunkPosition pos, double distToBlock) {
        if (!((FakePlayerItemInWorldManager) drone.getFakePlayer().theItemInWorldManager).isDigging()
            || !((FakePlayerItemInWorldManager) drone.getFakePlayer().theItemInWorldManager).isAcknowledged()) {
            int x = pos.chunkPosX;
            int y = pos.chunkPosY;
            int z = pos.chunkPosZ;
            ...
            if (!ignoreBlock(block) && isBlockValidForFilter(worldCache, drone, pos, widget)) {
                ...
                FakePlayerItemInWorldManager manager = (FakePlayerItemInWorldManager) drone
                    .getFakePlayer().theItemInWorldManager;
    ...
    public static boolean isBlockValidForFilter(IBlockAccess worldCache, IDroneBase drone, ChunkPosition pos,
        ProgWidgetAreaItemBase widget) {
        int x = pos.chunkPosX;
        int y = pos.chunkPosY;
        int z = pos.chunkPosZ;
        Block block = worldCache.getBlock(x, y, z);
    ...
    private static ItemStack getSilkTouchBlock(Block block, int meta) {
        ...
        if (stack == null) {
            Method method = ReflectionHelper
                .findMethod(Block.class, block, new String[] { "func_149644_j", "createStackedBlock" }, int.class);
            try {
