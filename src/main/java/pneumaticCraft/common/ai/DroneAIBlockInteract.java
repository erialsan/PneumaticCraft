...
public class DroneAIBlockInteract extends DroneAIBlockInteraction {
    ...
    public DroneAIBlockInteract(IDroneBase drone, ProgWidgetAreaItemBase widget) {
        super(drone, widget);
        drone.getFakePlayer()
            .setSneaking(((IBlockRightClicker) widget).isSneaking());
    }

    @Override
    protected boolean isValidPosition(ChunkPosition pos) {
        return !visitedPositions.contains(pos)
            && (widget.isItemFilterEmpty() || DroneAIDig.isBlockValidForFilter(drone.getWorld(), drone, pos, widget));
    }
    ...
    protected boolean doBlockInteraction(ChunkPosition pos, double distToBlock) {
        ...
        if (drone.getFakePlayer()
            .getCurrentEquippedItem() != null
            && drone.getFakePlayer()
                .getCurrentEquippedItem().stackSize <= 0) {
            drone.getFakePlayer()
    ...
    public static void transferToDroneFromFakePlayer(IDroneBase drone) {
        ...
        for (int j = 1; j < drone.getFakePlayer().inventory.mainInventory.length; j++) {
            ...
            if (excessStack != null) {
                ItemStack remainder = PneumaticCraftUtils
                    .exportStackToInventory(drone.getInventory(), excessStack, ForgeDirection.UNKNOWN);
                if (remainder != null) {
    ...
    private boolean rightClick(ChunkPosition pos) {
        ...
        try {
            PlayerInteractEvent event = ForgeEventFactory
                .onPlayerInteract(player, Action.RIGHT_CLICK_AIR, x, y, z, faceDir.ordinal(), worldObj);
            ...
            if (stack != null && stack.getItem()
                .onItemUseFirst(stack, player, worldObj, x, y, z, faceDir.ordinal(), dx, dy, dz)) return false;

            if (!worldObj.isAirBlock(x, y, z)
                && block.onBlockActivated(worldObj, x, y, z, player, faceDir.ordinal(), dx, dy, dz)) return false;
            ...
            if (stack != null) {
                ...
                if (stack.getItem()
                    .onItemUse(stack, player, worldObj, useX, useY, useZ, faceDir.ordinal(), dx, dy, dz)) return false;
