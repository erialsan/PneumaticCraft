...
public class LogisticsManager {
    ...
    private void tryProvide(SemiBlockLogistics provider, SemiBlockLogistics requester,
        PriorityQueue<LogisticsTask> tasks) {
        ...
        if (providingInventory != null) {
            ...
            for (int i = 0; i < providingInventory.getSizeInventory(); i++) {
                ...
                if (providingStack != null
                    && (!(provider instanceof ISpecificProvider)
                        || ((ISpecificProvider) provider).canProvide(providingStack))
                    && IOHelper.canExtractItemFromInventory(providingInventory, providingStack, i, 0)) {
        ...
        if (provider.getTileEntity() instanceof IFluidHandler) {
            ...
            for (ForgeDirection d : ForgeDirection.VALID_DIRECTIONS) {
                ...
                if (providingStack != null
                    && (!(provider instanceof ISpecificProvider)
                        || ((ISpecificProvider) provider).canProvide(providingStack))
                    && providingTank.canDrain(d, providingStack.getFluid())) {
                    int requestedAmount = getRequestedAmount(requester, providingStack);
                    if (requestedAmount > 0) {
    ...
    public static int getRequestedAmount(SemiBlockLogistics requester, ItemStack providingStack) {
        ...
        int requestedAmount = requester instanceof ISpecificRequester
            ? ((ISpecificRequester) requester).amountRequested(providingStack)
            : providingStack.stackSize;
    ...
    public static class LogisticsTask implements Comparable<LogisticsTask> {
        ...
        public int compareTo(LogisticsTask task) {
            ...
            int otherValue = task.transportingItem != null ? task.transportingItem.stackSize * 100
                : task.transportingFluid.stack.amount;
