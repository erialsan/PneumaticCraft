...
public class DroneEntityAIInventoryImport extends DroneAIImExBase {
    ...
    private boolean importItems(ChunkPosition pos, boolean simulate) {
        ...
        if (inv != null) {
            Set<Integer> accessibleSlots = PneumaticCraftUtils
                .getAccessibleSlotsForInventoryAndSides(inv, ((ISidedWidget) widget).getSides());
            for (Integer i : accessibleSlots) {
                ItemStack stack = inv.getStackInSlot(i);
                if (stack != null
                    && IOHelper.canExtractItemFromInventory(inv, stack, i, ((ISidedWidget) widget).getSides())) {
