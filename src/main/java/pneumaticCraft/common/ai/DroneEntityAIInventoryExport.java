...
public class DroneEntityAIInventoryExport extends DroneAIImExBase {
    ...
    private boolean export(ChunkPosition pos, boolean simulate) {
        IInventory inv = IOHelper.getInventoryForTE(
            drone.getWorld()
                    ...
                    if (widget.isItemValidForFilters(droneStack)) {
                        for (int side = 0; side < 6; side++) {
                            ...
                            if (((ISidedWidget) widget).getSides()[side] && droneStack != null) {
                                ...
                                int stackSize = drone.getInventory()
                                    .getStackInSlot(i).stackSize
                                    - (remainder == null ? droneStack.stackSize
                                        : droneStack.stackSize - remainder.stackSize);
                                droneStack.stackSize = stackSize;
                                int exportedItems = oldCount - stackSize;
                                if (!simulate) {
                                    drone.getInventory()
                                        .setInventorySlotContents(i, stackSize > 0 ? droneStack : null);
                                    decreaseCount(exportedItems);
                                }
                                if (simulate && exportedItems > 0) return true;
                                if (!(inv instanceof ISidedInventory)) break; // doing it for every side for no side
                                                                              // sensitive inventories would be a waste.
                            }
                        }
