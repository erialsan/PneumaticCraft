...
public class DroneGoToChargingStation extends EntityAIBase {
        ...
        if (drone.getPressure(null) < PneumaticValues.DRONE_LOW_PRESSURE) {
            for (TileEntity te : (List<TileEntity>) drone.worldObj.loadedTileEntityList) {
                if (te instanceof TileEntityChargingStation) {
                    ...
                    if (DroneClaimManager.getInstance(drone.worldObj)
                        .isClaimed(pos)) {
        ...
        Collections.sort(validChargingStations, new Comparator() {
            ...
            public int compare(Object arg1, Object arg2) {
                ...
                return Double.compare(
                    PneumaticCraftUtils
                        .distBetween(te1.xCoord, te1.yCoord, te1.zCoord, drone.posX, drone.posY, drone.posZ),
                    PneumaticCraftUtils
                        .distBetween(te2.xCoord, te2.yCoord, te2.zCoord, drone.posX, drone.posY, drone.posZ));
            }
        });

        for (TileEntityChargingStation station : validChargingStations) {
            boolean protect = PneumaticCraftUtils.getProtectingSecurityStations(
                drone.worldObj,
                station.xCoord,
                station.yCoord,
                station.zCoord,
            ...
            } else if (drone.getPathNavigator()
                .moveToXYZ(station.xCoord, station.yCoord + 1.5, station.zCoord)
                || drone.getPathNavigator()
                    .isGoingToTeleport()) {
                        isExecuting = true;
                        curCharger = station;
                        DroneClaimManager.getInstance(drone.worldObj)
                            .claim(pos);
                        return true;
                    } else {
    ...
    public boolean continueExecuting() {
        if (curCharger.getUpgrades(ItemMachineUpgrade.UPGRADE_DISPENSER_DAMAGE) == 0 || curCharger.isInvalid()) {// If
                                                                                                                 // our
                                                                                                                 // path
                                                                                                                 // was
                                                                                                                 // blocked.
            ...
            .isGoingToTeleport()
            && (drone.getNavigator()
                .getPath() == null || drone.getNavigator()
                    .getPath()
                    .isFinished())) {
                        isExecuting = drone.getPressure(null) < 9.9F
                            && curCharger.getPressure(ForgeDirection.UNKNOWN) > drone.getPressure(null) + 0.1F;
                        if (isExecuting) {
                            chargingTime++;
                            if (chargingTime > 20) {
                                drone.getPathNavigator()
                                    .moveToXYZ(curCharger.xCoord, curCharger.yCoord + 1.5, curCharger.zCoord);
                                if (drone.getNavigator()
                                    .getPath() == null || drone.getNavigator()
                                        .getPath()
                                        .isFinished()) {
                            ...
                            DroneClaimManager.getInstance(drone.worldObj)
                                .claim(new ChunkPosition(curCharger.xCoord, curCharger.yCoord, curCharger.zCoord));
                        }
                        return isExecuting;
                    } else {
                        chargingTime = 0;
