...
public class DroneEntityAIGoToLocation extends EntityAIBase {
    ...
    public boolean shouldExecute() {
        ...
        for (ChunkPosition c : validArea) {
            if (drone.getPosition()
                .squareDistanceTo(c.chunkPosX + 0.5, c.chunkPosY + 0.5, c.chunkPosZ + 0.5) < 0.50) return false;
            if (drone.getPathNavigator()
                .moveToXYZ(c.chunkPosX, c.chunkPosY, c.chunkPosZ)) {
                return !((IGotoWidget) gotoWidget).doneWhenDeparting();
            }
        }
        boolean teleport = drone.getPathNavigator()
            .isGoingToTeleport();
        if (teleport) {
            return true;
        } else {
