...
public class ContainerLogistics extends ContainerPneumaticBase {
    ...
    public ContainerLogistics(InventoryPlayer inventoryPlayer, SemiBlockLogistics logistics) {
        ...
        if (logistics != null) {
            ...
            for (int y = 0; y < 3; y++) {
                for (int x = 0; x < 9; x++) {
                    addSlotToContainer(
                        logistics.canFilterStack() ? new SlotPhantom(requests, y * 9 + x, x * 18 + 8, y * 18 + 29)
    ...
    public static SemiBlockLogistics getLogistics(EntityPlayer player, ItemStack itemRequester) {
        if (itemRequester != null && itemRequester.getItem() instanceof ItemLogisticsFrame) {
            SemiBlockLogistics logistics = (SemiBlockLogistics) SemiBlockManager
                .getSemiBlockForKey(((ItemLogisticsFrame) itemRequester.getItem()).semiBlockId);
    ...
    public void onContainerClosed(EntityPlayer player) {
        if (itemContainer && logistics != null) {
            ...
            NBTTagCompound settingTag = drops.get(0)
                .getTagCompound();
            if (player.getCurrentEquippedItem()
                .hasTagCompound()) {
                NBTTagCompound itemTag = player.getCurrentEquippedItem()
                    .getTagCompound();
                if (settingTag != null) {
