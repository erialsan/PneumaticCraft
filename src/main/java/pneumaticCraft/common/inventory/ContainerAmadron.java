...
public class ContainerAmadron extends ContainerPneumaticBase {
    ...
    public List<AmadronOffer> offers = new ArrayList<AmadronOffer>(
        AmadronOfferManager.getInstance()
            .getAllOffers());
    ...
    public ContainerAmadron(EntityPlayer player) {
        ...
        if (!player.worldObj.isRemote) {
            ...
            Map<AmadronOffer, Integer> shoppingCart = ItemAmadronTablet
                .getShoppingCart(player.getCurrentEquippedItem());
            ...
            currentOffers = AmadronOfferManager.getInstance()
                .countOffers(
                    player.getGameProfile()
                        .getId()
                        .toString());
            maxOffers = PneumaticCraftUtils.isPlayerOp(player) ? Integer.MAX_VALUE
                : AmadronOfferSettings.maxTradesPerPlayer;
        }
    }
            ...
            if (mouseButton == 2) {
                ...
                if (mouseButton == 0) shoppingAmounts[cartSlot] /= 2;
                    ...
                    if (offer instanceof AmadronOfferCustom) {
                        ...
                        if (custom.getPlayerId()
                            .equals(
                                player.getGameProfile()
                                    .getId()
                                    .toString())) {
                            if (AmadronOfferManager.getInstance()
                                .removeStaticOffer(custom)) {
            ...
            for (int i = 0; i < shoppingItems.length; i++) {
                if (shoppingItems[i] >= 0) {
                    ...
                    if (itemPos == null) {
                        ...
                        itemWorld = PneumaticCraftUtils.getWorldForDimension(
                            ItemAmadronTablet.getItemProvidingDimension(player.getCurrentEquippedItem()));
                    }
                    ChunkPosition liquidPos = ItemAmadronTablet
                        .getLiquidProvidingLocation(player.getCurrentEquippedItem());
                    World liquidWorld = null;
                    if (liquidPos != null) {
                        liquidWorld = PneumaticCraftUtils.getWorldForDimension(
                            ItemAmadronTablet.getLiquidProvidingDimension(player.getCurrentEquippedItem()));
                    }
                    EntityDrone drone = retrieveOrderItems(
                        offer,
                        shoppingAmounts[i],
                        itemWorld,
                        itemPos,
                        liquidWorld,
                        liquidPos);
                    if (drone != null) drone.setHandlingOffer(
                        offer,
                        shoppingAmounts[i],
    ...
    public static EntityDrone retrieveOrderItems(AmadronOffer offer, int times, World itemWorld, ChunkPosition itemPos,
        World liquidWorld, ChunkPosition liquidPos) {
        if (offer.getInput() instanceof ItemStack) {
            ...
            return (EntityDrone) PneumaticRegistry.getInstance()
    ...
    public static int capShoppingAmount(AmadronOffer offer, int wantedAmount, IInventory inv,
        IFluidHandler fluidHandler, ContainerAmadron container) {
        return capShoppingAmount(offer, wantedAmount, inv, inv, fluidHandler, fluidHandler, container);
    }

    public static int capShoppingAmount(AmadronOffer offer, int wantedAmount, IInventory inputInv, IInventory outputInv,
        ...
        if (offer.getInput() instanceof ItemStack) {
            if (inputInv != null) {
                ...
                for (int i = 0; i < inputInv.getSizeInventory(); i++) {
                    if (inputInv.getStackInSlot(i) != null && inputInv.getStackInSlot(i)
                        .isItemEqual(searchingItem)
                        && ItemStack.areItemStackTagsEqual(inputInv.getStackInSlot(i), searchingItem)) {
        ...
        if (offer.getOutput() instanceof ItemStack) {
            if (outputInv != null) {
                ...
                if (remainder != null) {
                    int maxAmount = (providingItem.stackSize - remainder.stackSize)
