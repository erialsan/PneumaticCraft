...
public abstract class HeatBehaviourLiquidTransition extends HeatBehaviourLiquid {
    ...
    public boolean isApplicable() {
        ...
        return fluid != null && fluid.getTemperature() >= getMinFluidTemp()
            && fluid.getTemperature() <= getMaxFluidTemp();
    }
    ...
    public void update() {
        if (fluidTemp == -1) {
            ...
            maxExchangedHeat = getMaxExchangedHeat()
                * (HeatExchangerManager.FLUID_RESISTANCE + getHeatExchanger().getThermalResistance());
        }
            ...
            while (!pending.isEmpty()) {
                ...
                for (ForgeDirection d : ForgeDirection.VALID_DIRECTIONS) {
                    ChunkPosition newPos = new ChunkPosition(
                        ...
                        pos.chunkPosZ + d.offsetZ);
                    Block checkingBlock = getWorld().getBlock(newPos.chunkPosX, newPos.chunkPosY, newPos.chunkPosZ);
                    if ((checkingBlock == getBlock()
                        || getBlock() == Blocks.flowing_water && checkingBlock == Blocks.water
                        || getBlock() == Blocks.flowing_lava && checkingBlock == Blocks.lava)
                        && traversed.add(newPos)) {
                        if (FluidUtils
                            .isSourceBlock(getWorld(), newPos.chunkPosX, newPos.chunkPosY, newPos.chunkPosZ)) {
                            getWorld()
                                .setBlock(newPos.chunkPosX, newPos.chunkPosY, newPos.chunkPosZ, turningBlockSource);
                            onLiquidTransition(newPos.chunkPosX, newPos.chunkPosY, newPos.chunkPosZ);
                            return;
                        } else {
                            getWorld()
                                .setBlock(newPos.chunkPosX, newPos.chunkPosY, newPos.chunkPosZ, turningBlockFlowing);
                            onLiquidTransition(newPos.chunkPosX, newPos.chunkPosY, newPos.chunkPosZ);
                            pending.push(newPos);
                        }
                    }
    ...
    protected void onLiquidTransition(int x, int y, int z) {
        NetworkHandler.sendToAllAround(
            new PacketPlaySound(
                ...
                z + 0.5,
                0.5F,
                2.6F + (getWorld().rand.nextFloat() - getWorld().rand.nextFloat()) * 0.8F,
                true),
            getWorld());
        for (int i = 0; i < 8; i++) {
            ...
            NetworkHandler
                .sendToAllAround(new PacketSpawnParticle("largesmoke", randX, y + 1, randZ, 0, 0, 0), getWorld());
        }
    }
}
