...
public class HeatExchangerManager {
    ...
    public void registerBlockExchanger(Block block, IHeatExchanger heatExchanger) {
        ...
        if (block instanceof IHeatExchanger) Log.warning(
            "The block " + block.getUnlocalizedName()
                + " is implementing IHeatExchanger. Therefore you don't need to register it as such");
        if (specialBlockExchangers.containsKey(block)) {
            Log.error(
                "The block " + block.getUnlocalizedName()
                    + " was registered as heat exchanger already! It won't be added!");
        } else {
