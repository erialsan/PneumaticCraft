...
public class AmadronOfferSettings extends JsonConfig {
    ...
    protected void writeToJson(JsonObject json) {
        json.addProperty(
            "description",
            "Various options to limit the ability of the trading system, to manage spam/abuse on public servers. Most can be changed client-side as well to for example stop notifications if the client wishes. Limitations apply to non-OP players only.");
    ...
    protected void readFromJson(JsonObject json) {
        maxTradesPerPlayer = json.get("maxTradesPerPlayer")
            .getAsInt();
        notifyOfTradeAddition = json.get("notifyOfTradeAddition")
            .getAsBoolean();
        notifyOfTradeRemoval = json.get("notifyOfTradeRemoval")
            .getAsBoolean();
        notifyOfDealMade = json.get("notifyOfDealMade")
            .getAsBoolean();
    }
