...
public class Config {
    ...
    public static final String[] CATEGORIES = new String[] { Configuration.CATEGORY_GENERAL, "plant_full-grown_effects",
        "plant_generation_options", "machine_properties", "advanced", "recipe_enabling", "third_party_enabling" };
    public static List<String> NO_MC_RESTART_CATS = Arrays
        .asList(new String[] { "plant_full-grown_effects", "plant_generation_options", "machine_properties" });
    private static ISubConfig[] subConfigs = new ISubConfig[] { new AmadronOfferSettings(),
        AmadronOfferStaticConfig.INSTANCE, new AmadronOfferPeriodicConfig(), new ProgWidgetConfig() };
    ...
    public static void init(File configFile) {
        if (configFile != null) {
            File oldConfig = new File(
                configFile.getAbsolutePath()
                    .replace("PneumaticCraft", "Minemaarten_PneumaticCraft"));// TODO remove legacy.
            ...
            for (ISubConfig subConfig : subConfigs) {
                ...
                try {
                    ...
                    Log.error(
                        "Config file " + subConfig.getFolderName()
                            + " failed to create! Unexpected things can happen!");
                    e.printStackTrace();
                }
            }
        }
        ...
        oilGenerationChance = config
            .get(
                Configuration.CATEGORY_GENERAL,
                "oil_generation_chance",
                15D,
        ...
        useHelmetModel = config.getBoolean(
            "Use Pneumatic Helmet model",
            Configuration.CATEGORY_GENERAL,
            false,
            "When true, the Pneumatic Helmet will be a model. Warning: this model looks far too good to be in MC");
        programmerDifficulty = config.getInt(
        ...
        property = config
            .get("machine_properties", "Kinetic Compressor (Buildcraft --> PneumaticCraft) efficiency", 40);
        ...
        elevatorBaseBlocksPerBase = config.getInt(
            "Height per Elevator Base",
            "machine_properties",
            4,
            1,
            256,
        ...
        enableUpdateChecker = config.get(Configuration.CATEGORY_GENERAL, "Enable Update Checker", true)
            .getBoolean(true);

        enableDroneSuffocationDamage = config
            .get(Configuration.CATEGORY_GENERAL, "Enable Drone Suffocation Damage", true)
            .getBoolean(true);
        enableCreeperDropExplosion = config.getBoolean(
            "Enable Creeper Explosions on seed drop",
            Configuration.CATEGORY_GENERAL,
            false,
            "When true, Creepers when dropping a Creeper Plant Seed will create a tiny explosion.");
        enableCreeperSeedDrop = config.get(Configuration.CATEGORY_GENERAL, "Enable Creeper Seed Drops", false)
            .getBoolean(true);
        enableSlimeSeedDrop = config.get(Configuration.CATEGORY_GENERAL, "Enable Slime Seed Drops", false)
            .getBoolean(true);
        enableEndermanSeedDrop = config.get(Configuration.CATEGORY_GENERAL, "Enable Enderman Seed Drops", false)
            .getBoolean(true);
        enableSquidSeedDrop = config.get(Configuration.CATEGORY_GENERAL, "Enable Squid Seed Drops", false)
            .getBoolean(true);
        includePlantsOnBonemeal = config
            .get(Configuration.CATEGORY_GENERAL, "Include Plastic Plants on bonemealing", false)
            .getBoolean(true);
        allowDirtBonemealing = config
            .get(Configuration.CATEGORY_GENERAL, "Allow dirt to be bonemealed for plastic plants", false)
            .getBoolean(true);

        if (foundConfigWithoutOil && (enableCreeperDropExplosion || enableCreeperSeedDrop
            || enableSlimeSeedDrop
            || enableEndermanSeedDrop
            || enableSquidSeedDrop
            || includePlantsOnBonemeal
            || allowDirtBonemealing)) {
            ...
            config
                .get(
                    ...
                    "When true, Creepers when dropping a Creeper Plant Seed will create a tiny explosion.")
                .set(false);
            config.get(Configuration.CATEGORY_GENERAL, "Enable Creeper Seed Drops", true)
                .set(false);
            config.get(Configuration.CATEGORY_GENERAL, "Enable Slime Seed Drops", true)
                .set(false);
            config.get(Configuration.CATEGORY_GENERAL, "Enable Enderman Seed Drops", true)
                .set(false);
            config.get(Configuration.CATEGORY_GENERAL, "Enable Squid Seed Drops", true)
                .set(false);
            config.get(Configuration.CATEGORY_GENERAL, "Include Plastic Plants on bonemealing", true)
                .set(false);
            config.get(Configuration.CATEGORY_GENERAL, "Allow dirt to be bonemealed for plastic plants", true)
                .set(false);
            shouldDisplayChangeNotification = true;
        }
        ...
        if (shouldDisplayChangeNotification) {
            Log.warning(
                "Disabled world generation of plants and plant mob drops in the config automatically, oil is turned on as replacement. This is only done once, you can change it as you wish now.");
        }

        enableCoalToDiamondsRecipe = config
            .get("recipe_enabling", "8 Block of Coal --> 1 Diamond (Pressure Chamber)", true)
            .getBoolean(true);
        enableElectricCompressorRecipe = config.get("recipe_enabling", "Electric Compressor", true)
            .getBoolean(true);
        enablePneumaticGeneratorRecipe = config.get("recipe_enabling", "Pneumatic Generator", true)
            .getBoolean(true);
        enablePneumaticPumpRecipe = config.get("recipe_enabling", "Pneumatic Pump", true)
            .getBoolean(true);
        enableCreeperPlantMaceratorRecipe = config
            .get("recipe_enabling", "Creeper Plant Seeds --> Gunpowder (IC2 Macerator)", true)
            .getBoolean(true);
        enableHeliumPlantMaceratorRecipe = config
            .get("recipe_enabling", "Helium Plant Seeds --> Glowstone (IC2 Macerator)", true)
            .getBoolean(true);
        enableFlyingFlowerExtractorRecipe = config
            .get("recipe_enabling", "Flying Flower Seeds --> Feather (IC2 Extractor)", true)
            .getBoolean(true);
        enablePropulsionPlantExtractorRecipe = config
            .get("recipe_enabling", "Propulsion Plant Seeds --> Sugar (IC2 Extractor)", true)
            .getBoolean(true);
    ...
    public static void postInit() {
        for (ISubConfig subConfig : subConfigs) {
            try {
                ...
                Log.error(
                    "Config file " + subConfig.getFolderName() + " failed to create! Unexpected things can happen!");
                e.printStackTrace();
            }
        }
    }
    ...
    public static void setProgrammerDifficulty(int difficulty) {
        config.get(Configuration.CATEGORY_GENERAL, "Programmer Difficulty", 0)
            .set(difficulty);
        init(null);
    }
