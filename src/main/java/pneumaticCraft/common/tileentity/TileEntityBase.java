...
public class TileEntityBase extends TileEntity implements IGUIButtonSensitive, IDescSynced, IPeripheral {
    ...
    public void sendDescriptionPacket() {
        // PacketDispatcher.sendPacketToAllAround(xCoord, yCoord, zCoord, TileEntityConstants.PACKET_UPDATE_DISTANCE,
    ...
    public void sendDescPacket(double maxPacketDistance) {
        NetworkHandler.sendToAllAround(
            new PacketDescription(this),
            new TargetPoint(worldObj.provider.dimensionId, xCoord, yCoord, zCoord, maxPacketDistance));
    }
    ...
    public void updateEntity() {
        ...
        if (!worldObj.isRemote) {
            if (this instanceof IHeatExchanger) {
                ((IHeatExchanger) this).getHeatExchangerLogic(ForgeDirection.UNKNOWN)
                    .update();
            }
    ...
    public int getUpgrades(int upgradeDamage) {
        return getUpgrades(
            upgradeDamage,
            this instanceof IPneumaticMachine ? ((IPneumaticMachine) this).getAirHandler()
                .getUpgradeSlots() : getUpgradeSlots());
    }
    ...
    protected int getUpgrades(int upgradeDamage, int... upgradeSlots) {
        ...
        if (inv == null && this instanceof TileEntityPneumaticBase
            && ((TileEntityPneumaticBase) this).parentTile instanceof IInventory)
    ...
    public float getSpeedMultiplierFromUpgrades(int[] upgradeSlots) {
        return (float) Math.pow(
            PneumaticValues.SPEED_UPGRADE_MULTIPLIER,
            Math.min(10, getUpgrades(ItemMachineUpgrade.UPGRADE_SPEED_DAMAGE, upgradeSlots)));
    }

    protected float getSpeedUsageMultiplierFromUpgrades(int[] upgradeSlots) {
        return (float) Math.pow(
            PneumaticValues.SPEED_UPGRADE_USAGE_MULTIPLIER,
            Math.min(10, getUpgrades(ItemMachineUpgrade.UPGRADE_SPEED_DAMAGE, upgradeSlots)));
    }
    ...
    public boolean isGuiUseableByPlayer(EntityPlayer par1EntityPlayer) {
        return worldObj.getTileEntity(xCoord, yCoord, zCoord) != this ? false
            : par1EntityPlayer.getDistanceSq(xCoord + 0.5D, yCoord + 0.5D, zCoord + 0.5D) <= 64.0D;
    }
    ...
    protected void initializeIfHeatExchanger() {
        if (this instanceof IHeatExchanger) {
            initializeHeatExchanger(
                ((IHeatExchanger) this).getHeatExchangerLogic(ForgeDirection.UNKNOWN),
                getConnectedHeatExchangerSides());
        }
    }
    ...
    protected void processFluidItem(int inputSlot, int outputSlot) {
        ...
        if (inv.getStackInSlot(inputSlot) != null) {
                ...
                if (fluid == null) {
                    if (fluidContainer.getItem() instanceof IFluidContainerItem) {
                        ...
                        if (fluid != null && fluidHandler.canFill(ForgeDirection.UNKNOWN, fluid.getFluid())) {
                            ...
                            int availableSpace = tankInfo.capacity
                                - (tankInfo.fluid != null ? tankInfo.fluid.amount : 0);
                            if (availableSpace >= amount) {
                                ...
                                if (fluidContainer.stackSize == 1 || inv.getStackInSlot(outputSlot) == null
                                    || canStack(singleFuelItem, inv.getStackInSlot(outputSlot))) {
                    ...
                    if (tankInfo.capacity - (tankInfo.fluid != null ? tankInfo.fluid.amount : 0) >= amount) {
                        ...
                        if (returnedItem == null || inv.getStackInSlot(outputSlot) == null
                            || canStack(returnedItem, inv.getStackInSlot(outputSlot))) {
                            ...
                            fluidHandler.fill(
                                ...
                                new FluidStack(fluid.getFluid(), amount, fluid.tag),
                                true);
                            inv.getStackInSlot(inputSlot).stackSize--;
            ...
            if (fluidContainer.getItem() instanceof IFluidContainerItem) {
                if (((IFluidContainerItem) fluidContainer.getItem()).getFluid(fluidContainer) == null
                    && (inv.getStackInSlot(outputSlot) == null
                        || canStack(fluidContainer, inv.getStackInSlot(outputSlot)))) {
                    if (inv.getStackInSlot(outputSlot) == null) {
        ...
        if (this instanceof IHeatExchanger) {
            ...
            luaMethods.add(new LuaMethod("getTemperature") {
                ...
                public Object[] call(Object[] args) throws Exception {
                    if (args.length == 0) {
                        return new Object[] { exchanger.getHeatExchangerLogic(ForgeDirection.UNKNOWN)
                            .getTemperature() };
                        ...
                        throw new IllegalArgumentException(
                            "getTemperature method requires 0 or 1 argument (direction: up, down, east, west, north, south!");
                    }
                }
            });
        }
