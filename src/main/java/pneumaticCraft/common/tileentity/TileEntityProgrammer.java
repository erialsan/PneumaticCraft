...
public class TileEntityProgrammer extends TileEntityBase implements IInventory, IGUITextFieldSensitive {
    ...
    public static void updatePuzzleConnections(List<IProgWidget> progWidgets) {
        ...
        for (IProgWidget checkedWidget : progWidgets) {
            ...
            if (parameters != null) {
                for (IProgWidget widget : progWidgets) {
                    if (widget != checkedWidget
                        && checkedWidget.getX() + checkedWidget.getWidth() / 2 == widget.getX()) {
                        for (int i = 0; i < parameters.length; i++) {
                            if (checkedWidget.canSetParameter(i) && parameters[i] == widget.returnType()
            ...
            if (checkedWidget.hasStepOutput()) {
                for (IProgWidget widget : progWidgets) {
                    if (widget.hasStepInput() && widget.getX() == checkedWidget.getX()
        ...
        for (IProgWidget checkedWidget : progWidgets) {
            if (checkedWidget.returnType() == null) { // if it's a program (import/export inventory, attack entity)
                                                      // rather than a parameter (area, item filter).
                Class<? extends IProgWidget>[] parameters = checkedWidget.getParameters();
                if (parameters != null) {
                    for (int i = 0; i < parameters.length; i++) {
        ...
        if (inventory[PROGRAM_SLOT] != null) {
            if (player == null || !player.capabilities.isCreativeMode) {
                ...
                for (ItemStack stack : requiredStacks) {
                    ...
                    if (player != null) {
                        for (int i = 0; i < player.inventory.getSizeInventory(); i++) {
                            if (PneumaticCraftUtils
                                .areStacksEqual(stack, player.inventory.getStackInSlot(i), true, true, false, false)) {
                    ...
                    if (left > 0) {
                        for (ForgeDirection d : ForgeDirection.VALID_DIRECTIONS) {
                            IInventory neighbor = IOHelper.getInventoryForTE(
                                getWorldObj()
                                    .getTileEntity(xCoord + d.offsetX, yCoord + d.offsetY, zCoord + d.offsetZ));
                            for (int slot : IOHelper.getAccessibleSlotsForInventory(neighbor, d.getOpposite())) {
                                if (IOHelper.canExtractItemFromInventory(
                                    neighbor,
                                    stack,
                                    slot,
                                    d.getOpposite()
                                    ...
                                    if (PneumaticCraftUtils
                                        .areStacksEqual(neighborStack, stack, true, true, false, false)) {
                ...
                for (ItemStack stack : returnedStacks) {
                    for (ForgeDirection d : ForgeDirection.VALID_DIRECTIONS) {
                        IInventory neighbor = IOHelper.getInventoryForTE(
                            getWorldObj().getTileEntity(xCoord + d.offsetX, yCoord + d.offsetY, zCoord + d.offsetZ));
                        stack = IOHelper.insert(
                            neighbor,
                            stack,
                            d.getOpposite()
                                .ordinal(),
    ...
    public boolean isItemValidForSlot(int i, ItemStack itemstack) {
        if (i == PROGRAM_SLOT && itemstack != null
            && (!(itemstack.getItem() instanceof IProgrammable)
                || !((IProgrammable) itemstack.getItem()).canProgram(itemstack)))
            return false;
        return true;
    }
    ...
    public boolean previewArea(int widgetX, int widgetY) {
        for (IProgWidget w : progWidgets) {
            if (w.getX() == widgetX && w.getY() == widgetY && w instanceof IAreaProvider) {
                ...
                AreaShowManager.getInstance()
                    .showArea(area, 0x00FF00, this);
            }
        }
        return true;
    }
    ...
    public void saveToHistory() {
        ...
        writeProgWidgetsToNBT(tag);
        if (history.tagCount() == 0 || !history.getCompoundTagAt(historyIndex)
            .equals(tag)) {
