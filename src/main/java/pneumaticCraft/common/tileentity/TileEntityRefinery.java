...
public class TileEntityRefinery extends TileEntityBase
    implements IFluidHandler, IHeatExchanger, IRedstoneControlled, IComparatorSupport {
    ...
    /**
     * The amounts of LPG, Gasoline, Kerosine and Diesel produced per 10mL Oil, depending on how many refineries are
        ...
        if (!worldObj.isRemote) {
            ...
            if (isMaster()) {
                ...
                if (redstoneAllows() && oilTank.getFluidAmount() >= 10) {
                    if (refineries.size() > 1 && refineries.size() <= refiningFluids.length
                        && refine(refineries, true)) {
                        ...
                        while (workTimer >= 20 && oilTank.getFluidAmount() >= 10) {
                            ...
                            for (int i = 0; i < 5; i++) NetworkHandler.sendToAllAround(
                                new PacketSpawnParticle(
                                    "largesmoke",
                                    xCoord + worldObj.rand.nextDouble(),
                                    yCoord + refineries.size(),
    ...
    public boolean refine(List<TileEntityRefinery> refineries, boolean simulate) {
        ...
        for (TileEntityRefinery refinery : refineries) {
            while (outputTable[i] == 0) i++;
            if (outputTable[i]
                != refinery.outputTank.fill(new FluidStack(refiningFluids[i], outputTable[i]), !simulate)) return false;
            i++;
        }
        return true;
    }
    ...
    public boolean redstoneAllows() {
        ...
        TileEntityRefinery refinery = this;
        while (poweredRedstone == 0
            && refinery.getTileCache()[ForgeDirection.UP.ordinal()].getTileEntity() instanceof TileEntityRefinery) {
            refinery = (TileEntityRefinery) refinery.getTileCache()[ForgeDirection.UP.ordinal()].getTileEntity();
    ...
    public FluidStack drain(ForgeDirection from, FluidStack resource, boolean doDrain) {
        return outputTank.getFluid() != null && outputTank.getFluid()
            .isFluidEqual(resource) ? outputTank.drain(resource.amount, doDrain) : null;
    }
