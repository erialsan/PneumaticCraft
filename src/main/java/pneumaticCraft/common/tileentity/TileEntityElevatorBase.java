...
public class TileEntityElevatorBase extends TileEntityPneumaticBase
    implements IInventory, IGUITextFieldSensitive, IRedstoneControlled, IMinWorkingPressure {
    ...
    private List<TileEntityElevatorBase> multiElevators;// initialized when multiple elevators are connected in a
                                                        // multiblock manner.
    @GuiSynced
    ...
    public TileEntityElevatorBase() {
        super(
            PneumaticValues.DANGER_PRESSURE_ELEVATOR,
            PneumaticValues.MAX_PRESSURE_ELEVATOR,
            PneumaticValues.VOLUME_ELEVATOR);
    ...
    public void updateEntity() {
        ...
        if (worldObj.isRemote && worldObj.getTotalWorldTime() % 60 == 0) coreElevator = null;// reset this because the
                                                                                             // client doesn't get
                                                                                             // notified of neighbor
                                                                                             // block updates.
        if (isCoreElevator()) {
            super.updateEntity();
            if (!worldObj.isRemote && isControlledByRedstone()) {
                ...
                if (targetExtension > oldExtension
                    && getPressure(ForgeDirection.UNKNOWN) < PneumaticValues.MIN_PRESSURE_ELEVATOR)
            ...
            if (extension < targetExtension) {
                ...
                addAir(
                    (int) ((oldExtension - extension) * PneumaticValues.USAGE_ELEVATOR
                        * (getSpeedUsageMultiplierFromUpgrades(getUpgradeSlots()) / speedMultiplier)),
                    ForgeDirection.UNKNOWN);// substract the ascended distance from the air reservoir.
            }
            if (extension > targetExtension) {
                soundName = Sounds.ELEVATOR_MOVING;
                if (extension > targetExtension + TileEntityConstants.ELEVATOR_SLOW_EXTENSION) {
    ...
    private void movePlayerDown() {
        ...
        AxisAlignedBB aabb = AxisAlignedBB
            .getBoundingBox(xCoord, yCoord + 1, zCoord, xCoord + 1, yCoord + oldExtension + 1.05F, zCoord + 1);
    ...
    private void moveEntities(float moveBy) {
        AxisAlignedBB aabb = AxisAlignedBB
            .getBoundingBox(xCoord, yCoord + 1, zCoord, xCoord + 1, yCoord + extension + 1, zCoord + 1);
        List<Entity> entityList = worldObj.getEntitiesWithinAABBExcludingEntity(null, aabb);
        for (Entity entity : entityList) {
            if (entity instanceof EntityPlayer) {
    ...
    public void updateRedstoneInputLevel() {
        ...
        for (TileEntityElevatorBase base : multiElevators) {
            ...
            while (worldObj.getBlock(base.xCoord, base.yCoord + i, base.zCoord) == Blockss.elevatorBase) {
                maxRedstone = Math.max(
                    maxRedstone,
                    PneumaticCraftUtils.getRedstoneLevel(worldObj, base.xCoord, base.yCoord + i, base.zCoord));
                i--;
            }
        }
        for (TileEntityElevatorBase base : multiElevators) {
    ...
    public void updateFloors() {
                ...
                for (TileEntityElevatorBase base : multiElevators) {
                    for (ForgeDirection dir : ForgeDirection.VALID_DIRECTIONS) {
                        if (dir != ForgeDirection.UP && dir != ForgeDirection.DOWN) {
                            if (base.worldObj
                                .getBlock(base.xCoord + dir.offsetX, base.yCoord + i + 2, base.zCoord + dir.offsetZ)
                                == Blockss.elevatorCaller) {
                                callerList.add(
                                    new ChunkPosition(
                                        base.xCoord + dir.offsetX,
                                        base.yCoord + i + 2,
                                        base.zCoord + dir.offsetZ));
        ...
        for (int j = 0; j < columns; j++) {
            for (int i = j * 12; i < floorHeights.length && i < j * 12 + 12; i++) {
                elevatorButtons[i] = new TileEntityElevatorCaller.ElevatorButton(
                    0.2D + 0.6D / columns * j,
                    0.5D + (Math.min(floorHeights.length, 12) - 2) * (buttonSpacing + buttonHeight) / 2
                        - i % 12 * (buttonHeight + buttonSpacing),
                    0.58D / columns,
                    buttonHeight,
                    i,
                    floorHeights[i]);
                elevatorButtons[i].setColor(
                    floorHeights[i] == targetExtension ? 0 : 1,
                    1,
                    floorHeights[i] == targetExtension ? 0 : 1);
        ...
        for (ChunkPosition p : callerList) {
            ...
            if (te instanceof TileEntityElevatorCaller) {
                ...
                if (callerFloor == -1) {
                    Log.error(
                        "Error while updating elevator floors! This will cause a indexOutOfBoundsException, index = -1");
                }
                ((TileEntityElevatorCaller) te).setEmittingRedstone(
                    PneumaticCraftUtils.areFloatsEqual(targetExtension, extension, 0.1F)
    ...
    public void onDescUpdate() {
        baseCamo = inventory[4] != null && inventory[4].getItem() instanceof ItemBlock
            ? ((ItemBlock) inventory[4].getItem()).field_150939_a
            : null;
        Block newFrameCamo = inventory[5] != null && inventory[5].getItem() instanceof ItemBlock
            ? ((ItemBlock) inventory[5].getItem()).field_150939_a
            : null;
    ...
    public boolean isItemValidForSlot(int i, ItemStack itemstack) {
        return itemstack.getItem() == Itemss.machineUpgrade && i < 4
            || itemstack.getItem() instanceof ItemBlock && i >= 4;
    }
    ...
    public boolean isConnectedTo(ForgeDirection side) {
        return side != ForgeDirection.UP && side != ForgeDirection.DOWN
            || worldObj.getBlock(xCoord, yCoord + side.offsetY, zCoord) != Blockss.elevatorBase;
    }
