...
public class TileEntityUniversalSensor extends TileEntityPneumaticBase
    implements IInventory, IRangeLineShower, IGUITextFieldSensitive, IMinWorkingPressure, IRedstoneControl {
    ...
    private final List<IComputerAccess> attachedComputers = new ArrayList<IComputerAccess>(); // keep track of the
                                                                                              // computers so we can
                                                                                              // raise a os.pullevent.

    public TileEntityUniversalSensor() {
        super(
            PneumaticValues.DANGER_PRESSURE_UNIVERSAL_SENSOR,
            PneumaticValues.MAX_PRESSURE_UNIVERSAL_SENSOR,
            PneumaticValues.VOLUME_UNIVERSAL_SENSOR);
    ...
    public void updateEntity() {
        ...
        if (!worldObj.isRemote) {
            ticksExisted++;
            ISensorSetting sensor = SensorHandler.instance()
                .getSensorFromPath(sensorSetting);
            if (sensor != null && getPressure(ForgeDirection.UNKNOWN) > PneumaticValues.MIN_PRESSURE_UNIVERSAL_SENSOR) {
                ...
                if (sensor instanceof IPollSensorSetting) {
                    ...
                    if (ticksExisted % ((IPollSensorSetting) sensor).getPollFrequency(this) == 0) {
                        int newRedstoneStrength = ((IPollSensorSetting) sensor)
    ...
    public void onEvent(Event event) {
        ISensorSetting sensor = SensorHandler.instance()
            .getSensorFromPath(sensorSetting);
        if (sensor != null && sensor instanceof IEventSensorSetting
            && getPressure(ForgeDirection.UNKNOWN) > PneumaticValues.MIN_PRESSURE_UNIVERSAL_SENSOR) {
            int newRedstoneStrength = ((IEventSensorSetting) sensor)
    ...
    private boolean setSensorSetting(ISensorSetting sensor) {
        if (areGivenUpgradesInserted(
            SensorHandler.instance()
                .getRequiredStacksFromText(sensor.getSensorPath()))) {
    ...
    public void handleGUIButtonPress(int buttonID, EntityPlayer player) {
        if (buttonID >= 10 && buttonID % 10 == 0) {
            String[] directories = SensorHandler.instance()
                .getDirectoriesAtLocation(getSensorSetting());
            ...
            for (int i : upgradeSlots) {
                if (inv.getStackInSlot(i) != null) {
                    if (inv.getStackInSlot(i)
                        .getItem() == Itemss.machineUpgrade
                        && inv.getStackInSlot(i)
                            .getItemDamage() == upgradeDamage) {
                        upgrades += inv.getStackInSlot(i).stackSize;
                    } else if (upgradeDamage == -1 && inv.getStackInSlot(i)
                        .getItem() == Itemss.GPSTool) {
                            upgrades += inv.getStackInSlot(i).stackSize;
                        }
                }
            }
    ...
    public void setInventorySlotContents(int slot, ItemStack itemStack) {
        ...
        if (!worldObj.isRemote && !getSensorSetting().equals("")
            && !areGivenUpgradesInserted(
                SensorHandler.instance()
                    .getRequiredStacksFromText(getSensorSetting()))) {
            setSensorSetting("");
        }
        positions = getGPSPositionsStatic(this, getRange());
    }
    ...
    private static Set<ChunkPosition> getGPSPositionsStatic(TileEntityUniversalSensor teUs, int sensorRange) {
        ...
        for (int i = TileEntityUniversalSensor.UPGRADE_SLOT_1; i <= TileEntityUniversalSensor.UPGRADE_SLOT_4; i++) {
            ...
            if (gps != null && gps.getItem() == Itemss.GPSTool) {
                ...
                if (pos != null && Math.abs(pos.chunkPosX - teUs.xCoord) <= sensorRange
                    && Math.abs(pos.chunkPosY - teUs.yCoord) <= sensorRange
                    && Math.abs(pos.chunkPosZ - teUs.zCoord) <= sensorRange) {
    ...
    public void addLuaMethods() {
            ...
            public Object[] call(Object[] args) throws Exception {
                if (args.length == 1) {
                    ...
                    if (args[0] instanceof String) {
                        sensor = SensorHandler.instance()
                            .getSensorForName((String) args[0]);
                    } else {
                        sensor = SensorHandler.instance()
                            .getSensorByIndex(((Double) args[0]).intValue() - 1);
                    }
                    ...
                    throw new IllegalArgumentException(
                        "setSensor needs one argument(a number as index, or a sensor name).");
                }
            }
        });
        ...
        luaMethods.add(new LuaMethod("getSensor") {
            ...
            public Object[] call(Object[] args) throws Exception {
                if (args.length == 0) {
                    ISensorSetting curSensor = SensorHandler.instance()
                        .getSensorFromPath(getSensorSetting());
                    return curSensor == null ? null
                        : new Object[] { getSensorSetting().substring(getSensorSetting().lastIndexOf('/') + 1) };
                } else {
        ...
        luaMethods.add(new LuaMethod("isSensorEventBased") {
            ...
            public Object[] call(Object[] args) throws Exception {
                if (args.length == 0) {
                    return new Object[] { SensorHandler.instance()
                        .getSensorFromPath(getSensorSetting()) instanceof IEventSensorSetting };
                } else {
        ...
        luaMethods.add(new LuaMethod("getSensorValue") {
            ...
            public Object[] call(Object[] args) throws Exception {
                if (args.length == 0) {
                    ISensorSetting s = SensorHandler.instance()
                        .getSensorFromPath(getSensorSetting());
                    if (s instanceof IPollSensorSetting) {
                        ...
                        throw new IllegalArgumentException(
                            "The selected sensor is pull event based. You can't poll the value.");
                    } else {
        ...
        luaMethods.add(new LuaMethod("setGPSToolCoordinate") {
            ...
            public Object[] call(Object[] args) throws Exception {
                if (args.length == 4) {
                    ItemStack stack = getStackInSlot(((Double) args[0]).intValue() - 1); // minus one, as lua is
                                                                                         // 1-oriented.
                    if (stack != null && stack.getItem() == Itemss.GPSTool) {
                        ItemGPSTool.setGPSLocation(
                            stack,
