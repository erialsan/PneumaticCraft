...
public class TileEntitySentryTurret extends TileEntityBase
    implements IRedstoneControlled, ISidedInventory, IGUITextFieldSensitive {
            ...
            if (getMinigun().getAttackTarget() == null && redstoneAllows()) {
                ...
                if (worldObj.getTotalWorldTime() % 20 == 0) {
                    List<EntityLivingBase> entities = worldObj
                        .selectEntitiesWithinAABB(EntityLivingBase.class, getTargetingBoundingBox(), entitySelector);
                    if (entities.size() > 0) {
                        Collections.sort(entities, new TargetSorter());
                        getMinigun().setAttackTarget(entities.get(0));
                        targetEntityId = entities.get(0)
                            .getEntityId();
                    }
                }
            } else {
            ...
            if (target != null) {
                if (!redstoneAllows() || !entitySelector.isEntityApplicable(target)) {
                    ...
                    if (worldObj.getTotalWorldTime() % 5 == 0) {
                        getFakePlayer().setPosition(xCoord + 0.5, yCoord + 0.5, zCoord + 0.5); // Make sure the
                                                                                               // knockback has the
                                                                                               // right direction.
    ...
    private boolean canSeeEntity(Entity entity) {
        Vec3 entityVec = Vec3.createVectorHelper(
    ...
    private class SentryTurretEntitySelector extends StringFilterEntitySelector {
        ...
        private boolean inRange(Entity entity) {
            return PneumaticCraftUtils
                .distBetween(new ChunkPosition(xCoord, yCoord, zCoord), entity.posX, entity.posY, entity.posZ) <= range;
        }

        private boolean isExcludedBySecurityStations(EntityPlayer player) {
            Iterator<TileEntitySecurityStation> iterator = PneumaticCraftUtils
                .getSecurityStations(worldObj, xCoord, yCoord, zCoord, false)
                .iterator();
            if (iterator.hasNext()) { // When there are Security Stations, all stations need to be allowing the player.
                while (iterator.hasNext()) {
                    if (!iterator.next()
                        .doesAllowPlayer(player)) return false;
                }
                return true;
            } else {
                return false; // When there are no Security Stations at all, the player isn't automatically 'allowed to
                              // live'.
            }
        }
    }
