...
public class TileEntityPressureChamberInterface extends TileEntityPressureChamberWall
    implements ISidedInventory, IGUITextFieldSensitive, IRedstoneControlled {
            ...
            if (interfaceMode != EnumInterfaceMode.NONE) {
                if (inventory[0] != null && ++inputTimeOut > 10) {
                    ...
                    if (outputProgress == 0) {
                        ...
                        if (interfaceMode == EnumInterfaceMode.EXPORT && inputProgress == MAX_PROGRESS
                            && redstoneAllows()) {
    ...
    private void importFromChamber(TileEntityPressureChamberValve core) {
        ...
        for (ItemStack chamberStack : chamberStacks) {
            if ((inventory[0] == null || inventory[0].isItemEqual(chamberStack))
                && isItemValidForSlot(0, chamberStack)) {
                int maxAllowedItems = Math.abs(core.currentAir) / PneumaticValues.USAGE_CHAMBER_INTERFACE;
                if (maxAllowedItems > 0) {
                    if (inventory[0] != null) maxAllowedItems = Math
                        .min(maxAllowedItems, chamberStack.getMaxStackSize() - inventory[0].stackSize);
                    int transferedItems = Math.min(chamberStack.stackSize, maxAllowedItems);
                    core.addAir(
                        (core.currentAir > 0 ? -1 : 1) * transferedItems * PneumaticValues.USAGE_CHAMBER_INTERFACE,
                        ForgeDirection.UNKNOWN);
                    ItemStack transferedStack = chamberStack.copy()
                        .splitStack(transferedItems);
        ...
        if (valve != null) {
            for (int i = 0; i < 6; i++) {
                ...
                if (valve.isCoordWithinChamber(worldObj, x, y, z)) {
                    enoughAir = Math.abs(valve.currentAir)
                        > inventory[0].stackSize * PneumaticValues.USAGE_CHAMBER_INTERFACE;
                    if (enoughAir) {
                        valve.addAir(
                            (valve.currentAir > 0 ? -1 : 1) * inventory[0].stackSize
                                * PneumaticValues.USAGE_CHAMBER_INTERFACE,
                            ForgeDirection.UNKNOWN);
    ...
    public List<String> getProblemStat() {
        ...
        if (interfaceMode == EnumInterfaceMode.NONE) {
            textList.addAll(
                PneumaticCraftUtils
                    .convertStringIntoList("\u00a77The Interface can't work.", GuiConstants.maxCharPerLineLeft));
            textList.addAll(
                PneumaticCraftUtils.convertStringIntoList(
                    "\u00a70-The Interface is not in a properly formed Pressure Chamber, and/or",
                    GuiConstants.maxCharPerLineLeft));
            textList.addAll(
                PneumaticCraftUtils.convertStringIntoList(
                    "\u00a70-The Interface is not adjacent to an air block of the Pressure Chamber, and/or",
                    GuiConstants.maxCharPerLineLeft));
            textList.addAll(
                PneumaticCraftUtils.convertStringIntoList(
                    "\u00a70-The Interface isn't orientated right.",
                    GuiConstants.maxCharPerLineLeft));
            ...
            textList.addAll(
                PneumaticCraftUtils.convertStringIntoList(
                    "\u00a77There's not enough pressure in the Pressure Chamber to move the items.",
                    GuiConstants.maxCharPerLineLeft));
            textList.addAll(
                PneumaticCraftUtils.convertStringIntoList(
                    "\u00a70Apply more pressure to the Pressure Chamber. The required pressure is dependent on the amount of items being transported.",
                    GuiConstants.maxCharPerLineLeft));
        }
        return textList;
    }
    ...
    public boolean isItemValidForSlot(int slot, ItemStack iStack) {
        ...
        switch (filterMode) {
            ...
            case CREATIVE_TAB:
                try {
                    int itemCreativeTabIndex = iStack.getItem()
                        .getCreativeTab() != null ? iStack.getItem()
                            .getCreativeTab()
                            .getTabIndex() : -1;
            ...
            case NAME_BEGINS:
                return iStack.getDisplayName()
                    .toLowerCase()
                    .startsWith(itemNameFilter.toLowerCase());
            case NAME_CONTAINS:
                return iStack.getDisplayName()
                    .toLowerCase()
    ...
    public boolean canInsertItem(int i, ItemStack itemstack, int j) {
        return inputProgress == MAX_PROGRESS && j == getRotation().getOpposite()
            .ordinal() && redstoneAllows();
    }
    ...
    public void handleGUIButtonPress(int guiID, EntityPlayer player) {
        if (guiID == 1) {
            ...
            isItemValidForSlot(0, new ItemStack(Items.stick));// when an SideOnly exception is thrown this method
                                                              // automatically will set the filter mode to Item.
