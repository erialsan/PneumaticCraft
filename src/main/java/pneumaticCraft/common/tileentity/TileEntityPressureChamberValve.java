...
public class TileEntityPressureChamberValve extends TileEntityPneumaticBase implements IInventory, IMinWorkingPressure {
    ...
    public TileEntityPressureChamberValve() {
        super(
            PneumaticValues.DANGER_PRESSURE_PRESSURE_CHAMBER,
            PneumaticValues.MAX_PRESSURE_PRESSURE_CHAMBER,
            PneumaticValues.VOLUME_PRESSURE_CHAMBER);
    ...
    protected int getVolumeFromUpgrades(int[] upgradeSlots) {
        return super.getVolumeFromUpgrades(getUpgradeSlots()) + (multiBlockSize > 0
            ? (int) Math.pow(multiBlockSize - 2, 3) * PneumaticValues.VOLUME_PRESSURE_CHAMBER_PER_EMPTY
            : 0);
    }
    ...
    public void updateEntity() {
        ...
        if (!worldObj.isRemote) {
            ...
            boolean[] connected = new boolean[] { true, true, true, true, true, true };// assume we are not leaking at
                                                                                       // any side
            ...
            List<Pair<ForgeDirection, IAirHandler>> teList = super.getConnectedPneumatics();// we need the super method,
                                                                                            // as the overridden method
                                                                                            // adds the other valves.
            ...
            for (int i = 0; i < 6; i++) if (!connected[i]) airLeak(ForgeDirection.getOrientation(i));
        }
        super.updateEntity();
        ...
        if (multiBlockSize != 0 && !worldObj.isRemote) {
            ...
            for (PressureChamberRecipe recipe : PressureChamberRecipe.chamberRecipes) {
                ...
                boolean isSufficientPressureInChamberFlag = recipe.pressure <= getPressure(ForgeDirection.UNKNOWN)
                    && recipe.pressure > 0F
                    || recipe.pressure >= getPressure(ForgeDirection.UNKNOWN) && recipe.pressure < 0F;
            ...
            for (IPressureChamberRecipe recipe : PressureChamberRecipe.specialRecipes) {
                ...
                boolean isSufficientPressureInChamberFlag = recipe.getCraftingPressure()
                    <= getPressure(ForgeDirection.UNKNOWN) && recipe.getCraftingPressure() > 0F
                    || recipe.getCraftingPressure() >= getPressure(ForgeDirection.UNKNOWN)
                        && recipe.getCraftingPressure() < 0F;
                if (isValidRecipeInChamberFlag) {
                    isValidRecipeInChamber = true;
            ...
            if (getPressure(ForgeDirection.UNKNOWN) > PneumaticValues.MAX_PRESSURE_LIVING_ENTITY) {
                AxisAlignedBB bbBox = AxisAlignedBB.getBoundingBox(
                    multiBlockX + 1,
                    multiBlockY + 1,
                    multiBlockZ + 1,
                    multiBlockX + multiBlockSize - 1,
                    multiBlockY + multiBlockSize - 1,
                    multiBlockZ + multiBlockSize - 1);
                ...
                for (EntityLivingBase entity : entities) {
                    ...
                    entity.attackEntityFrom(
                        DamageSourcePneumaticCraft.pressure,
                        (int) (getPressure(ForgeDirection.UNKNOWN) * 2D));
                }
            }
        }
        ...
        AxisAlignedBB bbBox = AxisAlignedBB.getBoundingBox(
            ...
            multiBlockY,
            multiBlockZ,
            multiBlockX + multiBlockSize,
            multiBlockY + multiBlockSize,
            multiBlockZ + multiBlockSize);
    ...
    public ItemStack[] getStacksInChamber() {
         ...
         * ItemStack stack = new ItemStack(blockID, 1, worldObj.getBlockMetadata(i + multiBlockX + 1, j + multiBlockY +
         * 1, k + multiBlockZ + 1));
         * if(stack.getItem() != null) addStackToList(stackList, stack);//getItem() shouldn't ever be able to be null,
         * but still some defence cuz https://github.com/MineMaarten/PneumaticCraft-API/issues/60
         * }
         * }
         * }
         * }
    ...
    public double[] clearStacksInChamber(Object... stacksToClear) {
        ...
        double[] outputPosition = new double[] { multiBlockX + multiBlockSize / 2D, multiBlockY + 1.2D,
            multiBlockZ + multiBlockSize / 2D };
    ...
    public void readFromNBT(NBTTagCompound tag) {
        ...
        for (int i = 0; i < tagList2.tagCount(); ++i) {
            ...
            if (tagCompound != null) {
                nbtValveList.add(
                    new int[] { tagCompound.getInteger("xCoord"), tagCompound.getInteger("yCoord"),
    ...
    public void onMultiBlockBreak() {
        dropInventory(
            worldObj,
            multiBlockX + multiBlockSize / 2D,
            multiBlockY + multiBlockSize / 2D,
            multiBlockZ + multiBlockSize / 2D);
        invalidateMultiBlock();
    }
        ...
        for (int x = 0; x < size; x++) {
            for (int y = 0; y < size; y++) {
                for (int z = 0; z < size; z++) {
                    ...
                    if (world.getBlock(x + baseX, y + baseY, z + baseZ) != Blockss.pressureChamberWall
                        && world.getBlock(x + baseX, y + baseY, z + baseZ) != Blockss.pressureChamberValve
                        && world.getBlock(x + baseX, y + baseY, z + baseZ) != Blockss.pressureChamberInterface) {
                        return false;
                        ...
                        ForgeDirection facing = ForgeDirection
                            .getOrientation(world.getBlockMetadata(x + baseX, y + baseY, z + baseZ));
                        if (xMid && yMid && (facing == ForgeDirection.NORTH || facing == ForgeDirection.SOUTH)
                            || xMid && zMid && (facing == ForgeDirection.UP || facing == ForgeDirection.DOWN)
                            || yMid && zMid && (facing == ForgeDirection.EAST || facing == ForgeDirection.WEST)) {
                        ...
                        if (te instanceof TileEntityPressureChamberWall
                            && ((TileEntityPressureChamberWall) te).getCore() != null) {
                            return false;
                        }
                    }
                }
    ...
    public boolean isCoordWithinChamber(World world, int x, int y, int z) {
        if (x > multiBlockX && x < multiBlockX + multiBlockSize - 1
            && y > multiBlockY
            && y < multiBlockY + multiBlockSize - 1
            && z > multiBlockZ
            && z < multiBlockZ + multiBlockSize - 1) {
            return true;
        }
        return false;
    }
