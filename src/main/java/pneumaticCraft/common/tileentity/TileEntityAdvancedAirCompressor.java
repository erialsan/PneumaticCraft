...
public class TileEntityAdvancedAirCompressor extends TileEntityAirCompressor implements IHeatExchanger {
    ...
    public static int getEfficiency(double temperature) {
        return MathHelper.clamp_int((int) ((625 - temperature) / 3), 0, 100);// 0% efficiency at > 350 degree C, 100% at
                                                                             // < 50 degree C.
    }

    @Override
    protected float getSpeedUsageMultiplierFromUpgrades(int[] upgradeSlots) {
        return getSpeedMultiplierFromUpgrades(upgradeSlots);// return the same as the speed multiplier, so adding speed
                                                            // upgrades doesn't affect the efficiency.
    }
