...
public class TileEntityOmnidirectionalHopper extends TileEntityBase implements ISidedInventory, IRedstoneControlled {
    ...
    protected boolean suckInItem(int maxItems) {
        ...
        for (int i = 0; i < maxItems; i++) {
            if (hasEmptySlot()) {
                ItemStack extracted = IOHelper.extractOneItem(inputInv, inputDir.getOpposite(), true);// simulate
                                                                                                      // extraction from
                                                                                                      // the neighbor.
                if (extracted != null) {
                    ItemStack inserted = IOHelper.insert(this, extracted, ForgeDirection.UNKNOWN, false);// if we can
                                                                                                         // insert the
                                                                                                         // item in this
                                                                                                         // hopper.
                    if (inserted == null) {
                        IOHelper.extractOneItem(inputInv, inputDir.getOpposite(), false); // actually retrieve it from
                                                                                          // the neighbor.
                        success = true;
                    } else {
                        break;
                ...
                for (int slot = 0; slot < 5; slot++) {
                    ...
                    stack = stack.copy();
                    stack.stackSize = 1;
                    ItemStack extracted = IOHelper.extract(inputInv, inputDir.getOpposite(), stack, true, true);// simulate
                                                                                                                // extraction
                                                                                                                // from
                                                                                                                // the
                                                                                                                // neighbor.
                    if (extracted != null) {
                        ItemStack inserted = IOHelper.insert(this, extracted, ForgeDirection.UNKNOWN, false);// if we
                                                                                                             // can
                                                                                                             // insert
                                                                                                             // the item
                                                                                                             // in this
                                                                                                             // hopper.
                        if (inserted == null) {
                            IOHelper.extract(inputInv, inputDir.getOpposite(), stack, true, false); // actually retrieve
                                                                                                    // it from the
                                                                                                    // neighbor.
                            success = true;
                            break;
        ...
        for (EntityItem entity : getNeighborItems(this, inputDir)) {
            if (!entity.isDead) {
                ...
                if (remainder == null) {
                    entity.setDead();
                    success = true;// Don't set true when the stack could not be fully consumes, as that means next
                                   // insertion there won't be any room.
                }
            }
        }
    ...
    public static List<EntityItem> getNeighborItems(TileEntity te, ForgeDirection inputDir) {
        AxisAlignedBB box = AxisAlignedBB.getBoundingBox(
            te.xCoord + inputDir.offsetX,
            te.yCoord + inputDir.offsetY,
            te.zCoord + inputDir.offsetZ,
            te.xCoord + inputDir.offsetX + 1,
            te.yCoord + inputDir.offsetY + 1,
            te.zCoord + inputDir.offsetZ + 1);
