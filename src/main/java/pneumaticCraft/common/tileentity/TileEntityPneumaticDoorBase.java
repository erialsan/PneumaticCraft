...
public class TileEntityPneumaticDoorBase extends TileEntityPneumaticBase
    implements IInventory, IRedstoneControl, IMinWorkingPressure {
    ...
    public TileEntityPneumaticDoorBase() {
        super(
            PneumaticValues.DANGER_PRESSURE_PNEUMATIC_DOOR,
            PneumaticValues.MAX_PRESSURE_PNEUMATIC_DOOR,
            PneumaticValues.VOLUME_PNEUMATIC_DOOR);
    ...
    public void updateEntity() {
            ...
            if (getPressure(ForgeDirection.UNKNOWN) >= PneumaticValues.MIN_PRESSURE_PNEUMATIC_DOOR) {
                if (worldObj.getTotalWorldTime() % 60 == 0) {
                    TileEntity te = worldObj
                        .getTileEntity(orientation.offsetX * 3 + xCoord, yCoord, orientation.offsetZ * 3 + zCoord);
        ...
        if (!worldObj.isRemote) addAir(
            (int) (-Math.abs(oldProgress - progress) * PneumaticValues.USAGE_PNEUMATIC_DOOR
        ...
        if (oldCamo != inventory[CAMO_SLOT]) {
            ...
            worldObj.setBlockMetadataWithNotify(
                ...
                yCoord,
                zCoord,
                inventory[CAMO_SLOT] != null ? inventory[CAMO_SLOT].getItemDamage() % 16 : 0,
                2);
    ...
    private boolean shouldOpen() {
        switch (redstoneMode) {
            case 0:
            case 1:
                int range = TileEntityConstants.RANGE_PNEUMATIC_DOOR_BASE
                    + this.getUpgrades(ItemMachineUpgrade.UPGRADE_RANGE);
                ...
                for (EntityPlayer player : players) {
                    if (PneumaticCraftUtils
                        .getProtectingSecurityStations(worldObj, xCoord, yCoord, zCoord, player, false, false) == 0) {
                        if (redstoneMode == 0) {
                            return true;
                        } else {
                            ((BlockPneumaticDoor) Blockss.pneumaticDoor).isTrackingPlayerEye = true;
                            ChunkPosition lookedPosition = PneumaticCraftUtils
                                .getEntityLookedBlock(player, range * 1.41F); // max range = range * sqrt(2).
                            ...
                            if (lookedPosition != null) {
                                if (lookedPosition.equals(new ChunkPosition(xCoord, yCoord, zCoord))) {
                                    ...
                                    if (door != null) {
                                        if (lookedPosition.equals(
                                            new ChunkPosition(door.xCoord, door.yCoord, door.zCoord))) return true;
                                        if (lookedPosition.equals(
                                            new ChunkPosition(
                                                door.xCoord,
                                                door.yCoord + (door.getBlockMetadata() < 6 ? 1 : -1),
                                                door.zCoord)))
                                            return true;
                                    }
                                }
    ...
    public void setNeighborOpening(boolean opening) {
        if (doubleDoor != null
            && doubleDoor.getPressure(ForgeDirection.UNKNOWN) >= PneumaticValues.MIN_PRESSURE_PNEUMATIC_DOOR) {
    ...
    private TileEntityPneumaticDoor getDoor() {
        ...
        if (te instanceof TileEntityPneumaticDoor) {
            if (orientation.getRotation(ForgeDirection.UP) == ForgeDirection.getOrientation(te.getBlockMetadata())
                && !((TileEntityPneumaticDoor) te).rightGoing) {
                return (TileEntityPneumaticDoor) te;
            } else
                if (orientation.getRotation(ForgeDirection.DOWN) == ForgeDirection.getOrientation(te.getBlockMetadata())
                    && ((TileEntityPneumaticDoor) te).rightGoing) {
                        return (TileEntityPneumaticDoor) te;
                    }
        }
