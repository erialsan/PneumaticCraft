...
public class TileEntitySecurityStation extends TileEntityBase
    implements ISidedInventory, IGUITextFieldSensitive, IRangeLineShower, IRedstoneControl {
    ...
    public final List<GameProfile> hackedUsers = new ArrayList<GameProfile>(); // Stores all the users that have hacked
                                                                               // this Security Station.
    public final List<GameProfile> sharedUsers = new ArrayList<GameProfile>(); // Stores all the users that have been
                                                                               // allowed by the stationOwner.
    @GuiSynced
    private int rebootTimer;// When the player decides to reset the station, this variable will hold the remaining
                            // reboot time.
    @GuiSynced
    ...
    public void updateEntity() {
        ...
        securityRange = Math.min(
            2 + getUpgrades(ItemMachineUpgrade.UPGRADE_RANGE, getUpgradeSlots()),
            TileEntityConstants.SECURITY_STATION_MAX_RANGE);
    ...
    /**
     * Will initiate the wireframe rendering. When invoked on the server, it sends a packet to every client to render
     * the box.
     */
    @Override
    public void showRangeLines() {
        if (worldObj.isRemote) {
            ...
            NetworkHandler.sendToAllAround(
                ...
                worldObj,
                TileEntityConstants.PACKET_UPDATE_DISTANCE + getSecurityRange());
        }
    }
                ...
                player.addChatComponentMessage(
                    new ChatComponentTranslation(
                        EnumChatFormatting.GREEN
                            + "This Security Station is out of order: Its network hasn't been properly configured."));
            } else {
    ...
    private boolean gameProfileEquals(GameProfile profile1, GameProfile profile2) {
        return profile1.getId() != null && profile2.getId() != null ? profile1.getId()
            .equals(profile2.getId())
            : profile1.getName()
                .equals(profile2.getName());
    }
    ...
    public void readFromPacket(NBTTagCompound tag) {
        ...
        for (int i = 0; i < sharedList.tagCount(); ++i) {
            ...
            sharedUsers.add(
                new GameProfile(
                    tagCompound.hasKey("uuid") ? UUID.fromString(tagCompound.getString("uuid")) : null,
    ...
    public boolean isPlayerOnWhiteList(EntityPlayer player) {
        for (int i = 0; i < sharedUsers.size(); i++) {
            ...
            if (gameProfileEquals(user, player.getGameProfile())) {
                if (user.getId() == null && player.getGameProfile()
                    .getId() != null) {
                    sharedUsers.set(i, player.getGameProfile());
                    Log.info(
                        "Legacy conversion: Security Station shared username '" + player.getCommandSenderName()
                            + "' is now using UUID '"
        ...
        for (int i = 0; i < hackedUsers.size(); i++) {
            ...
            if (gameProfileEquals(user, player.getGameProfile())) {
                if (user.getId() == null && player.getGameProfile()
                    ...
                    Log.info(
                        "Legacy conversion: Security Station hacked username '" + player.getCommandSenderName()
                            + "' is now using UUID '"
                            + player.getGameProfile()
                                .getId()
                            + "'.");
    ...
    /**
     * Returns true if the given slots are connected in the network. For this to be true both slots need to have a
     * network component stored as well.
     * 
     * @param firstSlot
     * @param secondSlot
     * @return
    ...
    public boolean connects(int firstSlot, int secondSlot) {
        if (firstSlot < 0 || secondSlot < 0
            ...
            || getStackInSlot(firstSlot) == null
            || getStackInSlot(secondSlot) == null) return false;
        for (int column = -1; column <= 1; column++) {
            for (int row = -1; row <= 1; row++) {
                if (firstSlot + row * 5 + column == secondSlot) {
                    if (firstSlot % 5 > 0 && firstSlot % 5 < 4 || secondSlot % 5 > 0 && secondSlot % 5 < 4
                        || secondSlot % 5 == firstSlot % 5) return true;
                }
            }
        }
        return false;
    ...
    public EnumNetworkValidityProblem checkForNetworkValidity() {
            ...
            if (getStackInSlot(i) != null) {
                switch (getStackInSlot(i).getItemDamage()) {
                    case ItemNetworkComponents.DIAGNOSTIC_SUBROUTINE:
                        if (subroutineSlot != -1) return EnumNetworkValidityProblem.TOO_MANY_SUBROUTINES;// only one
                                                                                                         // subroutine
                                                                                                         // per network
                        subroutineSlot = i;
                        break;
                    case ItemNetworkComponents.NETWORK_IO_PORT:
                        if (ioPortSlot != -1) return EnumNetworkValidityProblem.TOO_MANY_IO_PORTS;// only one subroutine
                                                                                                  // per network
                        ioPortSlot = i;
                        break;
                    case ItemNetworkComponents.NETWORK_REGISTRY:
                        if (registrySlot != -1) return EnumNetworkValidityProblem.TOO_MANY_REGISTRIES;// only one
                                                                                                      // subroutine per
                                                                                                      // network
                        registrySlot = i;
                        break;
        ...
        if (!traceComponent(subroutineSlot, ioPortSlot, new boolean[35]))
            return EnumNetworkValidityProblem.NO_CONNECTION_SUB_AND_IO_PORT;// check if there's a valid route between
                                                                            // the subroutine/ioPort
        if (!traceComponent(ioPortSlot, registrySlot, new boolean[35]))
