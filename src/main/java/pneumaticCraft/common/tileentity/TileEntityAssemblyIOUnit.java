...
public class TileEntityAssemblyIOUnit extends TileEntityAssemblyRobot {
    ...
    public void updateEntity() {
        ...
        if (worldObj.isRemote) {
            ...
            switch (state) {
                ...
                case STATE_MAX: // this will be set if we encounter an unknown state; prevents log-spam that would
                                // result from default-case
                    break;
                default:
    ...
    private boolean getItemFromCurrentDirection() {
        ...
        if (isImportUnit()) {
            if (searchedItemStack == null) { // we don't know what we're supposed to pick up
                    ...
                    if (inv.getStackInSlot(i) != null) {
                        if (inventory[0] == null) {
                            if (inv.getStackInSlot(i)
                                .isItemEqual(searchedItemStack)) {
                                inventory[0] = inv.decrStackSize(i, 1);
                            }
                        } else if (inv.getStackInSlot(i)
                            .isItemEqual(inventory[0])) {
                                inv.decrStackSize(i, 1);
                                inventory[0].stackSize++;
                            }
                        extracted = (inventory[0] == null ? 0 : inventory[0].stackSize) == searchedItemStack.stackSize; // we
                                                                                                                        // might
                                                                                                                        // need
                                                                                                                        // to
                                                                                                                        // pickup
                ...
                if (oldStackSize == (inventory[0] == null ? 0 : inventory[0].stackSize)) // nothing picked up, search
            ...
            if (tile instanceof TileEntityAssemblyPlatform) {
                ...
                if (plat.openClaw()) {
                    ...
                    if (!extracted) // something went wrong - either the platform is gone altogether, or the item is not
                                    // there anymore
                        state = STATE_SEARCH_SRC;
                }
            }
    ...
    private boolean putItemToCurrentDirection() {
        if (isImportUnit()) {
            ...
            if (inv == null) repeatDropOffSearch(); // inventory gone; close claw and search new drop-off-location
                ...
                for (int i = 0; i < 6; i++) {
                    inventory[0] = PneumaticCraftUtils
                        .exportStackToInventory(inv, inventory[0], ForgeDirection.getOrientation(i));
                    if (inventory[0] == null) break;
                }
                if (inventory[0] == null || startSize != inventory[0].stackSize) sendDescriptionPacket(); // TODO - is
                                                                                                          // this still
                                                                                                          // needed?
                                                                                                          // Shouldn't
                                                                                                          // @DescSynced
                ...
                if (inventory[0] != null && startSize == inventory[0].stackSize) repeatDropOffSearch(); // target-inventory
                                                                                                        // full or
                                                                                                        // unavailable
            }
    ...
    private void repeatDropOffSearch() {
        state = state >= STATE_RESET_CLOSECLAW_AFTER_PICKUP ? STATE_RESET_CLOSECLAW_AFTER_PICKUP
            : STATE_CLOSECLAW_AFTER_PICKUP;
    }
    ...
    private boolean isDoneInternal() {
        ...
        /*
         * if(super.isDone()) {
         * boolean searchDone = feedPlatformStep != 4 || searchedItemStack != null && inventory[0] != null &&
         * searchedItemStack.isItemEqual(inventory[0]) && inventory[0].stackSize == searchedItemStack.stackSize;
            ...
            for (ForgeDirection dir : ForgeDirection.VALID_DIRECTIONS) {
                if (dir != ForgeDirection.UP && dir != ForgeDirection.DOWN) {
                    ...
                    if (te instanceof IInventory) {
                        if (IOHelper.extract(te, ForgeDirection.UP, searchedItem, true, true) != null)
                ...
                for (ForgeDirection secDir : new ForgeDirection[] { ForgeDirection.WEST, ForgeDirection.EAST }) {
                    for (ForgeDirection primDir : new ForgeDirection[] { ForgeDirection.NORTH, ForgeDirection.SOUTH }) {
                        TileEntity te = worldObj.getTileEntity(
                            xCoord + primDir.offsetX + secDir.offsetX,
                            yCoord,
                            zCoord + primDir.offsetZ + secDir.offsetZ);
                        if (te instanceof IInventory) {
                            if (IOHelper.extract(te, ForgeDirection.UP, searchedItem, true, true) != null)
    ...
    /**
     * Returns the inventory the IOUnit can point to for the given item. This should only be invoked when you're sure
     * that there is
    ...
    public IInventory getInventory(ItemStack searchedItem) {
        ...
        return (IInventory) worldObj.getTileEntity(
            xCoord + inventoryDir[0].offsetX + inventoryDir[1].offsetX,
            yCoord,
            zCoord + inventoryDir[0].offsetZ + inventoryDir[1].offsetZ);
    }
        ...
        if (inventory instanceof ISidedInventory) {
            ...
            for (int slot : slotsInTop) {
                if (inventory.isItemValidForSlot(slot, exportedItem)) {
                    ...
                    if (stack == null || stack.isItemEqual(exportedItem) && stack.getMaxStackSize() > stack.stackSize)
                        return slot;
                }
            }
        } else {
    ...
    /**
     * Returns the inventory the IOUnit can point to export the given item. This should only be invoked when you're sure
     * that there is
     * an accessible inventory with the item surrounding this TE.
     * 
     * @param exportedItem
