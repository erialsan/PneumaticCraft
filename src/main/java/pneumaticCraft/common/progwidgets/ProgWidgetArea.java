...
public class ProgWidgetArea extends ProgWidget implements IAreaProvider, IVariableWidget {
    ...
    public void addErrors(List<String> curInfo, List<IProgWidget> widgets) {
        ...
        if (coord1Variable.equals("") && coord2Variable.equals("")
            && x1 == 0
            && y1 == 0
            && z1 == 0
            && x2 == 0
    ...
    public void getArea(Set<ChunkPosition> area) {
        ...
        if (size > 100000) { // Prevent memory problems when getting to ridiculous areas.
            if (aiManager != null) aiManager.getDrone()
                .overload();
            return;
        }
        ...
        switch (type) {
            ...
            case WALL:
                ...
                break;
            case SPHERE:
                double radius = areaPoints[1] != null ? PneumaticCraftUtils.distBetween(areaPoints[0], areaPoints[1])
                    : 0;
            ...
            case LINE:
                if (areaPoints[1] != null) {
                    Vec3 lineVec = Vec3
                        .createVectorHelper(
                            areaPoints[1].chunkPosX - areaPoints[0].chunkPosX,
                            areaPoints[1].chunkPosY - areaPoints[0].chunkPosY,
                            areaPoints[1].chunkPosZ - areaPoints[0].chunkPosZ)
                        .normalize();
                    lineVec.xCoord /= 10;
                    lineVec.yCoord /= 10;
                    lineVec.zCoord /= 10;
            ...
            case X_WALL:
                if (areaPoints[1] != null) {
                    ...
                    double maxDistance = Math.sqrt(
                        Math.pow(areaPoints[0].chunkPosY - areaPoints[1].chunkPosY, 2)
                            + Math.pow(areaPoints[0].chunkPosZ - areaPoints[1].chunkPosZ, 2));
                    while (totalDistance <= maxDistance) {
                        totalDistance += 0.1;
                        curY += lineVec.yCoord;
                        curZ += lineVec.zCoord;
            ...
            case Y_WALL:
                if (areaPoints[1] != null) {
                    ...
                    double totalDistance = 0;
                    double maxDistance = Math.sqrt(
                        Math.pow(areaPoints[0].chunkPosX - areaPoints[1].chunkPosX, 2)
                            + Math.pow(areaPoints[0].chunkPosZ - areaPoints[1].chunkPosZ, 2));
                    while (totalDistance <= maxDistance) {
                        totalDistance += 0.1;
                        curX += lineVec.xCoord;
            ...
            case X_CYLINDER:
                if (areaPoints[1] != null) {
                    double rad = areaPoints[1] != null ? PneumaticCraftUtils.distBetween(
                        areaPoints[0].chunkPosY,
                        areaPoints[0].chunkPosZ,
                        areaPoints[1].chunkPosY,
                        areaPoints[1].chunkPosZ) : 0;
                    ...
                    for (int y = Math.max(0, minY); y <= maxY && y < 256; y++) {
                        for (int z = minZ; z <= maxZ; z++) {
                            if (PneumaticCraftUtils.distBetween(areaPoints[0].chunkPosY, areaPoints[0].chunkPosZ, y, z)
                                <= rad) {
                                for (int x = minX; x <= maxX; x++) {
                                    area.add(new ChunkPosition(x, y, z));
                                }
            ...
            case Y_CYLINDER:
                if (areaPoints[1] != null) {
                    double rad = areaPoints[1] != null ? PneumaticCraftUtils.distBetween(
                        areaPoints[0].chunkPosX,
                        areaPoints[0].chunkPosZ,
                        areaPoints[1].chunkPosX,
                        areaPoints[1].chunkPosZ) : 0;
                    ...
                    for (int x = minX; x <= maxX; x++) {
                        for (int z = minZ; z <= maxZ; z++) {
                            if (PneumaticCraftUtils.distBetween(areaPoints[0].chunkPosX, areaPoints[0].chunkPosZ, x, z)
                                <= rad) {
                                for (int y = Math.max(0, minY); y <= maxY && y < 256; y++) {
                                    area.add(new ChunkPosition(x, y, z));
                                }
            ...
            case Z_CYLINDER:
                if (areaPoints[1] != null) {
                    ...
                    for (int x = minX; x <= maxX; x++) {
                        for (int y = Math.max(0, minY); y <= maxY && y < 256; y++) {
                            if (PneumaticCraftUtils.distBetween(areaPoints[0].chunkPosX, areaPoints[0].chunkPosY, x, y)
                                <= rad) {
                                for (int z = minZ; z <= maxZ; z++) {
                                    area.add(new ChunkPosition(x, y, z));
                                }
            ...
            case GRID:
                if (areaPoints[1] == null || areaPoints[0].equals(areaPoints[1]) || typeInfo <= 0) {
                    ...
                    for (int x = areaPoints[0].chunkPosX; areaPoints[0].chunkPosX < areaPoints[1].chunkPosX
                        ? x <= areaPoints[1].chunkPosX
                        : x >= areaPoints[1].chunkPosX; x += (areaPoints[0].chunkPosX < areaPoints[1].chunkPosX ? 1
                            : -1) * interval) {
                        for (int y = areaPoints[0].chunkPosY; areaPoints[0].chunkPosY < areaPoints[1].chunkPosY
                            ? y <= areaPoints[1].chunkPosY
                            : y >= areaPoints[1].chunkPosY; y += (areaPoints[0].chunkPosY < areaPoints[1].chunkPosY ? 1
                                : -1) * interval) {
                            for (int z = areaPoints[0].chunkPosZ; areaPoints[0].chunkPosZ < areaPoints[1].chunkPosZ
                                ? z <= areaPoints[1].chunkPosZ
                                : z >= areaPoints[1].chunkPosZ; z += (areaPoints[0].chunkPosZ < areaPoints[1].chunkPosZ
                                    ? 1
                                    : -1) * interval) {
                                if (y > 0 && y < 256) area.add(new ChunkPosition(x, y, z));
                            }
