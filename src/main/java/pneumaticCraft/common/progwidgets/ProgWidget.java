...
public abstract class ProgWidget implements IProgWidget {
    ...
    public ProgWidget() {
        if (getParameters() != null) connectedParameters = new IProgWidget[getParameters().length * 2]; // times two
                                                                                                        // because
                                                                                                        // black- and
                                                                                                        // whitelist.
    }
    ...
    public void renderExtraInfo() {
        if (getExtraStringInfo() != null) {
            ...
            String[] splittedInfo = WordUtils.wrap(getExtraStringInfo(), 40)
            ...
            for (int i = 0; i < splittedInfo.length; i++) {
                ...
                int startY = getHeight() / 2 - (fr.FONT_HEIGHT + 1) * (splittedInfo.length - 1) / 4
                    + (fr.FONT_HEIGHT + 1) * i / 2
                    - fr.FONT_HEIGHT / 4;
                Gui.drawRect(
                    startX * 2 - 1,
                    startY * 2 - 1,
                    startX * 2 + stringLength + 1,
                    startY * 2 + fr.FONT_HEIGHT + 1,
                    0xFFFFFFFF);
        ...
        if (this instanceof IVariableWidget) {
            ...
            for (String variable : variables) {
                if (!variable.equals("") && !variable.startsWith("#")
                    && !variable.startsWith("$")
                    && !isVariableSetAnywhere(widgets, variable)) {
                    curInfo.add(
                        StatCollector
                            .translateToLocalFormatted("gui.progWidget.general.warning.variableNeverSet", variable));
                }
            }
        }
    }
    ...
    public IProgWidget copy() {
        try {
            IProgWidget copy = this.getClass()
                .newInstance();
