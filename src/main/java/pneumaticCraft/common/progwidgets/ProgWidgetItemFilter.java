...
public class ProgWidgetItemFilter extends ProgWidget implements IVariableWidget {
    ...
    public static void drawItemStack(ItemStack p_146982_1_, int p_146982_2_, int p_146982_3_, String p_146982_4_) {
        ...
        itemRender
            .renderItemOverlayIntoGUI(font, mc.getTextureManager(), p_146982_1_, p_146982_2_, p_146982_3_, p_146982_4_);
        GL11.glPopMatrix();
    ...
    public void readFromNBT(NBTTagCompound tag) {
        ...
        filter = tag.getTag("id") instanceof NBTPrimitive ? ItemStack.loadItemStackFromNBT(tag)
            : loadItemStackByName(tag);
    ...
    public static boolean isItemValidForFilters(ItemStack item, List<ProgWidgetItemFilter> whitelist,
        List<ProgWidgetItemFilter> blacklist, int blockMeta) {
        if (blacklist != null) {
            for (ProgWidgetItemFilter black : blacklist) {
                if (PneumaticCraftUtils.areStacksEqual(
                    black.getFilter(),
                    item,
                    black.useMetadata && blockMeta == -1,
                    black.useNBT,
                    black.useOreDict,
                    black.useModSimilarity)) {
        ...
        if (whitelist == null || whitelist.size() == 0) {
            ...
            for (ProgWidgetItemFilter white : whitelist) {
                if (PneumaticCraftUtils.areStacksEqual(
                    white.getFilter(),
                    item,
                    white.useMetadata && blockMeta == -1,
                    white.useNBT,
                    white.useOreDict,
                    white.useModSimilarity)) {
