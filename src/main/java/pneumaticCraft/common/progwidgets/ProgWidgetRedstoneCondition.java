    ...
    protected DroneAIBlockCondition getEvaluator(IDroneBase drone, IProgWidget widget) {
        return new DroneAIBlockCondition(drone, (ProgWidgetAreaItemBase) widget) {
            ...
            protected boolean evaluate(ChunkPosition pos) {
                int redstoneLevel = PneumaticCraftUtils
                    .getRedstoneLevel(drone.getWorld(), pos.chunkPosX, pos.chunkPosY, pos.chunkPosZ);
                int requiredRedstone = ((ICondition) widget).getRequiredCount();
                return ((ICondition) widget).getOperator() == ICondition.Operator.EQUALS
                    ? requiredRedstone == redstoneLevel
                    : redstoneLevel >= requiredRedstone;
            }
