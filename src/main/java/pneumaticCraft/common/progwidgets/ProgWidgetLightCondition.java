    ...
    protected DroneAIBlockCondition getEvaluator(IDroneBase drone, IProgWidget widget) {
        return new DroneAIBlockCondition(drone, (ProgWidgetAreaItemBase) widget) {
            ...
            protected boolean evaluate(ChunkPosition pos) {
                int lightLevel = drone.getWorld()
                    .getBlockLightValue(pos.chunkPosX, pos.chunkPosY, pos.chunkPosZ);
                int requiredLight = ((ICondition) widget).getRequiredCount();
                return ((ICondition) widget).getOperator() == ICondition.Operator.EQUALS ? requiredLight == lightLevel
                    : lightLevel >= requiredLight;
            }
