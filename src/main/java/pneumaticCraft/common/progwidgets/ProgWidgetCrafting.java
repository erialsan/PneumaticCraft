...
public class ProgWidgetCrafting extends ProgWidget implements ICraftingWidget, ICountWidget {
    ...
    public void addErrors(List<String> curInfo, List<IProgWidget> widgets) {
        ...
        for (int y = 0; y < 3; y++) {
            ...
            for (int x = 0; x < 3 && itemFilter != null; x++) {
                if (!itemFilter.getVariable()
                    .equals("")) usingVariables = true;
    ...
    private ItemStack getRecipeResult(World world) {
        return CraftingManager.getInstance()
            .findMatchingRecipe(getCraftingGrid(), world);
    }
    ...
    public static class DroneAICrafting extends EntityAIBase {
        ...
        public boolean shouldExecute() {
            ...
            InventoryCrafting craftingGrid = widget.getCraftingGrid();
            for (int crafted = 0; !((ICountWidget) widget).useCount()
                || crafted < ((ICountWidget) widget).getCount(); crafted++) {
                ...
                for (int i = 0; i < equivalentsList.length; i++) {
                    ...
                    if (originalStack != null) {
                        ...
                        for (int j = 0; j < drone.getInventory()
                            .getSizeInventory(); j++) {
                            ItemStack droneStack = drone.getInventory()
                                .getStackInSlot(j);
                            if (droneStack != null && (droneStack.getItem() == originalStack.getItem()
                                || PneumaticCraftUtils.isSameOreDictStack(droneStack, originalStack))) {
        ...
        public boolean craft(ItemStack craftedStack, InventoryCrafting craftMatrix) {
            ...
            FMLCommonHandler.instance()
                .firePlayerCraftingEvent(drone.getFakePlayer(), craftedStack, craftMatrix);
            ...
            for (int i = 0; i < craftMatrix.getSizeInventory(); ++i) {
                ...
                if (itemstack1 != null) {
                    if (itemstack1.getItem()
                        ...
                        if (itemstack2 != null && itemstack2.isItemStackDamageable()
                            ...
                            MinecraftForge.EVENT_BUS
                                .post(new PlayerDestroyItemEvent(drone.getFakePlayer(), itemstack2));
                            continue;
                        }
                    ...
                    itemstack1.stackSize--;// As this stack references to the Drones stacks in its inventory, we can do
                                           // this.
                }
            }
            ...
            for (int i = 0; i < drone.getInventory()
                .getSizeInventory(); i++) {
                ItemStack stack = drone.getInventory()
                    .getStackInSlot(i);
