...
public class UpdateChecker extends Thread {
    ...
    private int displayDelay = 200;// build in a delay, as many mods display it on right away, I'll wait a few seconds
                                   // until their spam is gone (A).
        ...
        if (event.phase == TickEvent.Phase.END && event.player.worldObj.isRemote) {
            ...
            if (doneChecking && displayDelay == 0) {
                if (hasNewVersion) {
                    sendMessage(
                        event.player,
                        "There's a new " + updateSize
                            + " version of PneumaticCraft available: version "
                            + latestVersion
                            + " (you are running "
    ...
    private void checkForUpdate() {
        ...
        try {
            ...
            while (curDigit >= 0) {
                try {
                    ...
                    if (totalText.contains("It looks like nothing was found at this location. Maybe try a search?")) {
                        ...
                        // String totalText = new System.Net.WebClient().DownloadString(Constants.URL +
                        // checkingVersion.Replace('.', '-') + "/");
                        String[] text = totalText.replace("<br />", "")
                            .replace("<p>", "")
                            .replace("</p>", "")
                            .replace("\t", "")
                            .replace("</a>", "")
            ...
            Log.error(
                "The URL of the current version changed, the current running version is weirdly formatted (dev version?), minemaarten.com is down or there is a problem with your internet connection.");
        }
    }
            ...
            if (astring.length > 0 && astring[0].equalsIgnoreCase("changelog")) {
                if (astring.length > 1) {
                    if (astring[1].equalsIgnoreCase("additions")) {
                        ...
                        sendMessage(
                            icommandsender,
                            "You'll now get a notification when a new version of PneumaticCraft is released. Checking now...");
                        Config.config.get(Configuration.CATEGORY_GENERAL, "Enable Update Checker", true)
                            .set(true);
                        Config.config.save();
                        // if(Config.enableUpdateChecker) FMLCommonHandler.instance().bus().unregister(instance());
                        // causes NPE for some reason...
                        ...
                        sendMessage(
                            icommandsender,
                            "You'll no longer get a notification when there's a new version of PneumaticCraft available.");
                        Config.config.get(Configuration.CATEGORY_GENERAL, "Enable Update Checker", true)
                            .set(false);
                        Config.config.save();
                        // if(Config.enableUpdateChecker)
                        // FMLCommonHandler.instance().bus().unregister(instance());//save cpu time by unsubscribing to
                        // the tickevent. //causes NPE for some reason.
