...
public class CoFHCore implements IThirdParty, IGuiHandler {
    ...
    @Override
    public void preInit() {
        pneumaticDynamo = new BlockPneumaticDynamo(Material.iron).setHardness(3.0F)
            .setResistance(10.0F)
            .setBlockName("pneumaticDynamo");
        fluxCompressor = new BlockFluxCompressor(Material.iron).setHardness(3.0F)
            .setResistance(10.0F)
            .setBlockName("fluxCompressor");
        ...
        GameRegistry.addRecipe(
            new ShapedOreRecipe(
                new ItemStack(Itemss.compressedIronGear),
                " i ",
                "isi",
                " i ",
                'i',
        ...
        PneumaticRegistry.getInstance()
            .registerBlockTrackEntry(new BlockTrackEntryRF());
        PneumaticRegistry.getInstance()
            .registerCustomBlockInteractor(new DroneInteractRFExport());
        PneumaticRegistry.getInstance()
            .registerCustomBlockInteractor(new DroneInteractRFImport());
    ...
    public void clientSide() {
        ClientProxy.registerBaseModelRenderer(
            fluxCompressor,
            TileEntityFluxCompressor.class,
            new ModelThirdPartyCompressor(ModelThirdPartyCompressor.Type.RF));
        ClientProxy
            .registerBaseModelRenderer(pneumaticDynamo, TileEntityPneumaticDynamo.class, new ModelPneumaticDynamo());
    }

    @Override
    public Object getServerGuiElement(int ID, EntityPlayer player, World world, int x, int y, int z) {
        if (ID == EnumGuiId.PNEUMATIC_DYNAMO.ordinal() || ID == EnumGuiId.FLUX_COMPRESSOR.ordinal())
    ...
    public Object getClientGuiElement(int ID, EntityPlayer player, World world, int x, int y, int z) {
        switch (EnumGuiId.values()[ID]) {
            case PNEUMATIC_DYNAMO:
                return new GuiPneumaticDynamo(
                    player.inventory,
                    (TileEntityPneumaticDynamo) world.getTileEntity(x, y, z));
    ...
    public static IEnergyStorage getEnergyStorage(IDrone entity) {
        ...
        if (property == null) {
            ...
            property.energy
                .setCapacity(100000 + 100000 * entity.getUpgrades(ItemMachineUpgrade.UPGRADE_DISPENSER_DAMAGE));
