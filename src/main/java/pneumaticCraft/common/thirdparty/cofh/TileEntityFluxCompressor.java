...
public class TileEntityFluxCompressor extends TileEntityPneumaticBase
    implements IEnergyHandler, IEnergyInfo, IInventory, IRedstoneControlled, IRFConverter, IHeatExchanger {
    ...
    public TileEntityFluxCompressor() {
        super(
            PneumaticValues.DANGER_PRESSURE_FLUX_COMPRESSOR,
            PneumaticValues.MAX_PRESSURE_FLUX_COMPRESSOR,
            PneumaticValues.VOLUME_FLUX_COMPRESSOR);
    ...
    public void updateEntity() {
        ...
        if (!worldObj.isRemote) {
            if (worldObj.getTotalWorldTime() % 5 == 0) {
                airPerTick = (int) (40 * this.getSpeedUsageMultiplierFromUpgrades()
                    * getEfficiency()
                    * Config.fluxCompressorEfficiency
                    / 100
                    / 100);
