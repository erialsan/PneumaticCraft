...
public class DroneInteractRFImport implements ICustomBlockInteract {
    ...
    @Override
    public boolean doInteract(ChunkPosition pos, IDrone drone, IBlockInteractHandler interactHandler,
        boolean simulate) {
            ...
            if (te instanceof IEnergyProvider) {
                ...
                for (ForgeDirection d : ForgeDirection.VALID_DIRECTIONS) {
                    if (interactHandler.getSides()[d.ordinal()]) {
                        int transferedEnergy = droneEnergy
                            .receiveEnergy(
                                provider.extractEnergy(
                                    d,
                                    interactHandler.useCount() ? interactHandler.getRemainingCount()
                                        : Integer.MAX_VALUE,
                                    true),
                                true);
