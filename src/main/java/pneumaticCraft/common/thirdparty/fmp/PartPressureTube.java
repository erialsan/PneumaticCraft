...
public class PartPressureTube extends TMultiPart implements IPneumaticPosProvider, IPneumaticWrenchable, TSlottedPart,
    ...
    public boolean activate(EntityPlayer player, MovingObjectPosition hit, ItemStack item) {
        boolean result = Blockss.pressureTube.onBlockActivated(
            player.worldObj,
            x(),
            y(),
            z(),
    ...
    public Iterable<Cuboid6> getOcclusionBoxes() {
        ...
        for (int i = 0; i < 6; i++) {
            if (tube.modules[i] != null) boxes.add(boundingBoxes[i]);// The full bounding box of modules is too big for
                                                                     // hollow covers.
        }
        return boxes;
    }
    ...
    public void renderDynamic(Vector3 pos, float partialTicks, int renderPass) {
        if (renderPass == 0) {
             ...
             * GL11.glPopMatrix();
             */
            TileEntityRendererDispatcher.instance.getSpecialRenderer(tube)
                .renderTileEntityAt(tube, pos.x, pos.y, pos.z, partialTicks);
