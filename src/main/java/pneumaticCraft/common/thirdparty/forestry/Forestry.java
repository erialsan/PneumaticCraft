...
public class Forestry implements IThirdParty {
    ...
    private static Class[] logics = new Class[] { FarmLogicSquid.class, FarmLogicFire.class, FarmLogicCreeper.class,
        FarmLogicSlime.class, FarmLogicRain.class, FarmLogicEnder.class, FarmLogicLightning.class, null,
        FarmLogicBurst.class, FarmLogicPotion.class, FarmLogicRepulsion.class, FarmLogicHelium.class,
        FarmLogicChopper.class, null, FarmLogicPropulsion.class, FarmLogicFlying.class };
    ...
    public void preInit() {
        ...
        try {
            Field field = ReflectionHelper
                .findField(Class.forName("forestry.farming.gadgets.StructureLogicFarm"), "bricks");
        ...
        try {
            Constructor c = Class.forName("forestry.farming.circuits.CircuitFarmLogic")
            ...
            for (ItemStack stack : ((ItemPlasticElectronTube) plasticElectronTube).getSubItems()) {
                ...
                if (logics[meta] != null) {
                    ChipsetManager.solderManager.addRecipe(
                        layoutManaged,
                        new ItemStack(plasticElectronTube, 1, meta),
                        (ICircuit) c.newInstance("plasticPlant" + meta, logics[meta]));
                }
            }
        } catch (Throwable e) {
    ...
    public void init() {
        PneumaticRegistry.getInstance()
            .registerFuel(FluidRegistry.getFluid("biomass"), 500000);
        PneumaticRegistry.getInstance()
            .registerFuel(FluidRegistry.getFluid("bioethanol"), 500000);

        for (ItemStack stack : ((ItemPlasticElectronTube) plasticElectronTube).getSubItems()) {
            RecipeManagers.fabricatorManager.addRecipe(
                null,
                FluidRegistry.getFluidStack("glass", 500),
                stack.copy(),
                new Object[] { " X ", "#X#", "XXX", '#', Items.redstone, 'X',
                    new ItemStack(Itemss.plastic, 1, stack.getItemDamage()) });
        }
