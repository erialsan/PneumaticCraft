...
public class FarmLogicHelium implements IFarmLogic {
    ...
    public boolean isAcceptedResource(ItemStack itemstack) {
        return itemstack.getItem() instanceof ItemBlock
            && Blocks.netherrack == ((ItemBlock) itemstack.getItem()).field_150939_a;
    }

    @Override
    public boolean isAcceptedGermling(ItemStack itemstack) {
        return itemstack.getItem() == Itemss.plasticPlant
            && itemstack.getItemDamage() == ItemPlasticPlants.HELIUM_PLANT_DAMAGE;
    }
    ...
    private boolean tryPlaceSoil(int x, int y, int z) {
        if (housing.getWorld()
            ...
            if (housing.hasResources(new ItemStack[] { new ItemStack(Blocks.netherrack) })) {
                ...
                housing.getWorld()
                    .setBlock(x, y, z, Blocks.netherrack);
                return true;
            }
        }
        return false;
    ...
    public Collection<ICrop> harvest(int x, int y, int z, ForgeDirection d, int extent) {
        ...
        for (int i = 0; i < extent; i++) {
            ICrop crop = farmable
                .getCropAt(housing.getWorld(), x + d.offsetX * i, y + d.offsetY * i - 3, z + d.offsetZ * i);
