...
public class NEIAssemblyControllerRecipeManager extends PneumaticCraftPlugins {
    ...
    public void loadCraftingRecipes(String outputId, Object... results) {
        if (outputId.equals(getRecipesID())) {
            for (int i = 0; i < ItemAssemblyProgram.PROGRAMS_AMOUNT; i++) {
                ...
                for (int j = 0; j < program.getRecipeList()
                    .size(); j++) arecipes.add(getShape(i, j));
            }
    ...
    protected MultipleInputOutputRecipe getShape(int programMetadata, int recipeIndex) {
        ...
        AssemblyRecipe recipe = program.getRecipeList()
            .get(recipeIndex);
        MultipleInputOutputRecipe shape = new MultipleInputOutputRecipe();
        ItemStack[] inputStacks = new ItemStack[] { recipe.getInput() };// for now not useful to put it in an array, but
                                                                        // supports when adding multiple input/output.
    ...
    public void loadCraftingRecipes(ItemStack result) {
        for (int i = 0; i < ItemAssemblyProgram.PROGRAMS_AMOUNT; i++) {
            ...
            for (int j = 0; j < program.getRecipeList()
                .size(); j++) {
                if (NEIClientUtils.areStacksSameTypeCrafting(
                    program.getRecipeList()
                        .get(j)
                        .getOutput(),
                    result)) {
    ...
    public void loadUsageRecipes(ItemStack ingredient) {
        for (int i = 0; i < ItemAssemblyProgram.PROGRAMS_AMOUNT; i++) {
            AssemblyProgram program = ItemAssemblyProgram.getProgramFromItem(i);
            boolean[] addedRecipe = new boolean[program.getRecipeList()
                .size()];
            for (int j = 0; j < program.getRecipeList()
                .size(); j++) {
                if (NEIClientUtils.areStacksSameTypeCrafting(
            ...
            if (ingredient.getItem() == Itemss.assemblyProgram && ingredient.getItemDamage() == i) {
                for (int j = 0; j < program.getRecipeList()
                    .size(); j++) if (!addedRecipe[j]) arecipes.add(getShape(i, j));
            } else {
