...
public class MFR implements IThirdParty {
    ...
    public void preInit() {
        try {
            for (Block block : ItemPlasticPlants.getBlockToSeedMap()
                .keySet()) {
    ...
    public void init() {
        PneumaticRegistry.getInstance()
            .registerXPLiquid(FluidRegistry.getFluid("mobessence"), 77);
    }
    ...
    private void register(String methodName, Class parameterType, Object parameter) throws Throwable {
        ...
        registryClass.getMethod(methodName, parameterType)
