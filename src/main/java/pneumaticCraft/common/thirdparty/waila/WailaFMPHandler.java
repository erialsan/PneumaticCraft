...
public class WailaFMPHandler implements IWailaFMPProvider {
    ...
    public List<String> getWailaBody(ItemStack itemStack, List<String> currenttip, IWailaFMPAccessor accessor,
        ...
        TileEntityPressureTube tube = ModInteractionUtils.getInstance()
            .getTube(accessor.getTileEntity());
        if (tube != null) {
            NBTTagCompound tubeTag = accessor.getNBTData()
                .getCompoundTag("tube");
            tube.currentAir = tubeTag.getInteger("currentAir");
            WailaPneumaticHandler.addTipToMachine(currenttip, tube);
            TubeModule module = BlockPressureTube.getLookedModule(
                accessor.getWorld(),
                accessor.getTileEntity().xCoord,
                accessor.getTileEntity().yCoord,
                accessor.getTileEntity().zCoord,
                accessor.getPlayer());
            if (module != null) {
