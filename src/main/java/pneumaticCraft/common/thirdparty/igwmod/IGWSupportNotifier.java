...
/**
 * This class is meant to be copied to your own mod which implements IGW-Mod. When properly implemented by instantiating
 * a new instance somewhere in your mod
 * loading stage, this will notify the player when it doesn't have IGW in the instance. It also needs to have the config
 * option enabled to
...
public class IGWSupportNotifier {
    ...
    public IGWSupportNotifier() {
        if (FMLCommonHandler.instance()
            .getSide() == Side.CLIENT && !Loader.isModLoaded("IGWMod")) {
            ...
            if (config.get(
                ...
                List<ModContainer> loadedMods = Loader.instance()
                    .getActiveModList();
    ...
    public void onPlayerJoin(TickEvent.PlayerTickEvent event) {
        if (event.player.worldObj.isRemote && event.player == FMLClientHandler.instance()
        ...
        public void run() {
            ...
            try {
                ...
                String version = obj.get("version")
                    .getAsString();
                String mcVersion = obj.get("mcversion")
                    .getAsString();
                File renamedFile = new File(
                    String.format(
                        "." + File.separator + "mods" + File.separator + "IGW-Mod-%s-%s-universal.jar",
                        mcVersion,
                        version));
                    ...
                    if (Minecraft.getMinecraft().thePlayer != null) Minecraft.getMinecraft().thePlayer.addChatMessage(
                        new ChatComponentText(
                            EnumChatFormatting.RED
                                + "The version of Minecraft you are running doesn't seem to match the version of IGW-Mod that has been downloaded. The mod may not work."));
                }
