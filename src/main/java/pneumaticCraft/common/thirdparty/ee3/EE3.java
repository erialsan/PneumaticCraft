...
public class EE3 implements IThirdParty {
    ...
    @Override
    public void preInit() {
        for (ItemStack seed : ItemPlasticPlants.getBlockToSeedMap()
            .values()) {
    ...
    public void postInit() {
        for (PressureChamberRecipe recipe : PressureChamberRecipe.chamberRecipes) {
            if (recipe.output.length == 1) {
                ...
                Log.info(
                    "Found a Pressure Chamber recipe that has more than one output. This will cause problems with DynEMC!");
            }
        }
