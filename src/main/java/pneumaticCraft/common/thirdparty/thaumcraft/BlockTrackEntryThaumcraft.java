    ...
    public void addInformation(World world, int x, int y, int z, TileEntity te, List<String> infoList) {
        if (te instanceof IAspectContainer) {
            ...
            if (container instanceof INode) {
                ...
                infoList.add(
                    I18n.format("blockTracker.info.thaumcraft.nodetype") + " "
                        + I18n.format("nodetype." + node.getNodeType() + ".name"));
                if (node.getNodeModifier() != null) infoList.add(
                    I18n.format("blockTracker.info.thaumcraft.nodeModifier") + " "
                        + I18n.format("nodemod." + node.getNodeModifier() + ".name"));
            }
        }
    }
