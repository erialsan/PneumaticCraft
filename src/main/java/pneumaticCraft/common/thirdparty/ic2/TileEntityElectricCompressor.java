...
public class TileEntityElectricCompressor extends TileEntityPneumaticBase
    implements IEnergySink, IWrenchable, IInventory, IRedstoneControlled, IHeatExchanger {
    ...
    public int outputTimer;// set to 20 when receiving energy, and decreased to 0 when not. Acts as a buffer before
                           // sending packets to update the client's rotation logic.
    ...
    public TileEntityElectricCompressor() {
        super(
            PneumaticValues.DANGER_PRESSURE_ELECTRIC_COMPRESSOR,
            PneumaticValues.MAX_PRESSURE_ELECTRIC_COMPRESSOR,
            PneumaticValues.VOLUME_ELECTRIC_COMPRESSOR);
    ...
    public void updateEntity() {
        ...
        if (!worldObj.isRemote) {
            /*
             * if(getPressure(ForgeDirection.UNKNOWN) > PneumaticValues.MIN_PRESSURE_ELECTRIC_COMPRESSOR &&
             * redstoneAllows()) {
    ...
    public boolean isItemValidForSlot(int i, ItemStack itemstack) {
        return itemstack != null
            && (itemstack.getItem() == Itemss.machineUpgrade || PneumaticCraftUtils.isIC2Upgrade(itemstack.getItem()));
    }
    ...
    public double injectEnergy(ForgeDirection directionFrom, double amount, double voltage) {
         ...
         * if(!worldObj.isRemote) {
         * TODO put back when IC2 is sorted out worldObj.createExplosion(null, xCoord + 0.5, yCoord + 0.5, zCoord + 0.5,
         * 0.5F, true);
