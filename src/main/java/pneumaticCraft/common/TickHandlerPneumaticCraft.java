...
public class TickHandlerPneumaticCraft {
    ...
    public void tickEnd(TickEvent.WorldTickEvent event) {
        if (event.phase == TickEvent.Phase.END) {
            ...
            DroneClaimManager.getInstance(world)
                .update();
            if (!event.world.isRemote && event.world.provider.dimensionId == 0
                && event.world.getWorldTime() % (24000 / AmadronOfferPeriodicConfig.timesPerDay) == 1) {
                AmadronOfferManager.getInstance()
                    .shufflePeriodicOffers();
            }
            if (!event.world.isRemote && event.world.getTotalWorldTime() % 100 == 0) {
                double tickTime = net.minecraft.util.MathHelper.average(MinecraftServer.getServer().tickTimeArray)
                    * 1.0E-6D;// In case world are going to get their own thread:
                              // MinecraftServer.getServer().worldTickTimes.get(event.world.provider.dimensionId)
                NetworkHandler.sendToDimension(new PacketServerTickTime(tickTime), event.world.provider.dimensionId);
                if (event.world.getTotalWorldTime() % 600 == 0) AmadronOfferManager.getInstance()
                    .tryRestockCustomOffers();
            }
        }
    }
    ...
    private void handleElectrostaticGeneration(World world, Entity entity) {
        ...
        for (TileEntityElectrostaticCompressor compressor : compressors) {
            compressor.addAir(
                PneumaticValues.PRODUCTION_ELECTROSTATIC_COMPRESSOR / compressors.size(),
    ...
    public static void getElectrostaticGrid(List<int[]> list, World world, int x, int y, int z) {
        for (ForgeDirection d : ForgeDirection.VALID_DIRECTIONS) {
            if ((world.getBlock(x + d.offsetX, y + d.offsetY, z + d.offsetZ) == net.minecraft.init.Blocks.iron_bars
                || world.getBlock(x + d.offsetX, y + d.offsetY, z + d.offsetZ) == Blockss.electrostaticCompressor)
                && !listContainsCoord(list, x + d.offsetX, y + d.offsetY, z + d.offsetZ)) {
