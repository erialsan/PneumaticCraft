...
public class WorldGeneratorPneumaticCraft implements IWorldGenerator {
    ...
    public void generateSurface(World world, Random rand, int chunkX, int chunkZ) {
        if (rand.nextDouble() < Config.oilGenerationChance / 100D) {
            ...
            new WorldGenLakes(
                FluidRegistry.getFluid(Fluids.oil.getName())
                    .getBlock()).generate(world, rand, chunkX + 8, y, chunkZ + 8);
        }

        for (int j = 0; j < 16; j++) {
            if (rand.nextDouble() < Config.configPlantGenerationChance[j] && j != ItemPlasticPlants.HELIUM_PLANT_DAMAGE
                && j != ItemPlasticPlants.FIRE_FLOWER_DAMAGE) {
                ...
                for (int i = 0; i < plantsInGroup; i++) {
                    ...
                    if (y > 0 && ((BlockPneumaticPlantBase) ItemPlasticPlants.getPlantBlockIDFromSeed(j))
                        .canPlantGrowOnThisBlock(world.getBlock(x, y - 1, z), world, x, y - 1, z)) {
            ...
            if (rand.nextInt(10) == 0) {// Each chunks has a 1/10 chance to spawn a group of Fire Flowers.
                ...
                for (int i = 0; i < plantsInGroup; i++) {
                    ...
                    for (int j = 0; j < 64; j++) {
                        ...
                        if (world.isAirBlock(x, y, z) && ((BlockPneumaticPlantBase) Blockss.fireFlower)
                            .canPlantGrowOnThisBlock(world.getBlock(x, y - 1, z), world, x, y - 1, z)) {
