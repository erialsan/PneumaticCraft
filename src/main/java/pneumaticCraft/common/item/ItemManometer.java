...
public class ItemManometer extends ItemPressurizable {
     ...
     */
    @Override
    public boolean onItemUseFirst(ItemStack iStack, EntityPlayer player, World world, int x, int y, int z, int side,
        ...
        if (((IPressurizable) iStack.getItem()).getPressure(iStack) > 0F) {
            ...
            if (te instanceof IManoMeasurable) {
                ...
                machine.getAirHandler()
                    .printManometerMessage(player, info);
            }
            for (String s : info) curInfo.add(new ChatComponentTranslation(s));
            if (te instanceof IHeatExchanger) {
                IHeatExchangerLogic exchanger = ((IHeatExchanger) te)
                    .getHeatExchangerLogic(ForgeDirection.getOrientation(side));
                if (exchanger != null) {
                    curInfo
                        .add(new ChatComponentTranslation("waila.temperature", (int) exchanger.getTemperature() - 273));
                } else {
                        ...
                        if (exchanger != null) {
                            curInfo.add(
                                new ChatComponentTranslation(
                                    "waila.temperature." + d.toString()
                                        .toLowerCase(),
                                    (int) exchanger.getTemperature() - 273));
                        }
                    }
                }
            }
            ...
            player.addChatComponentMessage(
                new ChatComponentTranslation(EnumChatFormatting.RED + "The Manometer doesn't have any charge!"));
            return false;
        }
        return false;
    }
        ...
        if (!player.worldObj.isRemote) {
            if (entity instanceof IManoMeasurable) {
                if (((IPressurizable) iStack.getItem()).getPressure(iStack) > 0F) {
                    ...
                    player.addChatComponentMessage(
                        new ChatComponentTranslation(
                            EnumChatFormatting.RED + "The Manometer doesn't have any charge!"));
                }
            }
        }
        return false;
