...
public class ItemRemote extends ItemPneumatic {
     ...
     */
    @Override
    public boolean onItemUseFirst(ItemStack remote, EntityPlayer player, World world, int x, int y, int z, int side,
            ...
            if (te instanceof TileEntitySecurityStation) {
                if (((TileEntitySecurityStation) te).doesAllowPlayer(player)) {
                    ...
                    player.addChatComponentMessage(
                        new ChatComponentTranslation("gui.remote.boundSecurityStation", x, y, z));
                    return true;
                } else {
    ...
    public void addInformation(ItemStack remote, EntityPlayer player, List curInfo, boolean moreInfo) {
        ...
        if (tag != null && tag.hasKey("securityX")) {
            ...
            for (String s : WordUtils
                .wrap(I18n.format("gui.remote.tooltip.boundToSecurityStation", dimensionId, x, y, z), 40)
                .split(System.getProperty("line.separator"))) {
                curInfo.add(s);
            }
        } else {
            for (String s : WordUtils.wrap(I18n.format("gui.remote.tooltip.rightClickToBind"), 40)
                .split(System.getProperty("line.separator"))) {
                curInfo.add(s);
            }
        }
        ...
        if (player.isSneaking()) {
            if (isAllowedToEdit(player, remote)) {
                player.openGui(
                    ...
                    EnumGuiId.REMOTE_EDITOR.ordinal(),
                    player.worldObj,
                    (int) player.posX,
                    (int) player.posY,
                    (int) player.posZ);
                NetworkHandler.sendTo(
                    new PacketNotifyVariablesRemote(
            ...
            if (tag.hasKey("securityX")) {
                ...
                if (world != null) {
                    ...
                    if (te instanceof TileEntitySecurityStation) {
                        ...
                        if (!canAccess) {
                            player.addChatComponentMessage(
                                new ChatComponentTranslation("gui.remote.noEditRights", x, y, z));
                        }
                        return canAccess;
                    }
                }
