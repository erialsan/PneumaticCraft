...
public class ItemGPSTool extends ItemPneumatic {
    ...
    @Override
    public boolean onItemUse(ItemStack IStack, EntityPlayer player, World world, int x, int y, int z, int par7,
        float par8, float par9, float par10) {
        setGPSLocation(IStack, x, y, z);
        if (!world.isRemote) player.addChatComponentMessage(
            new ChatComponentTranslation(
                EnumChatFormatting.GREEN + "[GPS Tool] Set Coordinates to " + x + ", " + y + ", " + z + "."));
    ...
    public ItemStack onItemRightClick(ItemStack stack, World world, EntityPlayer player) {
        if (world.isRemote) {
            ...
            FMLCommonHandler.instance()
                .showGuiScreen(new GuiGPSTool(pos != null ? pos : new ChunkPosition(0, 0, 0), getVariable(stack)));
        }
        return stack;
    }
    ...
    public void onUpdate(ItemStack stack, World world, Entity entity, int slot, boolean heldItem) {
        ...
        if (!var.equals("") && !world.isRemote) {
            ChunkPosition pos = GlobalVariableManager.getInstance()
                .getPos(var);
    ...
    public static ChunkPosition getGPSLocation(ItemStack gpsTool) {
        ...
        if (compound != null) {
            ...
            if (!var.equals("") && FMLCommonHandler.instance()
    ...
    public static void setGPSLocation(ItemStack gpsTool, int x, int y, int z) {
        ...
        String var = getVariable(gpsTool);
        if (!var.equals("")) GlobalVariableManager.getInstance()
            .set(var, x, y, z);
    }
    ...
    public static String getVariable(ItemStack gpsTool) {
        return gpsTool.hasTagCompound() ? gpsTool.getTagCompound()
            .getString("variable") : "";
    }
}
