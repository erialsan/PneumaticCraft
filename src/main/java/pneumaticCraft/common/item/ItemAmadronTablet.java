...
public class ItemAmadronTablet extends ItemPressurizable implements IAmadronInterface {
    ...
    @Override
    public boolean onItemUse(ItemStack tablet, EntityPlayer player, World world, int x, int y, int z, int par7,
        ...
        if (te instanceof IFluidHandler) {
            if (!world.isRemote) {
                ...
                player.addChatComponentMessage(
                    new ChatComponentTranslation(
                        "message.amadronTable.setLiquidProvidingLocation",
                        x,
                        y,
                        z,
            ...
            if (!world.isRemote) {
                ...
                player.addChatComponentMessage(
                    new ChatComponentTranslation(
                        "message.amadronTable.setItemProvidingLocation",
                        x,
                        y,
                        z,
                        world.provider.dimensionId,
    ...
    public void addInformation(ItemStack stack, EntityPlayer player, List infoList, boolean par4) {
        ...
        if (pos != null) {
            ...
            infoList.add(
                I18n.format(
                    "gui.tooltip.amadronTablet.itemLocation",
                    pos.chunkPosX,
                    pos.chunkPosY,
                    pos.chunkPosZ,
                    dim));
        ...
        if (pos != null) {
            ...
            infoList.add(
                I18n.format(
                    "gui.tooltip.amadronTablet.fluidLocation",
                    pos.chunkPosX,
                    pos.chunkPosY,
                    pos.chunkPosZ,
                    dim));
    ...
    public static int getItemProvidingDimension(ItemStack tablet) {
        return tablet.hasTagCompound() ? tablet.getTagCompound()
            .getInteger("itemDim") : 0;
    }
    ...
    public static Map<AmadronOffer, Integer> getShoppingCart(ItemStack tablet) {
        ...
        if (tablet.hasTagCompound() && tablet.getTagCompound()
            .hasKey("shoppingCart")) {
            NBTTagList list = tablet.getTagCompound()
                .getTagList("shoppingCart", 10);
            for (int i = 0; i < list.tagCount(); i++) {
                NBTTagCompound tag = list.getCompoundTagAt(i);
                offers.put(
                    tag.hasKey("inStock") ? AmadronOfferCustom.loadFromNBT(tag) : AmadronOffer.loadFromNBT(tag),
