...
public class ItemPlasticPlants extends ItemPneumatic {
    ...
    public static final String[] PLANT_NAMES = new String[] { "Squid Plant", "Fire Flower", "Creeper Plant",
        "Slime Plant", "Rain Plant", "Ender Plant", "Lightning Plant", "Adrenaline Plant", "Burst Plant",
        "Potion Plant", "Repulsion Plant", "Helium Plant", "Chopper Plant", "Music Plant", "Propulsion Plant",
        "Flying Flower" };
    public static final boolean[] NEEDS_GENERATION = new boolean[] { true, true, true, true, true, false, true, false,
    ...
    public boolean onItemUse(ItemStack stack, EntityPlayer player, World world, int x, int y, int z, int side,
        ...
        if (side != (plant.isPlantHanging() ? 0 : 1)) {
            ...
            && player.canPlayerEdit(x, y + (plant.isPlantHanging() ? -1 : 1), z, side, stack)) {
                if (plant.canBlockStay(world, x, y + (plant.isPlantHanging() ? -1 : 1), z)
                    && world.isAirBlock(x, y + (plant.isPlantHanging() ? -1 : 1), z)) {
    ...
    public static void onEntityConstruction(Entity entity) {
        if (entity instanceof EntityItem) {
            if (((EntityItem) entity).getExtendedProperties("PneumaticCraft_Active") == null)
    ...
    public void addInformation(ItemStack stack, EntityPlayer player, List infoList, boolean par4) {
        ...
        infoList.add(
            EnumChatFormatting.RED + "They only exist still to provide a smooth transition towards the Oil system!");
    ...
    public boolean onEntityItemUpdate(EntityItem entityItem) {
        ...
        if (motProps == null && (itemDamage % 16 == ItemPlasticPlants.PROPULSION_PLANT_DAMAGE
            || itemDamage % 16 == ItemPlasticPlants.REPULSION_PLANT_DAMAGE)) {
        ...
        if (entityItem.onGround
            || Math.abs(entityItem.motionY) < 0.13D && (itemDamage % 16 == ItemPlasticPlants.HELIUM_PLANT_DAMAGE
                || itemDamage % 16 == ItemPlasticPlants.SQUID_PLANT_DAMAGE)) {
            ...
            if (!entityItem.worldObj.isRemote) {
                ...
                canSustain = ((BlockPneumaticPlantBase) blockID)
                    .canBlockStay(entityItem.worldObj, landedBlockX, landedBlockY, landedBlockZ);
                if (itemDamage % 16 == ItemPlasticPlants.FIRE_FLOWER_DAMAGE && !canSustain
                    && !isInChamber(entityItem.worldObj.getBlock(landedBlockX, landedBlockY - 1, landedBlockZ))
                    && net.minecraft.init.Blocks.fire
                        .canPlaceBlockAt(entityItem.worldObj, landedBlockX, landedBlockY, landedBlockZ)
                    && entityItem.worldObj.isAirBlock(landedBlockX, landedBlockY, landedBlockZ)) {
                    entityItem.worldObj
                        .setBlock(landedBlockX, landedBlockY, landedBlockZ, net.minecraft.init.Blocks.fire);
                }

                if (canSustain && isDelayOver) {
                    if (entityItem.worldObj.isAirBlock(landedBlockX, landedBlockY, landedBlockZ)) {

                        entityItem.worldObj.setBlock(
                            landedBlockX,
                            landedBlockY,
                            landedBlockZ,
                            blockID,
                            itemDamage > 15 || !isActive(entityItem) ? 0 : 7,
                            3);
                        ...
                        for (int i = 0; i < 10; i++) {
                            spawnParticle(
                                entityItem.worldObj,
                                "explode",
                                entityItem.posX + rand.nextDouble() - 0.5D,
                                entityItem.posY + rand.nextDouble() - 0.5D,
                                entityItem.posZ + rand.nextDouble() - 0.5D,
                                0.0D,
                                0.0D,
                                0.0D);
                        }
        ...
        if (itemDamage % 16 == ItemPlasticPlants.SQUID_PLANT_DAMAGE && entityItem.worldObj
            .isMaterialInBB(entityItem.boundingBox.contract(0.003D, 0.003D, 0.003D), Material.water)) {
    ...
    private boolean isInChamber(Block block) {
        return block == Blockss.pressureChamberInterface || block == Blockss.pressureChamberValve
            || block == Blockss.pressureChamberWall;
    }

    private boolean handlePropulsionBehaviour(EntityItem entityItem, double oldMotionX, double oldMotionZ) {
        int itemDamage = entityItem.getEntityItem()
            .getItemDamage();
        if (itemDamage == ItemPlasticPlants.PROPULSION_PLANT_DAMAGE) {
            boolean flag = Math.sqrt(entityItem.motionX * entityItem.motionX + entityItem.motionZ * entityItem.motionZ)
                < 0.1D;
            if (Math.sqrt(entityItem.motionX * entityItem.motionX + entityItem.motionZ * entityItem.motionZ) < 0.3D
                && entityItem.ticksExisted < 200) {
    ...
    private boolean handleRepulsionBehaviour(EntityItem entityItem, double oldMotionX, double oldMotionY,
        double oldMotionZ) {
        int itemDamage = entityItem.getEntityItem()
            .getItemDamage();
    ...
    private void spawnParticle(World world, String particleName, double spawnX, double spawnY, double spawnZ,
