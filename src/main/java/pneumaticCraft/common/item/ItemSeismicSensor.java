...
public class ItemSeismicSensor extends ItemPneumatic {
    ...
    public boolean onItemUse(ItemStack stack, EntityPlayer player, World world, int x, int y, int z, int par7,
        ...
        if (!world.isRemote) {
            ...
            while (testingY > 0) {
                testingY--;
                if (world.getBlock(x, testingY, z) == FluidRegistry.getFluid(Fluids.oil.getName())
                    .getBlock()) {
                    ...
                    while (!pendingPositions.empty()) {
                        ...
                        for (ForgeDirection d : ForgeDirection.VALID_DIRECTIONS) {
                            ...
                            if (world.getBlock(newPos.chunkPosX, newPos.chunkPosY, newPos.chunkPosZ)
                                == Fluids.oil.getBlock()
                                && world.getBlockMetadata(newPos.chunkPosX, newPos.chunkPosY, newPos.chunkPosZ) == 0
                                && oilPositions.add(newPos)) {
                    ...
                    player.addChatComponentMessage(
                        new ChatComponentTranslation(
                            ...
                            EnumChatFormatting.GREEN.toString() + (y - testingY),
                            EnumChatFormatting.GREEN.toString() + oilPositions.size() / 10 * 10));
                    return true;
                }
            }
