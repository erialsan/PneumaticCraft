...
@Optional.InterfaceList({ @Interface(iface = "thaumcraft.api.IRepairable", modid = ModIds.THAUMCRAFT),
...
public class ItemPneumaticArmor extends ItemArmor
    implements IPressurizable, IChargingStationGUIHolderItem, IRepairable, IRevealer, IGoggles, IVisDiscountGear {

    private final String textureLocation;
    private final int volume;

    public ItemPneumaticArmor(String textureLocation, ItemArmor.ArmorMaterial par2EnumArmorMaterial, int par3, int par4,
        int volume, int maxAir) {
    ...
    public void addInformation(ItemStack iStack, EntityPlayer player, List textList, boolean par4) {
        ...
        textList.add(
            (pressure < 0.5F ? EnumChatFormatting.RED : EnumChatFormatting.DARK_GREEN) + "Pressure: "
                + Math.round(pressure * 10D) / 10D
                + " bar");
                    ...
                    if (((String) textList.get(i)).contains("Coordinate Tracker")) {
                        textList.set(
                            ...
                            textList.get(i) + " (tracking "
                                ...
                                + coordHandler.z
                                + " in "
                                + coordHandler.worldObj.provider.getDimensionName()
                                + ")");
                        break;
                    }
                }
    ...
    public static RenderCoordWireframe getCoordTrackLocation(ItemStack helmetStack) {
        ...
        if (tag.getInteger("y") == -1 || FMLClientHandler.instance()
            .getClient().theWorld.provider.dimensionId != tag.getInteger("dimID")) return null;
        return new RenderCoordWireframe(
    ...
    public float getPressure(ItemStack iStack) {
        int volume = ItemPneumaticArmor.getUpgrades(ItemMachineUpgrade.UPGRADE_VOLUME_DAMAGE, iStack)
            * PneumaticValues.VOLUME_VOLUME_UPGRADE + PneumaticValues.PNEUMATIC_HELMET_VOLUME;
    ...
    public void onArmorTick(World world, EntityPlayer player, ItemStack iStack) {
        ...
        if (!world.isRemote && NBTUtil.hasTag(iStack, "Inventory")
            && iStack.getTagCompound()
