...
public class ItemEmptyPCB extends ItemNonDespawning {
    ...
    public void addInformation(ItemStack stack, EntityPlayer player, List infoList, boolean par4) {
        ...
        if (stack.hasTagCompound()) {
            infoList.add(
                "Etching progress: " + stack.getTagCompound()
                    .getInteger("etchProgress") + "%");
    ...
    public boolean onEntityItemUpdate(EntityItem entityItem) {
        ...
        if (Fluids.areFluidsEqual(
            FluidRegistry.lookupFluidForBlock(
                entityItem.worldObj.getBlock(
            ...
            int etchProgress = stack.getTagCompound()
                .getInteger("etchProgress");
            if (etchProgress < 100) {
                if (entityItem.ticksExisted % (TileEntityConstants.PCB_ETCH_TIME / 5) == 0) stack.getTagCompound()
                    .setInteger("etchProgress", etchProgress + 1);
            } else {
                entityItem.setEntityItemStack(
                    new ItemStack(
                        rand.nextInt(100) >= stack.getItemDamage() ? Itemss.unassembledPCB : Itemss.failedPCB));
            }
        }
        return false;
    }
