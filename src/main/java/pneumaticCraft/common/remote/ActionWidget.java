...
public abstract class ActionWidget<Widget extends IGuiWidget> {
    ...
    public void readFromNBT(NBTTagCompound tag, int guiLeft, int guiTop) {
        ...
        enablingValue = tag.hasKey("enablingX")
            ? new ChunkPosition(tag.getInteger("enablingX"), tag.getInteger("enablingY"), tag.getInteger("enablingZ"))
    ...
    public ActionWidget copy() {
        try {
            ActionWidget widget = this.getClass()
                .newInstance();
    ...
    public boolean isEnabled() {
        return enableVariable.equals("") || GlobalVariableManager.getInstance()
            .getPos(enableVariable)
            .equals(enablingValue);
    }
