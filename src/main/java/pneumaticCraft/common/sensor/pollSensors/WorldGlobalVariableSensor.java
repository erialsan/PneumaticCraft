...
public class WorldGlobalVariableSensor implements IPollSensorSetting {
    ...
    public List<String> getDescription() {
        ...
        text.add(
            EnumChatFormatting.BLACK
                + "Emits a redstone signal when the global variable specified its X position is not equal to 0");
        return text;
    }
    ...
    public int getRedstoneValue(World world, int x, int y, int z, int sensorRange, String textBoxText) {
        return GlobalVariableManager.getInstance()
            .getBoolean(textBoxText) ? 15 : 0;
    }
