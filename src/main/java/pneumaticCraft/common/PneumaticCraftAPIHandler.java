...
public class PneumaticCraftAPIHandler implements IPneumaticCraftInterface {
    ...
    public void addHackable(Class<? extends Entity> entityClazz, Class<? extends IHackableEntity> iHackable) {
        ...
        if (Entity.class.isAssignableFrom(iHackable)) {
            ...
            try {
                ...
                Log.error(
                    "Not able to register hackable entity: " + iHackable.getName()
                        + ". Does the class have a parameterless constructor?");
                e.printStackTrace();
            } catch (IllegalAccessException e) {
                Log.error(
                    "Not able to register hackable entity: " + iHackable.getName() + ". Is the class a public class?");
                e.printStackTrace();
            }
        }
    }
    ...
    public void addHackable(Block block, Class<? extends IHackableBlock> iHackable) {
        ...
        if (Block.class.isAssignableFrom(iHackable)) {
            ...
            try {
                ...
                Log.error(
                    "Not able to register hackable block: " + iHackable.getName()
                        + ". Does the class have a parameterless constructor?");
                e.printStackTrace();
            } catch (IllegalAccessException e) {
                Log.error(
                    "Not able to register hackable block: " + iHackable.getName() + ". Is the class a public class?");
                e.printStackTrace();
            }
        }
    }
    ...
    public List<IHackableEntity> getCurrentEntityHacks(Entity entity) {
        HackingEntityProperties hackingProps = (HackingEntityProperties) entity
    ...
    public void registerBlockExchanger(Block block, IHeatExchanger heatExchanger) {
        HeatExchangerManager.getInstance()
            .registerBlockExchanger(block, heatExchanger);
    }
    ...
    public void registerFuel(Fluid fluid, int mLPerBucket) {
        ...
        if (liquidFuels.containsKey(fluid.getName())) {
            Log.info(
                "Overriding liquid fuel entry " + fluid.getLocalizedName(new FluidStack(fluid, 1))
                    + " ("
                    + fluid.getName()
                    + ") with a fuel value of "
                    + mLPerBucket
                    + " (previously "
                    + liquidFuels.get(fluid.getName())
                    + ")");
    ...
    public void registerHeatBehaviour(Class<? extends HeatBehaviour> heatBehaviour) {
        HeatBehaviourManager.getInstance()
            .registerBehaviour(heatBehaviour);
    }
}
