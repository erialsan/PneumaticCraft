...
public class FluidPlastic extends FluidPneumaticCraft {
    ...
    public static int getTemperatureS(FluidStack plastic) {
        return PneumaticValues.PLASTIC_MIXER_MELTING_TEMP + 1;// plastic.tag != null ?
                                                              // plastic.tag.getInteger("temperature") : 295;
    }
    ...
    public static void addDye(FluidStack plastic, int dyeMetadata) {
        ...
        double ratio = PneumaticValues.PLASTIC_MIX_RATIO
            / (PneumaticValues.PLASTIC_MIX_RATIO * (plastic.amount / 1000D));
        for (int i = 0; i < 3; i++) {
    ...
    public static FluidStack mixFluid(FluidStack plastic, FluidStack otherPlastic) {
        ...
        // newTag.setInteger("temperature", (int)(ratio * getTemperatureS(plastic) + (1 - ratio) *
        // getTemperatureS(otherPlastic)));
    ...
    public static int getPlasticMeta(FluidStack plastic) {
        ...
        for (ItemStack s : plasticTypes) {
            ...
            double gap = Math.pow(plasticColor[0] - (dyeColors[i] >> 16), 2)
                + Math.pow(plasticColor[1] - (dyeColors[i] >> 8 & 255), 2)
                + Math.pow(plasticColor[2] - (dyeColors[i] & 255), 2);
            if (gap < closestGap) {
                closestGap = gap;
                bestMatching = i;
            }
