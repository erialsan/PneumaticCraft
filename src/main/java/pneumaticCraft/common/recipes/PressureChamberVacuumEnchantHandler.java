...
public class PressureChamberVacuumEnchantHandler implements IPressureChamberRecipe {
    ...
    public ItemStack[] isValidRecipe(ItemStack[] inputStacks) {
        ...
        for (ItemStack stack : inputStacks) {
            ...
            enchantmentCount = EnchantmentHelper.getEnchantments(stack)
                .size();
    ...
    /**
     * This method will be called when the recipe should output its items. te stacks the recipe outputs, maybe dependent
     * on the input stacks.
     * 
    ...
    public ItemStack[] craftRecipe(ItemStack[] inputStacks, ItemStack[] removedStacks) {
        ...
        if (enchantedStack == null) {
            System.err.println(
                "[Pressure Chamber Vacuum Enchantment Handler] No enchanted stack found! Report to MineMaarten!");
            return null;
        }

        // take a random enchantment of the enchanted stack.
        int enchIndex = new Random().nextInt(
            ((NBTTagList) enchantedStack.getTagCompound()
                .getTag("ench")).tagCount());
        NBTTagCompound enchTag = ((NBTTagList) enchantedStack.getTagCompound()
            .getTag("ench")).getCompoundTagAt(enchIndex);

        ((NBTTagList) enchantedStack.getTagCompound()
            .getTag("ench")).removeTag(enchIndex);
        if (((NBTTagList) enchantedStack.getTagCompound()
            .getTag("ench")).tagCount() == 0) {
            enchantedStack.getTagCompound()
                .removeTag("ench");
            if (enchantedStack.getTagCompound()
                .hasNoTags()) {
