...
public class AmadronOffer {
    ...
    public AmadronOffer(Object input, Object output) {
        ...
        if (input instanceof ItemStack) {
            if (((ItemStack) input).stackSize <= 0)
    ...
    public String getVendor() {
        return StatCollector.translateToLocal("gui.amadron");// Hardcoded for now until inter-player trading is
                                                             // implemented.
    }
    ...
    public boolean passesQuery(String query) {
        ...
        return getObjectName(getInput()).toLowerCase()
            .contains(queryLow)
            || getObjectName(getOutput()).toLowerCase()
                .contains(queryLow)
            || getVendor().toLowerCase()
                .contains(queryLow);
    }

    private String getObjectName(Object object) {
        return object instanceof ItemStack ? ((ItemStack) object).getDisplayName()
            : ((FluidStack) object).getLocalizedName();
    }
    ...
    public JsonObject toJson() {
        ...
        if (input instanceof ItemStack) {
            inputObject.addProperty(
                ...
                GameData.getItemRegistry()
                    .getNameForObject(((ItemStack) input).getItem()));
        ...
        if (output instanceof ItemStack) {
            outputObject.addProperty(
                "id",
                GameData.getItemRegistry()
                    .getNameForObject(((ItemStack) output).getItem()));
    ...
    public static AmadronOffer fromJson(JsonObject object) {
        ...
        if (inputObject.has("damage")) {
            ...
            if (item != null) {
                input = new ItemStack(
                    item,
                    inputObject.get("amount")
                        .getAsInt(),
                    inputObject.get("damage")
                        .getAsInt());
            } else {
                Log.error(
                    "Invalid Amadron Offer input item. Invalid item name: " + inputObject.get("id")
                        .getAsString() + ". Offer will be skipped");
                return null;
            }
        } else {
            Fluid fluid = FluidRegistry.getFluid(
                inputObject.get("id")
                    .getAsString());
            if (fluid != null) {
                input = new FluidStack(
                    fluid,
                    inputObject.get("amount")
                        .getAsInt());
            } else {
                Log.error(
                    "Invalid Amadron Offer input fluid. Invalid fluid name: " + inputObject.get("id")
                        .getAsString() + ". Offer will be skipped");
                return null;
            }
        }
        ...
        if (outputObject.has("damage")) {
            ...
            if (item != null) {
                output = new ItemStack(
                    item,
                    outputObject.get("amount")
                        .getAsInt(),
                    outputObject.get("damage")
                        .getAsInt());
            } else {
                Log.error(
                    "Invalid Amadron Offer output item. Invalid item name: " + outputObject.get("id")
                        .getAsString() + ". Offer will be skipped");
                return null;
            }
        } else {
            Fluid fluid = FluidRegistry.getFluid(
                outputObject.get("id")
                    .getAsString());
            if (fluid != null) {
                output = new FluidStack(
                    fluid,
                    outputObject.get("amount")
                        .getAsInt());
            } else {
                Log.error(
                    "Invalid Amadron Offer output fluid. Invalid fluid name: " + outputObject.get("id")
                        .getAsString() + ". Offer will be skipped");
                return null;
            }
        }
    ...
    public boolean equals(Object o) {
        if (o instanceof AmadronOffer) {
            ...
            if (offer.getInput()
                .getClass() == getInput().getClass()
                && offer.getOutput()
                    .getClass() == getOutput().getClass()) {
