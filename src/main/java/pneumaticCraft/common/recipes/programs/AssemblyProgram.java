...
public abstract class AssemblyProgram {
    ...
    /**
     * Retrieves the needed machines for this Assembly Program. As a Controller is always needed this doesn't have to be
     * returned.
     * 
     * @return
     */
     ...
     */
    public abstract boolean executeStep(TileEntityAssemblyController controller, TileEntityAssemblyPlatform platform,
        TileEntityAssemblyIOUnit ioUnitImport, TileEntityAssemblyIOUnit ioUnitExport, TileEntityAssemblyDrill drill,
        TileEntityAssemblyLaser laser);
                ...
                problemList.addAll(
                    PneumaticCraftUtils.convertStringIntoList(
                        EnumChatFormatting.GRAY
                            + "The input IO Unit can't find an inventory with a Block of Compressed Iron.",
                        GuiConstants.maxCharPerLineLeft));
                problemList.addAll(
                    PneumaticCraftUtils.convertStringIntoList(
                        EnumChatFormatting.BLACK
                            + "Place an inventory with a Block of Compressed Iron surrounding the IO Unit.",
                        GuiConstants.maxCharPerLineLeft));
                break;
            case NO_OUTPUT:
                problemList.addAll(
                    PneumaticCraftUtils.convertStringIntoList(
                        EnumChatFormatting.GRAY + "The output IO Unit can't find an inventory to place the output in.",
                        GuiConstants.maxCharPerLineLeft));
                problemList.addAll(
                    PneumaticCraftUtils.convertStringIntoList(
                        EnumChatFormatting.BLACK + "Place an inventory / make space in a connected inventory.",
                        GuiConstants.maxCharPerLineLeft));
                break;
        }
    }
    ...
    public static boolean isValidInput(AssemblyRecipe recipe, ItemStack input) {
        return input != null
            && (input.isItemEqual(recipe.getInput())
                || PneumaticCraftUtils.isSameOreDictStack(input, recipe.getInput()))
