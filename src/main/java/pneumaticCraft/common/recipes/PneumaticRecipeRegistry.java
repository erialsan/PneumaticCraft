...
public class PneumaticRecipeRegistry implements IPneumaticRecipeRegistry {
    ...
    public void registerThermopneumaticProcessingPlantRecipe(FluidStack requiredFluid, ItemStack requiredItem,
        ...
        registerThermopneumaticProcessingPlantRecipe(
            new BasicThermopneumaticProcessingPlantRecipe(
                requiredFluid,
                requiredItem,
                output,
                requiredTemperature,
    ...
    public static ItemStack getSingleStack(Object o) {
        if (o instanceof ItemStack) {
            ...
            ItemStack s = OreDictionaryHelper.getOreDictEntries(pair.getKey())
                .get(0);
            s = s.copy();
    ...
    public void registerDefaultPeriodicAmadronOffer(Object input, Object output) {
        AmadronOffer offer = new AmadronOffer(input, output);
        AmadronOfferManager.getInstance()
            .addPeriodicOffer(offer);
    }

    @Override
    public void registerHeatFrameCoolRecipe(Object input, ItemStack output) {
        if (input == null) throw new NullPointerException("Input can't be null!");
        if (!(input instanceof ItemStack) && !(input instanceof Pair)) throw new IllegalArgumentException(
            "Input needs to be of type ItemStack or (Apache's) Pair<String, Integer>. Violating object: " + input);
