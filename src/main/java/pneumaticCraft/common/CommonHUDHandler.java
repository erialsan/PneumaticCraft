...
public class CommonHUDHandler {
    ...
    public boolean[] upgradeRenderersInserted = new boolean[UpgradeRenderHandlerList.instance().upgradeRenderers
        .size()];
    ...
    public static CommonHUDHandler getHandlerForPlayer(EntityPlayer player) {
        CommonHUDHandler handler = PneumaticCraft.proxy.getCommonHudHandler().playerHudHandlers
            .get(player.getCommandSenderName());
        if (handler != null) return handler;
        PneumaticCraft.proxy.getCommonHudHandler().playerHudHandlers
            .put(player.getCommandSenderName(), new CommonHUDHandler());
        return getHandlerForPlayer(player);
    }

    @SideOnly(Side.CLIENT)
    public static CommonHUDHandler getHandlerForPlayer() {
        return getHandlerForPlayer(
                ...
                if (helmetStack != null && helmetStack.getItem() == Itemss.pneumaticHelmet) {
                    ...
                    if (!player.worldObj.isRemote) {
                        if (ticksExisted > getStartupTime() && !player.capabilities.isCreativeMode) {
                            ((IPressurizable) helmetStack.getItem()).addAir(
                                helmetStack,
                                (int) -UpgradeRenderHandlerList.instance()
                                    .getAirUsage(player, false));
                        }
                    }
    ...
    private void handleHacking(EntityPlayer player) {
        if (hackedBlock != null) {
            ...
            if (hackableBlock != null) {
                if (++hackTime >= hackableBlock
                    .getHackTime(hackedBlock.world, hackedBlock.x, hackedBlock.y, hackedBlock.z, player)) {
                    hackableBlock.onHackFinished(player.worldObj, hackedBlock.x, hackedBlock.y, hackedBlock.z, player);
                    PneumaticCraft.proxy.getHackTickHandler()
                        .trackBlock(hackedBlock, hackableBlock);
            ...
            if (hackableEntity != null) {
                if (++hackTime >= hackableEntity.getHackTime(hackedEntity, player)) {
                    ...
                    PneumaticCraft.proxy.getHackTickHandler()
                        .trackEntity(hackedEntity, hackableEntity);
                    NetworkHandler.sendToAllAround(
                        new PacketHackingEntityFinish(hackedEntity),
                        new NetworkRegistry.TargetPoint(
                            hackedEntity.worldObj.provider.dimensionId,
                            hackedEntity.posX,
                            hackedEntity.posY,
                            hackedEntity.posZ,
                            64));
    ...
    public void checkHelmetInventory(ItemStack helmetStack) {
        ...
        for (int i = 0; i < UpgradeRenderHandlerList.instance().upgradeRenderers.size(); i++) {
            upgradeRenderersInserted[i] = UpgradeRenderHandlerList.instance().upgradeRenderers.get(i)
                .isEnabled(helmetStacks);
        }
    }
