...
public class BlockPneumaticDoor extends BlockPneumaticCraftModeled {
    ...
    public boolean isTrackingPlayerEye;// will be true when the Pneumatic Door Base is determining if it should open the
                                       // door dependant
    ...
    public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int par6, float par7,
        ...
        if (!world.isRemote && doorBase != null
            && doorBase.redstoneMode == 2
            && doorBase.getPressure(ForgeDirection.UNKNOWN) >= PneumaticValues.MIN_PRESSURE_PNEUMATIC_DOOR) {
    ...
    public void breakBlock(World world, int x, int y, int z, Block block, int meta) {
        if (meta >= 6) {
            if (world.getBlock(x, y - 1, z) == Blockss.pneumaticDoor)
                dropBlockAsItem(world, x, y - 1, z, new ItemStack(Blockss.pneumaticDoor));
    ...
    private TileEntityPneumaticDoorBase getDoorBase(World world, int x, int y, int z) {
        ...
        if (meta < 6) {
            ...
            TileEntity te1 = world.getTileEntity(
                x + dir.getRotation(ForgeDirection.UP).offsetX,
                y,
                z + dir.getRotation(ForgeDirection.UP).offsetZ);
            ...
            TileEntity te2 = world.getTileEntity(
                x + dir.getRotation(ForgeDirection.DOWN).offsetX,
                y,
                z + dir.getRotation(ForgeDirection.DOWN).offsetZ);
