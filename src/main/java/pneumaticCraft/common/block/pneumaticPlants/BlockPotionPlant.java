...
public class BlockPotionPlant extends BlockPneumaticPlantBase {
    ...
    /**
     * Significantly (twice as much per 3 brewing stands) increase the growth rate when there brewing stands that are
     * currently brewing around the plant.
     */
    @Override
        ...
        for (int i = x - SQUARE_RADIUS; i <= x + SQUARE_RADIUS; i++) {
            for (int j = y - MAX_HEIGHT_DIFF; j <= y + MAX_HEIGHT_DIFF; j++) {
                for (int k = z - SQUARE_RADIUS; k <= z + SQUARE_RADIUS; k++) {
                    if (world.getBlock(i, j, k) == Blocks.brewing_stand
                        && world.getTileEntity(i, j, k) instanceof TileEntityBrewingStand) {
