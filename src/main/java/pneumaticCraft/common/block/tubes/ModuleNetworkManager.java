...
public class ModuleNetworkManager {
    ...
    public Set<TubeModule> getConnectedModules(TubeModule module) {
        ...
        while (!pendingTubes.isEmpty()) {
            ...
            TileEntityCache[] cache = ((TileEntityPneumaticBase) ((IPneumaticMachine) tube).getAirHandler())
                .getTileCache();
            for (ForgeDirection d : ForgeDirection.VALID_DIRECTIONS) {
                if (tube.sidesConnected[d.ordinal()]) {
                    TileEntityPressureTube newTube = ModInteractionUtils.getInstance()
                        .getTube(cache[d.ordinal()].getTileEntity());
