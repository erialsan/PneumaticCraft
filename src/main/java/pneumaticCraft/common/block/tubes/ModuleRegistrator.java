    ...
    public static void registerModule(Class<? extends TubeModule> moduleClass) {
        try {
            ...
            ModInteractionUtils.getInstance()
                .registerModulePart(module.getType());
            Item moduleItem = ModInteractionUtils.getInstance()
                .getModuleItem(module.getType());
            ...
            Log.error(
                "Not able to create an instance of the module " + moduleClass.getName()
                    + ". Is the constructor a parameterless one?");
            e.printStackTrace();
