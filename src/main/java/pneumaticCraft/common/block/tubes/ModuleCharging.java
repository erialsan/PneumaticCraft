...
public class ModuleCharging extends TubeModule {
            ...
            for (int i = 0; i < (upgraded ? 10 : 1) * PneumaticValues.CHARGING_STATION_CHARGE_RATE; i++) {
                ...
                for (int slot : accessibleSlots) {
                    ...
                    if (chargedItem != null && chargedItem.getItem() instanceof IPressurizable) {
                        ...
                        if (chargingItem.getPressure(chargedItem)
                            > airHandler.getPressure(ForgeDirection.UNKNOWN) + 0.01F
                            && chargingItem.getPressure(chargedItem) > 0F) {
                            chargingItem.addAir(chargedItem, -1);
                            airHandler.addAir(1, ForgeDirection.UNKNOWN);
                            charged = true;
                        } else if (chargingItem.getPressure(chargedItem)
                            < airHandler.getPressure(ForgeDirection.UNKNOWN) - 0.01F
                            && chargingItem.getPressure(chargedItem) < chargingItem.maxPressure(chargedItem)) {// if
                                                                                                               // there
                                                                                                               // is
                                                                                                               // pressure,
                                                                                                               // and
    ...
    private IInventory getConnectedInventory() {
        if (connectedInventory == null) {
            connectedInventory = new TileEntityCache(
                pressureTube.world(),
                pressureTube.x() + dir.offsetX,
                pressureTube.y() + dir.offsetY,
                pressureTube.z() + dir.offsetZ);
        }
        return connectedInventory.getTileEntity() instanceof IInventory
            ? (IInventory) connectedInventory.getTileEntity()
            : null;
    }
}
