...
public class ModuleRegulatorTube extends TubeModuleRedstoneReceiving implements IInfluenceDispersing {
    ...
    protected void renderModule() {
        ...
        if (isFake()) {
            if (!hasTicked) {
                ...
                TileEntity neighbor = tile.getWorldObj()
                ...
                if (inLine) {
                    inverted = ((IPneumaticMachine) neighbor).getAirHandler()
                        .getPressure(dir) > tile.getPressure(ForgeDirection.UNKNOWN);
                    NetworkHandler.sendToServer(
                        new PacketDescriptionPacketRequest(neighbor.xCoord, neighbor.yCoord, neighbor.zCoord));
                }
                hasTicked = true;
            }
    ...
    public int getMaxDispersion() {
        ...
        for (Pair<ForgeDirection, IAirHandler> entry : pressureTube.getAirHandler()
            .getConnectedPneumatics()) {
            if (entry.getKey()
                .equals(dir)) {
    ...
    public void addInfo(List<String> curInfo) {
        ...
        curInfo.add(
            "Threshold: " + EnumChatFormatting.WHITE + PneumaticCraftUtils.roundNumberTo(getThreshold(), 1) + " bar");
    }
