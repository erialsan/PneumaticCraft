...
public class ModuleLogistics extends TubeModule {
    ...
    public SemiBlockLogistics getFrame() {
        if (cachedFrame == null) {
            ISemiBlock semiBlock = SemiBlockManager.getInstance(getTube().world())
                .getSemiBlock(
                    getTube().world(),
                    getTube().x() + dir.offsetX,
                    getTube().y() + dir.offsetY,
                    getTube().z() + dir.offsetZ);
    ...
    public void update() {
        ...
        if (!getTube().world().isRemote) {
            if (powered != getTube().getAirHandler()
                .getPressure(null) >= MIN_PRESSURE) {
                powered = !powered;
                NetworkHandler.sendToAllAround(new PacketUpdateLogisticModule(this, 0), getTube().world());
            }
            if (--ticksUntilNextCycle <= 0) {
                ...
                for (TubeModule module : ModuleNetworkManager.getInstance()
                    .getConnectedModules(this)) {
                    if (module instanceof ModuleLogistics) {
                        ...
                        if (logistics.getColorChannel() == getColorChannel()) {
                            logistics.ticksUntilNextCycle = 100;// Make sure any connected module doesn't tick, set it
                                                                // to a 5 second timer. This is also a penalty value
                                                                // when no task is executed this tick.
                ...
                for (LogisticsTask task : tasks) {
                    if (task.isStillValid(
                        task.transportingItem != null ? task.transportingItem : task.transportingFluid.stack)) {
                        if (task.transportingItem != null) {
                            ItemStack remainder = IOHelper
                                .insert(task.requester.getTileEntity(), task.transportingItem.copy(), true);
                            if (remainder == null || remainder.stackSize != task.transportingItem.stackSize) {
                                ...
                                ItemStack extractedStack = IOHelper
                                    .extract(task.provider.getTileEntity(), toBeExtracted, true);
                                if (extractedStack != null) {
                                    ...
                                    int airUsed = (int) (ITEM_TRANSPORT_COST * extractedStack.stackSize
                                        * Math.pow(
                                            PneumaticCraftUtils.distBetweenSq(
                                                provider.getTube()
                                                    .x(),
                                                provider.getTube()
                                                    .y(),
                                                ...
                                                requester.getTube()
                                                    .z()),
                                            0.25));
                                    if (requester.getTube()
                                        .getAirHandler()
                                        .getCurrentAir(null) > airUsed) {
                                    ...
                                    if (amountFilled > 0) {
                                        ...
                                        for (ForgeDirection d : ForgeDirection.VALID_DIRECTIONS) {
                                            ...
                                            if (extractedFluid != null) {
                                                airUsed = (int) (FLUID_TRANSPORT_COST * extractedFluid.amount
    ...
    private void sendModuleUpdate(ModuleLogistics module, boolean enoughAir) {
        NetworkHandler.sendToAllAround(
            new PacketUpdateLogisticModule(module, enoughAir ? 1 : 2),
            module.getTube()
                .world());
    }
    ...
    public void addInfo(List<String> curInfo) {
        ...
        curInfo.add(
            StatCollector.translateToLocal("waila.logisticsModule.channel") + " "
                + EnumChatFormatting.YELLOW
                + StatCollector.translateToLocal("item.fireworksCharge." + ItemDye.field_150923_a[colorChannel]));
    }
}
