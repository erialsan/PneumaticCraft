...
public class BlockChargingStation extends BlockPneumaticCraftModeled {
    ...
    public void setBlockBoundsBasedOnState(IBlockAccess world, int x, int y, int z) {
        if (((TileEntityChargingStation) world.getTileEntity(x, y, z))
            .getUpgrades(ItemMachineUpgrade.UPGRADE_DISPENSER_DAMAGE) > 0) {
            setBlockBounds(0.0F, 0.0F, 0.0F, 1.0F, 1.0F, 1.0F);
        } else {
            setBlockBounds(
                BBConstants.CHARGING_STATION_MIN_POS,
                0F,
                BBConstants.CHARGING_STATION_MIN_POS,
                BBConstants.CHARGING_STATION_MAX_POS,
                BBConstants.CHARGING_STATION_MAX_POS_TOP,
                BBConstants.CHARGING_STATION_MAX_POS);
        }
    }
