...
public class BlockElevatorCaller extends BlockPneumaticCraft {
        ...
        if (rayTrace != null && rayTrace.sideHit == orientation.ordinal()) {
            ...
            if (te instanceof TileEntityElevatorCaller) {
                ...
                for (TileEntityElevatorCaller.ElevatorButton button : caller.getFloors()) {
                    ...
                    setBlockBounds(
                        startX,
                        1 - (float) (button.posY + button.height),
                        startZ,
                        endX,
                        1 - (float) button.posY,
                        endZ);
                    ...
                    if (buttonTrace != null) {
                        ...
                        setBlockBounds(
                            startX,
                            1.01F - (float) (button.posY + button.height),
                            startZ,
                            endX,
                            0.99F - (float) button.posY,
                            endZ);
    ...
    public boolean isOpaqueCube() {
        return false;// this should return false, because otherwise I can't give color to the rendered elevator buttons
                     // for some reason...
    }
