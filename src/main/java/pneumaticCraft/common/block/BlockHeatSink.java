...
public class BlockHeatSink extends BlockPneumaticCraftModeled {
    ...
    public void setBlockBoundsBasedOnState(IBlockAccess blockAccess, int par2, int par3, int par4) {
        ...
        setBlockBounds(
            dir.offsetX <= 0 ? 0 : 1F - BBConstants.HEAT_SINK_THICKNESS,
            dir.offsetY <= 0 ? 0 : 1F - BBConstants.HEAT_SINK_THICKNESS,
            dir.offsetZ <= 0 ? 0 : 1F - BBConstants.HEAT_SINK_THICKNESS,
            dir.offsetX >= 0 ? 1 : BBConstants.HEAT_SINK_THICKNESS,
            dir.offsetY >= 0 ? 1 : BBConstants.HEAT_SINK_THICKNESS,
            dir.offsetZ >= 0 ? 1 : BBConstants.HEAT_SINK_THICKNESS);
    }
    ...
    public void onEntityCollidedWithBlock(World world, int x, int y, int z, Entity entity) {
        ...
        if (heatSink.getHeatExchangerLogic(ForgeDirection.UNKNOWN)
            .getTemperature() > 323) {
            entity.setFire(3);
        }
    }
