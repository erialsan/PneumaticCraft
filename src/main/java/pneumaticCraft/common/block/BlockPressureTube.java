...
public class BlockPressureTube extends BlockPneumaticCraftModeled {
    ...
    public BlockPressureTube(Material par2Material, float dangerPressure, float criticalPressure, int volume) {
        ...
        boundingBoxes[0] = AxisAlignedBB.getBoundingBox(
            0.5 - width,
            BBConstants.PRESSURE_PIPE_MIN_POS - height,
            0.5 - width,
            0.5 + width,
            BBConstants.PRESSURE_PIPE_MIN_POS,
            0.5 + width);
        boundingBoxes[1] = AxisAlignedBB.getBoundingBox(
            0.5 - width,
            BBConstants.PRESSURE_PIPE_MAX_POS,
            0.5 - width,
            0.5 + width,
            BBConstants.PRESSURE_PIPE_MAX_POS + height,
            0.5 + width);
        boundingBoxes[2] = AxisAlignedBB.getBoundingBox(
            0.5 - width,
            0.5 - width,
            BBConstants.PRESSURE_PIPE_MIN_POS - height,
            0.5 + width,
            0.5 + width,
            BBConstants.PRESSURE_PIPE_MIN_POS);
        boundingBoxes[3] = AxisAlignedBB.getBoundingBox(
            0.5 - width,
            0.5 - width,
            BBConstants.PRESSURE_PIPE_MAX_POS,
            0.5 + width,
            0.5 + width,
            BBConstants.PRESSURE_PIPE_MAX_POS + height);
        boundingBoxes[4] = AxisAlignedBB.getBoundingBox(
            BBConstants.PRESSURE_PIPE_MIN_POS - height,
            0.5 - width,
            0.5 - width,
            BBConstants.PRESSURE_PIPE_MIN_POS,
            0.5 + width,
            0.5 + width);
        boundingBoxes[5] = AxisAlignedBB.getBoundingBox(
            BBConstants.PRESSURE_PIPE_MAX_POS,
            0.5 - width,
            0.5 - width,
            BBConstants.PRESSURE_PIPE_MAX_POS + height,
    ...
    public boolean tryPlaceModule(EntityPlayer player, World world, int x, int y, int z, int par6, boolean simulate) {
        if (player.getCurrentEquippedItem() != null) {
            if (player.getCurrentEquippedItem()
                .getItem() instanceof ItemTubeModule) {
                TileEntityPressureTube pressureTube = ModInteractionUtils.getInstance()
                    .getTube(world.getTileEntity(x, y, z));
                if (pressureTube.modules[par6] == null && ModInteractionUtils.getInstance()
                    .occlusionTest(boundingBoxes[par6], world.getTileEntity(x, y, z))) {
                    TubeModule module = ModuleRegistrator.getModule(
                        ((ItemTubeModule) player.getCurrentEquippedItem()
                            .getItem()).moduleName);
                    ...
                    if (!simulate) {
                        ...
                        world.notifyBlocksOfNeighborChange(
                            ...
                            z,
                            this,
                            ForgeDirection.getOrientation(par6)
                                .getOpposite()
                                .ordinal());
                        ...
                        world.playSoundEffect(
                            ...
                            y + 0.5,
                            z + 0.5,
                            Block.soundTypeGlass.getStepResourcePath(),
                            Block.soundTypeGlass.getVolume() * 5.0F,
                            Block.soundTypeGlass.getPitch() * .9F);
                    }
                    return true;
                }
            } else if (player.getCurrentEquippedItem()
                .getItem() == Itemss.advancedPCB && !simulate) {
    ...
    public static TubeModule getLookedModule(World world, int x, int y, int z, EntityPlayer player) {
        ...
        MovingObjectPosition mop = Blockss.pressureTube
            .collisionRayTrace(world, x, y, z, vecs.getLeft(), vecs.getRight());
        if (mop != null && mop.hitInfo instanceof ForgeDirection
            && (ForgeDirection) mop.hitInfo != ForgeDirection.UNKNOWN) {
            TileEntityPressureTube tube = ModInteractionUtils.getInstance()
                .getTube(world.getTileEntity(x, y, z));
    ...
    private boolean isCloserMOP(Vec3 origin, MovingObjectPosition originalMOP, MovingObjectPosition newMOP) {
        ...
        return PneumaticCraftUtils.distBetween(origin, newMOP.hitVec)
            < PneumaticCraftUtils.distBetween(origin, originalMOP.hitVec);
    }
    ...
    public ItemStack getPickBlock(MovingObjectPosition target, World world, int x, int y, int z) {
        if (target.hitInfo == ForgeDirection.UNKNOWN) {
            ...
            return new ItemStack(
                ModuleRegistrator.getModuleItem(tube.modules[((ForgeDirection) target.hitInfo).ordinal()].getType()));
        }
    }
        ...
        if (player.isSneaking()) {
            ...
            if (!player.capabilities.isCreativeMode) {
                EntityItem entity = new EntityItem(
                    ...
                    z + 0.5,
                    new ItemStack(
                        tube.maxPressure <= PneumaticValues.MAX_PRESSURE_PRESSURE_TUBE ? Blockss.pressureTube
                            : Blockss.advancedPressureTube));
                world.spawnEntityInWorld(entity);
                entity.onCollideWithPlayer(player);
            }
            ModInteractionUtils.getInstance()
                .removeTube(world.getTileEntity(x, y, z));
            return true;
        } else {
    ...
    public void randomDisplayTick(World par1World, int par2, int par3, int par4, Random par5Random) {
        ...
        if (te instanceof TileEntityPressureTube) {
            TileEntityPressureTube tePt = (TileEntityPressureTube) te;
            int l = 0;
            for (TubeModule module : tePt.modules) if (module != null) l = Math.max(l, module.getRedstoneLevel());
            if (l > 0) {
                // for(int i = 0; i < 4; i++){
        ...
        if (te instanceof TileEntityPressureTube) {
            ...
            for (int i = 0; i < 6; i++) {
                if (tePt.modules[i] != null) {
                    if ((side ^ 1) == i || i != side && tePt.modules[i].isInline()) {// if we are on the same side, or
                                                                                     // when we have an 'in line' module
                                                                                     // that is not on the opposite
                                                                                     // side.
    ...
    public boolean canConnectRedstone(IBlockAccess world, int x, int y, int z, int side) {
        ...
        for (int i = 0; i < 6; i++) {
            if (tube.modules[i] != null) {
                if ((side ^ 1) == i || i != side && tube.modules[i].isInline()) {// if we are on the same side, or when
                                                                                 // we have an 'in line' module that is
