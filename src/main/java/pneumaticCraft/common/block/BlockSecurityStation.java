...
public class BlockSecurityStation extends BlockPneumaticCraftModeled {
    ...
    public void setBlockBoundsBasedOnState(IBlockAccess par1IBlockAccess, int par2, int par3, int par4) {
        setBlockBounds(
            BBConstants.SECURITY_STATION_MIN_POS,
            0F,
            BBConstants.SECURITY_STATION_MIN_POS,
            BBConstants.SECURITY_STATION_MAX_POS,
            BBConstants.SECURITY_STATION_MAX_POS_TOP,
            BBConstants.SECURITY_STATION_MAX_POS);
    }
                ...
                if (te != null) {
                    if (te.isPlayerOnWhiteList(player)) {
                        player.openGui(
                            ...
                            EnumGuiId.SECURITY_STATION_INVENTORY.ordinal(),
                            world,
                            x,
                            y,
                            z);
                        ...
                        player.addChatComponentMessage(
                            new ChatComponentTranslation(
                                EnumChatFormatting.GREEN + "You've already hacked this Security Station!"));
                    } else if (getPlayerHackLevel(player) < te.getSecurityLevel()) {
                        player.addChatComponentMessage(
                            new ChatComponentTranslation(
                                EnumChatFormatting.RED
                                    + "You can't access or hack this Security Station. To hack it you need at least a Pneumatic Helmet upgraded with "
                                    + te.getSecurityLevel()
                                    + " Security upgrade(s)."));
                    } else {
