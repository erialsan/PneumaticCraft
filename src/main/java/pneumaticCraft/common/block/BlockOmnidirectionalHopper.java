...
public class BlockOmnidirectionalHopper extends BlockPneumaticCraftModeled {
    ...
    @Override
    public void onBlockPlacedBy(World world, int par2, int par3, int par4, EntityLivingBase par5EntityLiving,
        ItemStack par6ItemStack) {
        ((TileEntityOmnidirectionalHopper) world.getTileEntity(par2, par3, par4)).setDirection(
            PneumaticCraftUtils.getDirectionFacing(par5EntityLiving, true)
                .getOpposite());
    }
    ...
    public boolean rotateBlock(World world, EntityPlayer player, int x, int y, int z, ForgeDirection face) {
        ...
        if (te instanceof TileEntityOmnidirectionalHopper) {
            ...
            if (player != null && player.isSneaking()) {
                ...
                if (newMeta == teOh.getDirection()
                    .ordinal()) newMeta = (newMeta + 1) % 6;
                world.setBlockMetadataWithNotify(x, y, z, newMeta, 3);
            } else {
                int newRotation = (teOh.getDirection()
                    .ordinal() + 1) % 6;
    ...
    public MovingObjectPosition collisionRayTrace(World world, int x, int y, int z, Vec3 origin, Vec3 direction) {
        ...
        if (te instanceof TileEntityOmnidirectionalHopper) {
            ...
            setBlockBounds(
                o.offsetX == 1 ? 10 / 16F : 0,
                o.offsetY == 1 ? 10 / 16F : 0,
                o.offsetZ == 1 ? 10 / 16F : 0,
                o.offsetX == -1 ? 6 / 16F : 1,
                o.offsetY == -1 ? 6 / 16F : 1,
                o.offsetZ == -1 ? 6 / 16F : 1);
