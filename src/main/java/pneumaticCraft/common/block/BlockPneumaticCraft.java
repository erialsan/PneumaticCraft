...
public abstract class BlockPneumaticCraft extends BlockContainer implements IPneumaticWrenchable, IPeripheralProvider {
    ...
    protected BlockPneumaticCraft(Material par2Material) {
        super(par2Material);
        setBlockTextureName(Textures.BLOCK_PRESSURE_TUBE); // registering an icon to render for the block breaking
                                                           // animation
    ...
    public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int par6, float par7,
        ...
        if (player.isSneaking() || getGuiID() == null
            || isRotatable() && player.getCurrentEquippedItem() != null
                && (player.getCurrentEquippedItem()
                    .getItem() == Itemss.manometer || ModInteractionUtils.getInstance()
                        .isModdedWrench(
                            player.getCurrentEquippedItem()
                                .getItem())))
            return false;
            ...
            if (!world.isRemote) {
                ...
                if (te != null && !FluidUtils.tryInsertingLiquid(
                    te,
                    player.getCurrentEquippedItem(),
    ...
    public void onBlockPlacedBy(World par1World, int par2, int par3, int par4, EntityLivingBase par5EntityLiving,
        ...
        if (isRotatable()) {
            int l = PneumaticCraftUtils.getDirectionFacing(par5EntityLiving, canRotateToTopOrBottom())
                .ordinal();
            ...
            if (isRotatable()) {
                ...
                if (!rotateCustom(world, x, y, z, side, meta)) {
                    ...
                    if (rotateForgeWay()) {
                        ...
                        newMeta = ForgeDirection.getOrientation(meta)
                            .getRotation(side)
                            .ordinal();
    ...
    public int getComparatorInputOverride(World world, int x, int y, int z, int side) {
        return ((IComparatorSupport) world.getTileEntity(x, y, z))
            .getComparatorValue(ForgeDirection.getOrientation(side));
    }
}
