...
public class IOHelper {
    ...
    public static ItemStack extract(TileEntity tile, ForgeDirection direction, ItemStack requestedStack,
        boolean useItemCount, boolean simulate) {
     ...
     * @param useItemCount
     *                       if true, it'll only retrieve the stack of the exact item count given. it'll look in
     *                       multiple slots of the inventory. if false, the
     ...
     */
    public static ItemStack extract(TileEntity tile, ForgeDirection direction, ItemStack requestedStack,
        boolean useItemCount, boolean simulate, int fuzzySetting) {
        ...
        if (inv != null) {
            ...
            for (int slot : accessibleSlots) {
                ...
                if (stack != null && stack.stackSize > 0
                    && stack.isItemEqual(requestedStack)
            ...
            if (itemsFound >= requestedStack.stackSize) {
                ...
                for (int slot : accessibleSlots) {
                    ItemStack stack = inv.getStackInSlot(slot);
                    if (stack != null && stack.isItemEqual(requestedStack)
                        && IOHelper.canExtractItemFromInventory(inv, stack, slot, direction.ordinal())) {
    ...
    public static ItemStack extractOneItem(TileEntity tile, ForgeDirection dir, boolean simulate) {
        ...
        if (inv != null) {
            ...
            for (int slot : accessibleSlots) {
                ItemStack stack = inv.getStackInSlot(slot);
                if (stack != null && stack.stackSize > 0
                    && IOHelper.canExtractItemFromInventory(inv, stack, slot, dir.ordinal())) {
                    if (simulate) {
    ...
    public static boolean canInsertItemToInventory(IInventory inventory, ItemStack itemStack, int slot,
        boolean[] sides) {
    ...
    public static boolean canExtractItemFromInventory(IInventory inventory, ItemStack itemStack, int slot,
        boolean[] sides) {
        for (int i = 0; i < sides.length; i++) {
    ...
    public static boolean canInsertItemToInventory(IInventory inventory, ItemStack itemStack, int slot, int side) {
        ...
        return inventory.isItemValidForSlot(slot, itemStack) && (!(inventory instanceof ISidedInventory)
            || ((ISidedInventory) inventory).canInsertItem(slot, itemStack, side));
    }
    ...
    public static boolean canExtractItemFromInventory(IInventory inventory, ItemStack itemStack, int slot, int side) {
        ...
        return !(inventory instanceof ISidedInventory)
            || ((ISidedInventory) inventory).canExtractItem(slot, itemStack, side);
    }
