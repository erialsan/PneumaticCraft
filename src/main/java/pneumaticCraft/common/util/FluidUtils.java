...
public class FluidUtils {
    ...
    public static boolean tryInsertingLiquid(TileEntity te, ItemStack liquidContainer, boolean creative,
        ...
        if (te instanceof IFluidHandler) {
            ...
            if (liquidContainer != null) {
                ...
                if (fluid != null) {
                    fluid.amount = 1000;
                    if (fluidHandler.canFill(ForgeDirection.UNKNOWN, fluid.getFluid())
                        && fluidHandler.fill(ForgeDirection.UNKNOWN, fluid, false) == 1000) {
                        fluidHandler.fill(ForgeDirection.UNKNOWN, fluid, true);
                        if (!creative) {
                            liquidContainer.stackSize--;

                            ItemStack returnedItem = null;
                            FluidContainerData[] allFluidData = FluidContainerRegistry
                    ...
                    if (fluid != null) {
                        fluid = fluid.copy();
                        if (fluidHandler.canFill(ForgeDirection.UNKNOWN, fluid.getFluid())
                            && fluidHandler.fill(ForgeDirection.UNKNOWN, fluid, false) == fluid.amount) {
                ...
                if (containerCapacity > 0 || liquidContainer.getItem() == Items.bucket) {
                    ...
                    if (extractedLiquid != null && extractedLiquid.amount == containerCapacity) {
                        ItemStack filledContainer = FluidContainerRegistry
                            .fillFluidContainer(extractedLiquid, liquidContainer);
                    ...
                    FluidStack extractedLiquid = fluidHandler
                        .drain(ForgeDirection.UNKNOWN, container.getCapacity(singleItem), false);
