...
public abstract class SemiBlockLogistics extends SemiBlockBasic {
    ...
    public void update() {
        super.update();
        if (!world.isRemote) {
            Iterator<Map.Entry<ItemStack, Integer>> iterator = incomingStacks.entrySet()
                .iterator();
            ...
            Iterator<Map.Entry<FluidStackWrapper, Integer>> it = incomingFluid.entrySet()
                .iterator();
            while (it.hasNext()) {
    ...
    private boolean playerIsHoldingLogisticItems() {
        ...
        return stack != null
            && (stack.getItem() == Itemss.logisticsConfigurator || stack.getItem() instanceof ItemLogisticsFrame);
    }
    ...
    public void readFromNBT(NBTTagCompound tag) {
        ...
        for (int i = 0; i < tagList.tagCount(); i++) {
            fluidFilters[tagList.getCompoundTagAt(i)
                .getInteger("index")].readFromNBT(tagList.getCompoundTagAt(i));
        }
