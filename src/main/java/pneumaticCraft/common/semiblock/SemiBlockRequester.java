...
@Optional.InterfaceList({ @Interface(iface = "appeng.api.networking.IGridHost", modid = ModIds.AE2),
    ...
    @Interface(iface = "pneumaticCraft.common.semiblock.MEInventoryExtension", modid = ModIds.AE2),
...
public class SemiBlockRequester extends SemiBlockLogistics implements ISpecificRequester, IProvidingInventoryListener,
    IGridHost, IGridBlock, ICraftingProvider, ICraftingWatcherHost, IStackWatcherHost, ICellContainer, IGridTickable {
    ...
    public void update() {
        ...
        if (!world.isRemote) {
            ...
            Iterator<Map.Entry<TileEntity, Integer>> iterator = providingInventories.entrySet()
                .iterator();
    ...
    public boolean isPlacedOnInterface() {
        return getTileEntity() != null && AEApi.instance()
            .definitions()
            .blocks()
            .iface()
            .maybeBlock()
            .get() == getTileEntity().getBlockType();
    }
    ...
    private boolean checkForInterface() {
        if (isPlacedOnInterface()) {
            ...
            if (te instanceof IGridHost) {
                ...
                try {
                    AEApi.instance()
                        .createGridConnection(getGridNode(null), ((IGridHost) te).getGridNode(null));
    ...
    public TickRateModulation tickingRequest(IGridNode arg0, int arg1) {
        notifyNetworkOfCraftingChange();
        if (gridNode != null) {
            getGridNode(null).getGrid()
                .postEvent(new MENetworkCellArrayUpdate());// Doing it on interval, as doing it right after
                                                           // AEApi.instance().createGridConnection doesn't seem to
                                                           // work..
        }
