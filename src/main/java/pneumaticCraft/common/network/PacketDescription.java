...
public class PacketDescription extends LocationIntPacket<PacketDescription> {
    ...
    public PacketDescription(IDescSynced te) {
        ...
        type = te.getSyncType();
        values = new Object[te.getDescriptionFields()
            .size()];
        types = new byte[values.length];
        for (int i = 0; i < values.length; i++) {
            values[i] = te.getDescriptionFields()
                .get(i)
                .getValue();
            types[i] = PacketUpdateGui.getType(
                te.getDescriptionFields()
                    .get(i));
        }
        ...
        switch (type) {
            ...
            case SEMI_BLOCK:
                if (message.x == 0 && message.y == 0 && message.z == 0) {
                    ...
                    return SemiBlockManager.getInstance(player.worldObj)
