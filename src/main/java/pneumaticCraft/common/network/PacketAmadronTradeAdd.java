...
public class PacketAmadronTradeAdd extends PacketAbstractAmadronTrade<PacketAmadronTradeAdd> {
    ...
    public void handleClientSide(PacketAmadronTradeAdd message, EntityPlayer player) {
        if (AmadronOfferSettings.notifyOfTradeAddition) player.addChatMessage(
            new ChatComponentText(
                I18n.format(
                    "message.amadron.playerAddedTrade",
                    message.getOffer()
                        .getVendor(),
                    WidgetAmadronOffer.getStringForObject(
                        message.getOffer()
                            .getOutput()),
                    WidgetAmadronOffer.getStringForObject(
                        message.getOffer()
    ...
    public void handleServerSide(PacketAmadronTradeAdd message, EntityPlayer player) {
        ...
        } else if (AmadronOfferManager.getInstance()
