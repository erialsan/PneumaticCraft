 ...
 * the
 * block's class. If you don't have access to the class (vanilla blocks), you can implement this interface in a separate
 * class
 * and register it using PneumaticRegistry.registry.addHackable(blockClass, IHackableBlockClass). With the former way
 * there will be one instance only per type. In the latter, there will
...
public interface IHackableBlock {
    ...
    /**
     * Returning true will allow the player to hack this block. This can be used to only allow hacking on certain
     * conditions.
     */
    ...
    /**
     * Return the time it takes to hack this block in ticks. For more powerful hacks, a longer required hacking time is
     * adviced.
     */
    public int getHackTime(IBlockAccess world, int x, int y, int z, EntityPlayer player);

    /**
     * When the player hacked the block for getHackTime(World, int, int, int) ticks this will be called on both server
     * and client side.
     */
     ...
     * going (for mob spawners, to keep them neutralized),
