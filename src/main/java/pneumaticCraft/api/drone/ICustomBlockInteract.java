...
public interface ICustomBlockInteract {
     ...
     * 
     * For every position in the selected area the drone will visit every block (ordered from closest to furthest). It
     * will call this method with 'simulate = true'. If this method returns true, the drone will navigate to this
     * location, and call this method again with 'simulate = false' It will keep doing this until this method returns
     * false.
     * 
     * When interactHandler.useCount() returns true:
     * In the interface of the puzzle piece users can specify a 'use count' and fill in the maximum count they want to
     * use. When not simulating, you should only import/export up to interactHandler.getRemainingCount(), and you should
     * notify the removed/added count by doing interactHandler.decreaseCount(int count).
     * 
     ...
     * @param simulate        will be true when trying to figure out whether or not the drone should navigate to this
     *                        block, false when next to this block.
     * @return
     */
