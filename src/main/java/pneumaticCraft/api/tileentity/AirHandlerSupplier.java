...
public class AirHandlerSupplier {
    ...
    /**
     * Returns a new instance of an IAirHandler. This handler handles everything pressurized air related: Air
     * dispersion,
     ...
     * @param maxFlow          maximum mL/tick that this machine can disperse. Tier one machines do 50mL/tick while Tier
     *                         two have 200mL/tick.
     * @param volume           Volume of the machine's internal storage. These vary from 1000mL for small machines to
     *                         10,000mL for the big ones.
    ...
    public static IAirHandler getAirHandler(float dangerPressure, float criticalPressure, int volume) {
        ...
        try {
            if (airHandlerConstructor == null)
                airHandlerConstructor = Class.forName("pneumaticCraft.common.tileentity.TileEntityPneumaticBase")
                    .getConstructor(float.class, float.class, int.class);
            airHandler = (IAirHandler) airHandlerConstructor.newInstance(dangerPressure, criticalPressure, volume);
        } catch (Exception e) {
            System.err.println(
                "[PneumaticCraft API] An error has occured whilst trying to get an AirHandler. Here's a stacktrace:");
            e.printStackTrace();
        }
        return airHandler;
    }
