 ...
 * to power Furnaces with heat,
 * and to turn Lava into Obsidian when heat is drained. This only works for ticking heat logic, so not for static heat
 * sources like lava blocks.
 */
public abstract class HeatBehaviour<Tile extends TileEntity> {
     ...
     * (getWorld(), getX(), getY(), getZ(), getBlock(), getTileEntity()).
     * 
     * @return
     */
    ...
    public boolean equals(Object o) {
        if (o instanceof HeatBehaviour) {
            ...
            return behaviour.getId()
                .equals(getId()) && behaviour.getX() == getX()
                && behaviour.getY() == getY()
                && behaviour.getZ() == getZ();
        } else {
            return false;
