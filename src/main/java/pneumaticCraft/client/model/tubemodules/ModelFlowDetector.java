...
public class ModelFlowDetector extends ModelBase implements IBaseModel {
    ...
    public void renderDynamic(float size, TileEntity te, float partialTicks) {
        ...
        for (int i = 0; i < parts; i++) {
            Shape1.rotateAngleZ = (float) i / parts * 2 * (float) Math.PI + (flowDetector != null
                ? flowDetector.oldRotation + (flowDetector.rotation - flowDetector.oldRotation) * partialTicks
                : 0);
            Shape1.render(size);
        }
    }
