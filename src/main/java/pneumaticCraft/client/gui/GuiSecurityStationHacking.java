...
public class GuiSecurityStationHacking extends GuiSecurityStationBase {
    ...
    public void initGui() {
        ...
        addAnimatedStat("gui.tab.info", Textures.GUI_INFO_LOCATION, 0xFF8888FF, true)
            .setText("gui.tab.info.tile.securityStation.hacking");
        addAnimatedStat("gui.tab.upgrades", Textures.GUI_UPGRADES_LOCATION, 0xFF0000FF, true)
            .setText("gui.tab.upgrades.tile.securityStation.hacking");
        addAnimatedStat(
            Itemss.nukeVirus.getUnlocalizedName() + ".name",
            new ItemStack(Itemss.nukeVirus),
            0xFF18c9e8,
            false).setText("gui.tab.info.tile.securityStation.nukeVirus");
        addAnimatedStat(
            Itemss.stopWorm.getUnlocalizedName() + ".name",
            new ItemStack(Itemss.stopWorm),
            0xFFc13232,
            false).setText("gui.tab.info.tile.securityStation.stopWorm");

        if (playerBackgroundBridges == null) {
            playerBackgroundBridges = new NetworkConnectionBackground(
                this,
                te,
                xStart + 21,
                yStart + 26,
    ...
    protected void drawGuiContainerForegroundLayer(int x, int y) {
        ...
        fontRendererObj.drawString(
            (aiBridges.isTracing() ? EnumChatFormatting.RED : EnumChatFormatting.GREEN) + "Tracing: "
                + PneumaticCraftUtils.convertTicksToMinutesAndSeconds(aiBridges.getRemainingTraceTime(), true),
            15,
            7,
            4210752);
    ...
    protected void addProblems(List<String> text) {
        ...
        if (aiBridges.isTracing()) {
            ...
            text.add(
                EnumChatFormatting.BLACK + "Time till trace: "
                    + PneumaticCraftUtils.convertTicksToMinutesAndSeconds(aiBridges.getRemainingTraceTime(), false));
        }
    }
    ...
    protected void mouseClicked(int par1, int par2, int par3) {
        ...
        if (aiBridges.isTracing() && par1 >= guiLeft + 155
            && par1 <= guiLeft + 171
            && par2 >= guiTop + 55
            && par2 <= guiTop + 75) {
            EntityPlayer player = FMLClientHandler.instance()
    ...
    public void onGuiClosed() {
        if (aiBridges.isTracing() && !hackerBridges.hackedSuccessfully)
            NetworkHandler.sendToServer(new PacketSecurityStationFailedHack(te.xCoord, te.yCoord, te.zCoord));
    ...
    public void removeUpdatesOnConnectionHandlers() {
        ClientTickHandler.instance()
            .removeUpdatedObject(hackerBridges);
        ClientTickHandler.instance()
            .removeUpdatedObject(aiBridges);
    }
