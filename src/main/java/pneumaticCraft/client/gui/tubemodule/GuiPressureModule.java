...
public class GuiPressureModule extends GuiTubeModule {
    ...
    public void initGui() {
        ...
        addWidget(
            (IGuiWidget) new GuiAnimatedStat(
                this,
                "gui.tab.info",
                Textures.GUI_INFO_LOCATION,
                xStart,
                yStart + 5,
                0xFF8888FF,
                null,
                true).setText("gui.tab.info.tubeModule"));
        advancedMode = new GuiCheckBox(0, guiLeft + 6, guiTop + 15, 0xFF000000, "gui.tubeModule.advancedConfig")
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        for (int i = 0; i < 16; i++) {
            ...
            if (longer) {
                fontRendererObj.drawString(
                    i + "",
                    graphLeft - 5 - fontRendererObj.getStringWidth(i + ""),
                    graphHighY + (graphLowY - graphHighY) * (15 - i) / 15 - 3,
                    0xFF000000);
                drawHorizontalLine(
                    graphLeft + 4,
                    graphRight,
                    graphHighY + (graphLowY - graphHighY) * (15 - i) / 15,
                    i == 0 ? 0xFF000000 : 0x33000000);

            }
            drawHorizontalLine(
                graphLeft - (longer ? 5 : 3),
                graphLeft + 3,
                graphHighY + (graphLowY - graphHighY) * (15 - i) / 15,
                0xFF000000);
        }
        for (int i = 0; i < 31; i++) {
            ...
            if (longer) {
                fontRendererObj.drawString(
                    i + "",
                    graphLeft + (graphRight - graphLeft) * i / 30 - fontRendererObj.getStringWidth(i + "") / 2 + 1,
                    graphLowY + 6,
                    0xFF000000);
                drawVerticalLine(graphLeft + (graphRight - graphLeft) * i / 30, graphHighY, graphLowY - 2, 0x33000000);
            }
            drawVerticalLine(
                graphLeft + (graphRight - graphLeft) * i / 30,
                graphLowY - 3,
                graphLowY + (longer ? 5 : 3),
                0xFF000000);
        }
        ...
        if (module instanceof TubeModuleRedstoneReceiving) {
            ...
            drawHorizontalLine(
                graphLeft + 4,
                graphRight,
                graphHighY + (graphLowY - graphHighY)
                    * (15 - ((TubeModuleRedstoneReceiving) module).getReceivingRedstoneLevel())
                    / 15,
                0xFFFF0000);
            String status = "Current threshold: "
                + PneumaticCraftUtils.roundNumberTo(((TubeModuleRedstoneReceiving) module).getThreshold(), 1)
                + " bar";
            fontRendererObj.drawString(
                status,
                guiLeft + xSize / 2 - fontRendererObj.getStringWidth(status) / 2,
                guiTop + 173,
                0xFF000000);
        }
