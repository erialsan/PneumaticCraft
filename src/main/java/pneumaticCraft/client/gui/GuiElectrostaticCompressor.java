...
public class GuiElectrostaticCompressor extends GuiPneumaticContainerBase<TileEntityElectrostaticCompressor> {
    ...
    public String getRedstoneButtonText(int mode) {
        return mode == 0 ? "gui.tab.redstoneBehaviour.button.never"
    ...
    protected void addPressureStatInfo(List<String> pressureStatText) {
        ...
        pressureStatText.add(
            EnumChatFormatting.BLACK
                + PneumaticCraftUtils
                    .roundNumberTo(PneumaticValues.PRODUCTION_ELECTROSTATIC_COMPRESSOR / connectedCompressors, 1)
                + " mL/lightning strike");
        pressureStatText.add(EnumChatFormatting.GRAY + "Maximum air redirection:");
        pressureStatText.add(
            EnumChatFormatting.BLACK
                + PneumaticCraftUtils
                    .roundNumberTo(PneumaticValues.MAX_REDIRECTION_PER_IRON_BAR * te.ironBarsBeneath, 1)
                + " mL/lightning strike");
    }
    ...
    protected void addProblems(List<String> textList) {
        ...
        if (PneumaticValues.MAX_REDIRECTION_PER_IRON_BAR * te.ironBarsBeneath
            < PneumaticValues.PRODUCTION_ELECTROSTATIC_COMPRESSOR / connectedCompressors) {
            textList.add(
                EnumChatFormatting.GRAY
                    + "When lightning strikes with a full air tank not all the energy can be redirected!");
            textList.add(
                EnumChatFormatting.BLACK
                    + "Connect up more Iron Bars to the underside of the Electrostatic Compressor.");
        }
    }
    ...
    public void updateScreen() {
        ...
        if (ticksExisted % 20 == 0) {
            ...
            TickHandlerPneumaticCraft
                .getElectrostaticGrid(coordList, te.getWorldObj(), te.xCoord, te.yCoord, te.zCoord);
            connectedCompressors = 0;
            for (int[] coord : coordList) {
                if (te.getWorldObj()
                    .getBlock(coord[0], coord[1], coord[2]) == Blockss.electrostaticCompressor) {
