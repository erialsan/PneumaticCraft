...
public class GuiPneumaticHelmet extends GuiPneumaticInventoryItem {
    ...
    public void initGui() {
        ...
        addAnimatedStat("gui.tab.info", Textures.GUI_INFO_LOCATION, 0xFF8888FF, true)
            .setText("gui.tab.info.item.pneumaticHelmet");
        ...
        addAnimatedStat(searchUpgradeStat.getDisplayName(), searchUpgradeStat, 0xFF0000FF, false)
            .setText("gui.tab.info.item.pneumaticHelmet.searchUpgrade");
        ItemStack coordinateTracker = new ItemStack(
            Itemss.machineUpgrade,
            1,
            ItemMachineUpgrade.UPGRADE_COORDINATE_TRACKER_DAMAGE);
        addAnimatedStat(coordinateTracker.getDisplayName(), coordinateTracker, 0xFF0000FF, false)
            .setText("gui.tab.info.item.pneumaticHelmet.coordinateTracker");
        ItemStack entityTracker = new ItemStack(Itemss.machineUpgrade, 1, ItemMachineUpgrade.UPGRADE_ENTITY_TRACKER);
        addAnimatedStat(entityTracker.getDisplayName(), entityTracker, 0xFF0000FF, true)
            .setText("gui.tab.info.item.pneumaticHelmet.entityTracker");
        ItemStack blockTracker = new ItemStack(Itemss.machineUpgrade, 1, ItemMachineUpgrade.UPGRADE_BLOCK_TRACKER);
        addAnimatedStat(blockTracker.getDisplayName(), blockTracker, 0xFF0000FF, true)
            .setText("gui.tab.info.item.pneumaticHelmet.blockTracker");
    }

    @Override
    public void updateScreen() {
        super.updateScreen();
        CommonHUDHandler.getHandlerForPlayer()
            .checkHelmetInventory(itemStack);
    ...
    private List<String> getStatusText() {
        ...
        text.add("\u00a77Air Usage:");
        float totalUsage = UpgradeRenderHandlerList.instance()
            .getAirUsage(
        ...
        if (totalUsage > 0F) {
            ...
            for (int i = 0; i < UpgradeRenderHandlerList.instance().upgradeRenderers.size(); i++) {
                if (CommonHUDHandler.getHandlerForPlayer(player).upgradeRenderersInserted[i]) {
                    ...
                    float upgradeUsage = handler
                        .getEnergyUsage(CommonHUDHandler.getHandlerForPlayer(player).rangeUpgradesInstalled, player);
                    if (upgradeUsage > 0F) {
                        text.add(
                            EnumChatFormatting.BLACK.toString() + PneumaticCraftUtils.roundNumberTo(upgradeUsage, 1)
                                + " mL/tick ("
                                + handler.getUpgradeName()
                                + ")");
                    }
                }
            }
        ...
        int volume = ItemPneumaticArmor.getUpgrades(
            ItemMachineUpgrade.UPGRADE_VOLUME_DAMAGE,
            te.getStackInSlot(TileEntityChargingStation.CHARGE_INVENTORY_INDEX)) * PneumaticValues.VOLUME_VOLUME_UPGRADE
            + getDefaultVolume();
        ...
        if (totalUsage == 0) {
            ...
            text.add(
                "\u00a70" + PneumaticCraftUtils.convertTicksToMinutesAndSeconds((int) (airLeft / totalUsage), false));
        }
        return text;
    }
