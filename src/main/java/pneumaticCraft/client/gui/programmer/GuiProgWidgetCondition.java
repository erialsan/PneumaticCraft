...
public class GuiProgWidgetCondition extends GuiProgWidgetAreaShow<ProgWidget> {
        ...
        if (requiresNumber()) {
            ...
            for (int i = 0; i < ICondition.Operator.values().length; i++) {
                radioButton = new GuiRadioButton(
                    ...
                    guiTop + baseY + i * 12,
                    0xFF000000,
                    ICondition.Operator.values()[i].toString());
                radioButton.checked = ((ICondition) widget).getOperator()
                    .ordinal() == i;
                addWidget(radioButton);
                radioButtons.add(radioButton);
                radioButton.otherChoices = radioButtons;
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        fontRendererObj.drawString(
            widget.getExtraStringInfo(),
            guiLeft + xSize / 2 - fontRendererObj.getStringWidth(widget.getExtraStringInfo()) / 2,
            guiTop + 120,
            0xFF000000);
    }
