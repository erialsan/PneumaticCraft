...
public class GuiKeybindCheckBox extends GuiCheckBox {
    ...
    public GuiKeybindCheckBox(int id, int x, int y, int color, String text, String keyBindingName) {
        ...
        if (!trackedCheckboxes.containsKey(keyBindingName)) {
            checked = Config.config.get("pneumatic_helmet_widgetDefaults", keyBindingName, true)
                .getBoolean();
    ...
    public void onMouseClicked(int mouseX, int mouseY, int button) {
        if (button == 0) {
            ...
            if (trackedBox != this) {
                ...
                Config.config.get("pneumatic_helmet_widgetDefaults", keyBindingName, true)
                    .set(checked);
                Config.config.save();
                        ...
                        NetworkHandler.sendToServer(
                            new PacketToggleHelmetFeature(
                                ...
                                GuiKeybindCheckBox.trackedCheckboxes
                                    .get("pneumaticHelmet.upgrade.coreComponents").checked && checked));
                    }
                }
                if (keyBindingName.equals("pneumaticHelmet.upgrade.coreComponents")) {
                    for (int i = 0; i < UpgradeRenderHandlerList.instance().upgradeRenderers.size(); i++) {
                        NetworkHandler.sendToServer(
                            new PacketToggleHelmetFeature(
                                (byte) i,
                                checked && GuiKeybindCheckBox.trackedCheckboxes.get(
    ...
    public void onKeyInput(InputEvent.KeyInputEvent event) {
        ...
        if (mc.inGameHasFocus && keyBinding != null && keyBinding.isPressed()) {
            ...
            HUDHandler.instance()
                .addMessage(
                    I18n.format(
                        "pneumaticHelmet.message." + (checked ? "enable" : "disable") + "Setting",
                        I18n.format(text)),
    ...
    public static KeyBinding setOrAddKeybind(String keybindName, int keyCode) {
        GameSettings gameSettings = FMLClientHandler.instance()
            .getClient().gameSettings;
        for (KeyBinding keyBinding : gameSettings.keyBindings) {
            if (keyBinding != null && keyBinding.getKeyDescription()
                .equals(keybindName)) {
        ...
        if (keyCode < 0) {
            if (((ClientProxy) PneumaticCraft.proxy).keybindToKeyCodes.containsKey(keybindName)) {// If the keybind can
                                                                                                  // be found in the
                                                                                                  // options file
