...
public class WidgetComboBox extends WidgetTextField {
    ...
    public WidgetComboBox setElements(String[] elements) {
        ...
        for (String s : elements) this.elements.add(s);
        Collections.sort(this.elements);
        return this;
    }
    ...
    private List<String> getApplicableElements() {
        ...
        for (String element : elements) {
            if (fixedOptions || element.toLowerCase()
                .contains(getText().toLowerCase())) list.add(element);
        }
        return list;
    }
    ...
    public void postRender(int mouseX, int mouseY, float partialTick) {
        ...
        if (enabled && isFocused()) {
            ...
            drawRect(
                ...
                yPosition + height + 1,
                xPosition + width + 1,
                yPosition + height + 3 + applicableElements.size() * fontRenderer.FONT_HEIGHT,
                -6250336);
            drawRect(
                xPosition,
                yPosition + height + 1,
                xPosition + width,
                yPosition + height + 2 + applicableElements.size() * fontRenderer.FONT_HEIGHT,
                -16777216);
            for (int i = 0; i < applicableElements.size(); i++) {
                String element = applicableElements.get(i);
                fontRenderer.drawStringWithShadow(
                    fontRenderer.trimStringToWidth(element, getWidth()),
                    xPosition + 4,
                    yPosition + height + 2 + i * fontRenderer.FONT_HEIGHT,
                    14737632);
            }
        }
    }
        ...
        if (enabled) {
            ...
            for (int i = 0; i < applicableElements.size(); i++) {
                if (new Rectangle(
                    ...
                    yPosition + height + 2 + i * fontRenderer.FONT_HEIGHT,
                    width,
                    fontRenderer.FONT_HEIGHT).contains(mouseX, mouseY)) {
    ...
    public Rectangle getBounds() {
        return enabled && isFocused()
            ? new Rectangle(
                ...
                yPosition,
                width,
                height + 2 + getApplicableElements().size() * fontRenderer.FONT_HEIGHT)
            : super.getBounds();
    }
