    ...
    public void render(int mouseX, int mouseY, float partialTick) {
        ...
        if (renderBackground) {
            Minecraft.getMinecraft()
                .getTextureManager()
                .bindTexture(Textures.WIDGET_AMADRON_OFFER);
        ...
        if (shoppingAmount > 0) {
            Minecraft.getMinecraft().fontRenderer.drawString(
                shoppingAmount + "",
                x + 36 - Minecraft.getMinecraft().fontRenderer.getStringWidth("" + shoppingAmount) / 2,
                y + (customOffer ? 15 : 20),
                0xFF000000);
        }
        if (customOffer) {
            ...
            Minecraft.getMinecraft().fontRenderer.drawString(
                EnumChatFormatting.DARK_BLUE.toString() + custom.getStock() + "",
                x + 36 - Minecraft.getMinecraft().fontRenderer.getStringWidth("" + custom.getStock()) / 2,
                y + 25,
                0xFF000000);
        }
    }
    ...
    public void addTooltip(int mouseX, int mouseY, List<String> curTip, boolean shiftPressed) {
        ...
        if (!isInBounds) {
            ...
            if (offer.getVendor()
                ...
                curTip.addAll(
                    Arrays.asList(
                        WordUtils.wrap(I18n.format("gui.amadron.amadronWidget.sneakRightClickToRemove"), 40)
