...
public class GuiAnimatedStat implements IGuiAnimatedStat, IGuiWidget, IWidgetListener {
    ...
    public IGuiAnimatedStat setText(List<String> text) {
        ...
        for (String line : text) {
            textList.addAll(
                PneumaticCraftUtils
                    .convertStringIntoList(I18n.format(line), (int) (GuiConstants.maxCharPerLineLeft / textScale)));
        }
        onTextChange();
        return this;
    }
    ...
    public IGuiAnimatedStat setText(String text) {
        ...
        textList.addAll(
            PneumaticCraftUtils
                .convertStringIntoList(I18n.format(text), (int) (GuiConstants.maxCharPerLineLeft / textScale)));
        onTextChange();
        return this;
    }
    ...
    public void onTextChange() {
        if (textList.size() > MAX_LINES) {
             ...
             */
            addWidget(
                new WidgetVerticalScrollbar(-1000, leftSided ? -16 : 2, 20, (int) ((MAX_LINES * 10 - 20) * textSize))
        ...
        if (doneExpanding) {
            ...
            for (int i = curScroll; i < textList.size() && i < curScroll + MAX_LINES; i++) {
                ...
                if (textList.get(i)
                    .contains("\u00a70")
                    || textList.get(i)
                        .contains(EnumChatFormatting.DARK_RED.toString())) {
                    fontRenderer.drawString(
                        textList.get(i),
                        renderBaseX + (leftSided ? -renderWidth + 2 : 18),
                        renderAffectedY + (i - curScroll) * 10 + 12,
                        0xFFFFFF);
                } else {
                    fontRenderer.drawStringWithShadow(
                        textList.get(i),
    ...
    public boolean onKey(char key, int keyCode) {
        for (IGuiWidget widget : widgets) if (widget.onKey(key, keyCode)) return true;
        return false;
    }
