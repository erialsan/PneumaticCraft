...
public class GuiPlasticMixer extends GuiPneumaticContainerBase<TileEntityPlasticMixer> {
    ...
    public void initGui() {
        ...
        addWidget(
            new WidgetTemperature(
                ...
                500,
                te.getLogic(1),
                PneumaticValues.PLASTIC_MIXER_MELTING_TEMP));
        addWidget(new WidgetTank(3, guiLeft + 152, guiTop + 14, te.getFluidTank()));

        GuiAnimatedStat stat = addAnimatedStat(
            "gui.tab.plasticMixer.plasticSelection",
            new ItemStack(Itemss.plastic, 1, 1),
            0xFF005500,
            false);
        ...
        for (int x = 0; x < 4; x++) {
            for (int y = 0; y < 4; y++) {
                ...
                buttons[index] = new GuiButtonSpecial(index + 1, x * 21 + 4, y * 21 + 30, 20, 20, "")
                    .setRenderStacks(plastic)
                    .setTooltipText(plastic.getDisplayName());
                stat.addWidget(buttons[index]);
            }
        }
        stat.addWidget(
            lockSelection = new GuiCheckBox(17, 4, 18, 0xFF000000, "gui.plasticMixer.lockSelection")
    ...
    public String getRedstoneButtonText(int mode) {
        return mode == 3 ? "gui.tab.redstoneBehaviour.plasticMixer.button.selectOnSignal"
            : super.getRedstoneButtonText(mode);
    }
    ...
    protected void drawGuiContainerBackgroundLayer(float partialTicks, int x, int y) {
        ...
        for (int i = 0; i < 3; i++) {
            ...
            drawVerticalLine(
                guiLeft + 123,
                guiTop + 37 + i * 18,
                guiTop + 37 - MathHelper.clamp_int((int) (percentage * 16), 1, 15) + i * 18,
                0xFF000000 | 0xFF0000 >> 8 * i);
        }
    }
    ...
    protected void addProblems(List<String> curInfo) {
        ...
        if (te.getFluidTank()
            ...
            if (te.getStackInSlot(4) != null) {
                if (te.getLogic(1)
                    .getTemperature() >= PneumaticValues.PLASTIC_MIXER_MELTING_TEMP
                    && te.getFluidTank()
                        .getCapacity()
                        - te.getFluidTank()
                            .getFluidAmount()
        ...
        if (te.getStackInSlot(TileEntityPlasticMixer.INV_DYE_RED) == null) {
            curInfo.add(
                I18n.format("gui.tab.problems.plasticMixer.noDye", new ItemStack(Items.dye, 1, 1).getDisplayName()));
        }
        if (te.getStackInSlot(TileEntityPlasticMixer.INV_DYE_GREEN) == null) {
            curInfo.add(
                I18n.format("gui.tab.problems.plasticMixer.noDye", new ItemStack(Items.dye, 1, 2).getDisplayName()));
        }
        if (te.getStackInSlot(TileEntityPlasticMixer.INV_DYE_BLUE) == null) {
            curInfo.add(
                I18n.format("gui.tab.problems.plasticMixer.noDye", new ItemStack(Items.dye, 1, 4).getDisplayName()));
        }
