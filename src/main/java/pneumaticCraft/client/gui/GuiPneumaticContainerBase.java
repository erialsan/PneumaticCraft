...
public class GuiPneumaticContainerBase<Tile extends TileEntityBase> extends GuiContainer
    implements INEIGuiHandler, IWidgetListener {

    public final Tile te;
    private final ResourceLocation guiTexture;
    /**
     * Any GuiAnimatedStat added to this list will be tracked for mouseclicks, tooltip renders, rendering,updating
    ...
    protected GuiAnimatedStat addAnimatedStat(String title, ItemStack icon, int color, boolean leftSided) {
        ...
        GuiAnimatedStat stat = new GuiAnimatedStat(
            ...
            title,
            icon,
            xStart + (leftSided ? 0 : xSize),
            leftSided && lastLeftStat != null || !leftSided && lastRightStat != null ? 3 : yStart + 5,
            color,
    ...
    public void initGui() {
        ...
        if (shouldAddPressureTab() && te instanceof TileEntityPneumaticBase) {
            pressureStat = this
                .addAnimatedStat("gui.tab.pressure", new ItemStack(Blockss.pressureTube), 0xFF00AA00, false);
        }
        ...
        if (te instanceof IInventory) {
            ...
            if (te instanceof IHeatExchanger) {
                addAnimatedStat("gui.tab.info.heat.title", new ItemStack(Blocks.fire), 0xFFFF5500, false)
            ...
            if (shouldAddUpgradeTab()) {
                ...
                if (upgradeText.size() > 0)
                    addAnimatedStat("gui.tab.upgrades", Textures.GUI_UPGRADES_LOCATION, 0xFF0000FF, true)
                        .setText(upgradeText);
            }
        }
    }
    ...
    protected void drawGuiContainerBackgroundLayer(float partialTicks, int i, int j) {
        ...
        if (pressureStat != null) {
            ...
            if (gaugeLocation != null) GuiUtils.drawPressureGauge(
                fontRendererObj,
                -1,
                pneu.CRITICAL_PRESSURE,
                pneu.DANGER_PRESSURE,
                te instanceof IMinWorkingPressure ? ((IMinWorkingPressure) te).getMinWorkingPressure() : -1,
                pneu.getPressure(ForgeDirection.UNKNOWN),
                gaugeLocation.x,
                gaugeLocation.y,
                zLevel);
        }
    ...
    protected void drawGuiContainerForegroundLayer(int x, int y) {
        if (getInvNameOffset() != null && te instanceof IInventory) {
            ...
            String containerName = inv.hasCustomInventoryName() ? inv.getInventoryName()
                : StatCollector.translateToLocal(inv.getInventoryName() + ".name");
            fontRendererObj.drawString(
                containerName,
                xSize / 2 - fontRendererObj.getStringWidth(containerName) / 2 + getInvNameOffset().x,
                6 + getInvNameOffset().y,
                4210752);
        }
        if (getInvTextOffset() != null) fontRendererObj.drawString(
            StatCollector.translateToLocal("container.inventory"),
            8 + getInvTextOffset().x,
            ySize - 94 + getInvTextOffset().y,
            4210752);
    }
    ...
    public void drawScreen(int x, int y, float partialTick) {
        ...
        for (Object obj : buttonList) {
            if (obj instanceof GuiButtonSpecial) {
                ...
                if (button.xPosition < x && button.xPosition + button.getWidth() > x
                    && button.yPosition < y
                    && button.yPosition + button.getHeight() > y) {
        ...
        for (IGuiWidget widget : widgets) {
            if (widget.getBounds()
                .contains(x, y)) widget.addTooltip(x, y, tooltip, PneumaticCraft.proxy.isSneakingInGui());
        }
    ...
    public void updateScreen() {
        ...
        if (redstoneTab != null) {
            redstoneButton.displayString = I18n
                .format(getRedstoneButtonText(((IRedstoneControl) te).getRedstoneMode()));
        }
    }
    ...
    public String getRedstoneString() {
        return te instanceof IRedstoneControlled ? "gui.tab.redstoneBehaviour.enableOn"
            : "gui.tab.redstoneBehaviour.emitRedstoneWhen";
    }
    ...
    protected void addPressureStatInfo(List<String> pressureStatText) {
        ...
        pressureStatText.add(
            "\u00a70" + PneumaticCraftUtils.roundNumberTo(pneumaticTile.getPressure(ForgeDirection.UNKNOWN), 1)
                + " bar.");
    ...
    protected void mouseClicked(int par1, int par2, int par3) {
        ...
        for (IGuiWidget widget : widgets) {
            if (widget.getBounds()
                .contains(par1, par2)) widget.onMouseClicked(par1, par2, par3);
        ...
        if (widget instanceof IGuiAnimatedStat) {
            ...
            for (IGuiWidget w : widgets) {
                if (w instanceof IGuiAnimatedStat) {
                    ...
                    if (widget != stat && stat.isLeftSided() == leftSided) {// when the stat is on the same side, close
                                                                            // it.
                        stat.closeWindow();
                    }
                }
    ...
    public static void drawTexture(String texture, int x, int y) {
        Minecraft.getMinecraft()
            .getTextureManager()
            .bindTexture(GuiUtils.getResourceLocation(texture));
    ...
    public boolean hideItemPanelSlot(GuiContainer gui, int x, int y, int w, int h) {
        for (IGuiWidget widget : widgets) {
            if (widget instanceof IGuiAnimatedStat) {
                ...
                if (stat.getBounds()
                    .intersects(new Rectangle(x, y, w, h))) return true;
            }
        }
        return false;
    }
