...
public class GuiPressureChamber extends GuiPneumaticContainerBase<TileEntityPressureChamberValve> {
    ...
    public void initGui() {
        ...
        statusStat = addAnimatedStat(
            ...
            new ItemStack(Blockss.pressureChamberWall),
            0xFFFFAA00,
            false);
    }
    ...
    protected void drawGuiContainerForegroundLayer(int x, int y) {
        ...
        String containerName = te.hasCustomInventoryName() ? te.getInventoryName()
            : I18n.format(
                "gui.pressureChamberTitle",
                te.multiBlockSize + "x" + te.multiBlockSize + "x" + te.multiBlockSize);
        fontRendererObj
    ...
    private List<String> getStatusText() {
        ...
        if (PneumaticCraft.isNEIInstalled) {
            text.add(
                "\u00a70Click on the Pressure gauge to view all the recipes of this machine. Powered by ChickenBones' NEI.");
        } else {
            text.add(
                "\u00a70Install NEI (an other (client) mod by ChickenBones) to be able to see all the recipes of this machine.");
        }
        return text;
    }
