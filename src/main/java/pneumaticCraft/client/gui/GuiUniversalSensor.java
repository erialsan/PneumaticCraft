...
public class GuiUniversalSensor extends GuiPneumaticContainerBase<TileEntityUniversalSensor> {
    ...
    protected void drawGuiContainerForegroundLayer(int x, int y) {
        ...
        String[] folders = te.getSensorSetting()
            .split("/");
        if (folders.length == 1) {
            ItemStack[] requiredStacks = SensorHandler.instance()
                .getRequiredStacksFromText(folders[0]);
    ...
    protected void drawGuiContainerBackgroundLayer(float opacity, int x, int y) {
        ...
        ISensorSetting sensor = SensorHandler.instance()
            .getSensorFromPath(te.getSensorSetting());
        if (sensor != null) {
    ...
    public String getRedstoneButtonText(int mode) {
        return te.invertedRedstone ? "gui.tab.redstoneBehaviour.universalSensor.button.inverted"
    ...
    public void updateButtons() {
        ...
        if (!te.getSensorSetting()
            ...
            buttonList.add(
                new GuiButton(
                    ...
                    98,
                    20,
                    I18n.format("gui.universalSensor.button.showRange")));
        }
        String[] directories = SensorHandler.instance()
            .getDirectoriesAtLocation(te.getSensorSetting());
        ...
        for (int i = 0; i < buttonsOnPage; i++) {
            ...
            if (SensorHandler.instance()
                .getSensorFromPath(te.getSensorSetting() + "/" + buttonText) != null) {
            ...
            if (te.getSensorSetting()
                .equals("")) {
                ItemStack[] requiredStacks = SensorHandler.instance()
                    .getRequiredStacksFromText(buttonText);
                GuiButtonSpecial button = new GuiButtonSpecial(
                    buttonID,
                    buttonX,
                    buttonY,
                    buttonWidth,
    ...
    protected void addPressureStatInfo(List<String> pressureStatText) {
        ...
        if (te.isSensorActive) {
            ...
            pressureStatText
                .add(EnumChatFormatting.BLACK.toString() + PneumaticValues.USAGE_UNIVERSAL_SENSOR + "mL/tick");
        }
    }
    ...
    protected void addProblems(List<String> textList) {
        ...
        if (SensorHandler.instance()
            .getSensorFromPath(te.getSensorSetting()) == null) {
        ...
        for (int i = TileEntityUniversalSensor.UPGRADE_SLOT_1; i <= TileEntityUniversalSensor.UPGRADE_SLOT_4; i++) {
            if (te.getStackInSlot(i) != null && te.getStackInSlot(i)
                .getItem() == Itemss.GPSTool) {
                if (te.getStackInSlot(i)
                    .hasTagCompound()) {
                    NBTTagCompound gpsTag = te.getStackInSlot(i)
                        .getTagCompound();
                    ...
                    if (toolX == 0 && toolY == 0 && toolZ == 0) {
                        textList
                            .add(EnumChatFormatting.GRAY + "The inserted GPS Tool doesn't have a coordinate selected!");
                        textList.add(EnumChatFormatting.BLACK + "Insert a GPS Tool with stored coordinate.");
                        break;
                    }
                    int sensorRange = te.getRange();
                    if (Math.abs(toolX - te.xCoord) > sensorRange || Math.abs(toolY - te.yCoord) > sensorRange
                        || Math.abs(toolZ - te.zCoord) > sensorRange) {
                        textList.add(
                            EnumChatFormatting.GRAY
                                + "The stored coordinate in the GPS Tool is out of the Sensor's range!");
                        textList.add(
                            EnumChatFormatting.BLACK
                                + "Move the sensor closer, select a closer coordinate or insert Range Upgrades.");
                    }
                } else {
