...
public class GuiAmadron extends GuiPneumaticContainerBase {
    ...
    public void initGui() {
        ...
        addLabel(
            I18n.format("gui.search"),
            guiLeft + 76 - mc.fontRenderer.getStringWidth(I18n.format("gui.search")),
            guiTop + 41);
        ...
        List<String> tooltip = PneumaticCraftUtils
            .convertStringIntoList(I18n.format("gui.amadron.button.order.tooltip"), 40);
        addWidget(
            new GuiButtonSpecial(1, guiLeft + 6, guiTop + 15, 72, 20, I18n.format("gui.amadron.button.order"))
                .setTooltipText(tooltip));
        addTradeButton = new GuiButtonSpecial(
            2,
            guiLeft + 80,
            guiTop + 15,
            72,
            20,
            I18n.format("gui.amadron.button.addTrade"));
    ...
    public void updateScreen() {
        ...
        List<String> tooltip = PneumaticCraftUtils
            .convertStringIntoList(I18n.format("gui.amadron.button.addTrade.tooltip"), 40);
        tooltip.add(
            (addTradeButton.enabled ? EnumChatFormatting.GRAY : EnumChatFormatting.RED) + I18n.format(
                "gui.amadron.button.addTrade.tooltip.offerCount",
                container.currentOffers,
                container.maxOffers == Integer.MAX_VALUE ? "\u221E" : container.maxOffers));
    ...
    public void updateVisibleOffers() {
        ...
        for (int i = 0; i < visibleOffers.size(); i++) {
            ...
            WidgetAmadronOffer widget = new WidgetAmadronOffer(
                i,
                guiLeft + 6 + 73 * (i % 2),
                guiTop + 55 + 35 * (i / 2),
                offer) {
                ...
                public void onMouseClicked(int mouseX, int mouseY, int button) {
                    NetworkHandler.sendToServer(
                        new PacketAmadronOrderUpdate(
                            container.offers.indexOf(getOffer()),
                            button,
                            PneumaticCraft.proxy.isSneakingInGui()));
                }
            };
            addWidget(widget);
