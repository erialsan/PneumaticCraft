...
public class NetworkConnectionPlayerHandler extends NetworkConnectionHandler {
    ...
    public NetworkConnectionPlayerHandler(GuiSecurityStationBase gui, TileEntitySecurityStation station, int baseX,
        ...
        for (int i = 0; i < 35; i++) {
            if (station.getStackInSlot(i) != null && station.getStackInSlot(i)
                .getItemDamage() == ItemNetworkComponents.NETWORK_IO_PORT) {
                slotHacked[i] = true;
            }
        }
    }
        ...
        if (slot != null) {
            ...
            if (mouseButton == 1 && slotHacked[slot.slotNumber]) {
                ...
                if (!alreadyFortifying) {
                    balloons.add(
                        new GuiStatBalloon(
                            slot.xDisplayPosition + gui.getGuiLeft() + 8,
                            slot.yDisplayPosition + gui.getGuiTop() - 5,
                            slot.slotNumber));
                }
            }
            if (mouseButton == 2 && !slotHacked[slot.slotNumber]
                && ((GuiSecurityStationHacking) gui).hasNukeViruses()) {
    ...
    protected void onSlotHack(int slot, boolean nuked) {
        ...
        if (station.getStackInSlot(slot) != null && (station.getStackInSlot(slot)
            .getItemDamage() == ItemNetworkComponents.NETWORK_REGISTRY
            || station.getStackInSlot(slot)
                .getItemDamage() == ItemNetworkComponents.DIAGNOSTIC_SUBROUTINE)) {
            ...
            FMLClientHandler.instance()
                .getClient().thePlayer.closeScreen();
            player.addChatComponentMessage(
                new ChatComponentTranslation(
                    EnumChatFormatting.GREEN
                        + "Hacking successful! This Security Station now doesn't protect the area any longer!"));
            if (gui instanceof GuiSecurityStationHacking)
