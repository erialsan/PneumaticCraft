...
public class GuiSecurityStationInventory extends GuiSecurityStationBase {
    ...
    public void updateScreen() {
        ...
        if (te.getRebootTime() > 0) {
            rebootButtonString = te.getRebootTime() % 100 < 50 ? "Rebooting.."
                : PneumaticCraftUtils.convertTicksToMinutesAndSeconds(te.getRebootTime(), false);
        } else {
    ...
    protected void addProblems(List<String> text) {
        ...
        if (te.getRebootTime() > 0) {
            ...
            text.add(
                EnumChatFormatting.BLACK + "The station is rebooting ("
                    + PneumaticCraftUtils.convertTicksToMinutesAndSeconds(te.getRebootTime(), false)
                    + ").");
        } else if (te.isHacked()) {
                ...
                case NO_CONNECTION_SUB_AND_IO_PORT:
                    text.add(
                        EnumChatFormatting.BLACK
                            + "The Diagnostic Subroutine and the Network IO Port need to be connected in the network.");
                    break;
                case NO_CONNECTION_IO_PORT_AND_REGISTRY:
                    text.add(
                        EnumChatFormatting.BLACK
                            + "The Network Registry and the Network IO Port need to be connected in the network.");
                    break;
            }
        }
    }
    ...
    public void actionPerformed(IGuiWidget widget) {
        if (widget.getID() == 1 && !sharedUserTextField.getText()
            .equals(""))
    ...
    private void updateUserRemoveButtons() {
        ...
        for (int i = 0; i < te.sharedUsers.size(); i++) {
            Rectangle rect = accessStat.getButtonScaledRectangle(
                20,
                32 + i * 10,
                fontRendererObj.getStringWidth(
                    "-" + te.sharedUsers.get(i)
                        .getName()),
                8);
