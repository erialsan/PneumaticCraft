...
public class GuiPressureChamberInterface extends GuiPneumaticContainerBase<TileEntityPressureChamberInterface> {
    ...
    private static final ResourceLocation guiTextureCreativeFilter = new ResourceLocation(
        Textures.GUI_PRESSURE_CHAMBER_INTERFACE_CREATIVE_FILTER_LOCATION);
    ...
    public void initGui() {
        ...
        statusStat = addAnimatedStat(
            "Interface Status",
            new ItemStack(Blockss.pressureChamberInterface),
            0xFFFFAA00,
            false);
    ...
    protected void drawGuiContainerForegroundLayer(int x, int y) {
        ...
        int inputShift = (int) ((1F - (float) Math
            .cos((float) te.inputProgress / (float) TileEntityPressureChamberInterface.MAX_PROGRESS * Math.PI)) * 11);
        int outputShift = (int) ((1F - (float) Math
            .cos((float) te.outputProgress / (float) TileEntityPressureChamberInterface.MAX_PROGRESS * Math.PI)) * 11);
    ...
    protected void bindGuiTexture() {
        if (te.filterMode == TileEntityPressureChamberInterface.EnumFilterMode.ITEM) {
            ...
            mc.getTextureManager()
                .bindTexture(guiTextureCreativeFilter);
        }
    }
    ...
    protected void mouseClicked(int par1, int par2, int par3) {
        ...
        if (te.filterMode == TileEntityPressureChamberInterface.EnumFilterMode.NAME_BEGINS
            || te.filterMode == TileEntityPressureChamberInterface.EnumFilterMode.NAME_CONTAINS) {
    ...
    public void actionPerformed(IGuiWidget widget) {
        if (widget.getID() == 1) {
            if (filterStat != null) {
                ...
                } else if (te.filterMode.ordinal()
                    == TileEntityPressureChamberInterface.EnumFilterMode.values().length - 1) {
                        for (int i = 5; i < 14; i++) {
                            ((Slot) inventorySlots.inventorySlots.get(i)).xDisplayPosition -= 1000;
                        }
                    }
