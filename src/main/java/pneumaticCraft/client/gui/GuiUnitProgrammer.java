...
public class GuiUnitProgrammer extends GuiScreen {
    ...
    public GuiUnitProgrammer(List<IProgWidget> progWidgets, FontRenderer fontRendererObj, int guiLeft, int guiTop,
        ...
        scaleScroll = new WidgetVerticalScrollbar(guiLeft + 302, guiTop + 40, 129).setStates(9)
    ...
    public void renderForeground(int x, int y, IProgWidget tooltipExcludingWidget) {
        ...
        if (widget != null && widget != tooltipExcludingWidget) {
            ...
            if (errors.size() > 0) {
                ...
                for (String s : errors) {
                    String[] lines = WordUtils.wrap("-" + I18n.format(s), 30)
    ...
    public IProgWidget getHoveredWidget(int x, int y) {
        ...
        for (IProgWidget widget : progWidgets) {
            if (!isOutsideProgrammingArea(widget)) {
                if ((x - translatedX) / scale - guiLeft >= widget.getX()
                    && (y - translatedY) / scale - guiTop >= widget.getY()
                    && (x - translatedX) / scale - guiLeft <= widget.getX() + widget.getWidth() / 2
                    && (y - translatedY) / scale - guiTop <= widget.getY() + widget.getHeight() / 2) {
                    return widget;
                }
            }
        }
    ...
    public void render(int x, int y, boolean showFlow, boolean showInfo, boolean translate) {
        ...
        GL11.glScissor(
            (guiLeft + startX) * sr.getScaleFactor(),
            (sr.getScaledHeight() - areaHeight - (guiTop + startY)) * sr.getScaleFactor(),
            areaWidth * sr.getScaleFactor(),
            areaHeight * sr.getScaleFactor());
        ...
        if (translate && isLeftClicking
            && wasClicking
            && new Rectangle(guiLeft + startX, guiTop + startY, areaWidth, areaHeight).contains(origX, origY)) {
                            ...
                            for (IProgWidget w : progWidgets) {
                                if (w instanceof ILabel) {
                                    ...
                                    if (label != null && jumpLocation.equals(label)) {
                                        ...
                                        Vec3 arrowVec = Vec3.createVectorHelper(x1 - x2, y1 - y2, 0)
                                            .normalize();
                                        ...
                                        GL11.glVertex3d(
                                            guiLeft + midX + arrowVec.xCoord,
                                            guiTop + midY + arrowVec.yCoord,
                                            zLevel);
    ...
    public boolean isOutsideProgrammingArea(IProgWidget widget) {
        ...
        return x < startX || x + widget.getWidth() * scale / 2 > startX + areaWidth
            || y < startY
            || y + widget.getHeight() * scale / 2 > startY + areaHeight;
    }
