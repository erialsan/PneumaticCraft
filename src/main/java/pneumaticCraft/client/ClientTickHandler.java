...
public class ClientTickHandler {
    ...
    private final List<WeakReference<INeedTickUpdate>> updatedObjects = new ArrayList<WeakReference<INeedTickUpdate>>();// using
                                                                                                                        // weak
                                                                                                                        // references
                                                                                                                        // so
                                                                                                                        // we
    ...
    public void tickEnd(TickEvent.ClientTickEvent event) {
        if (event.phase == TickEvent.Phase.END) {
            ...
            for (int i = 0; i < updatedObjects.size(); i++) {
                INeedTickUpdate updatedObject = updatedObjects.get(i)
                    .get();
