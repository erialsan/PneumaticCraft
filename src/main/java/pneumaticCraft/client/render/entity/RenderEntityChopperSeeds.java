...
public class RenderEntityChopperSeeds extends RenderEntity {
    ...
    public void renderChopperSeeds(EntityChopperSeeds entity, double x, double y, double z, float var1,
        float partialTicks) {
        ...
        for (int i = 0; i < 4; i++) {
            GL11.glPushMatrix();
            GL11.glTranslated(
                Math.sin(0.5D * Math.PI * i + (entity.ticksExisted + partialTicks) * 0.4D) * radius,
                0,
                Math.cos(0.5D * Math.PI * i + (entity.ticksExisted + partialTicks) * 0.4D) * radius);
