...
public class RenderAphorismTile extends TileEntitySpecialRenderer {
    ...
    public void renderTileEntityAt(TileEntity tileentity, double x, double y, double z, float f) {
        ...
        if (FMLClientHandler.instance()
            .getClient().currentScreen instanceof GuiAphorismTile) {
            GuiAphorismTile gui = (GuiAphorismTile) FMLClientHandler.instance()
        ...
        for (int i = 0; i < textLines.length; i++) {
            ...
            RenderManager.instance.getFontRenderer()
                .drawString(
                    EnumChatFormatting.ITALIC + textLine,
                    -RenderManager.instance.getFontRenderer()
                        .getStringWidth(textLine) / 2,
                    -(textLines.length * 10) / 2 + i * 10 + 1,
                    0xFF000000);
        }
    ...
    private int getMaxLineWidth(String[] textList) {
        ...
        for (String string : textList) {
            int stringWidth = RenderManager.instance.getFontRenderer()
