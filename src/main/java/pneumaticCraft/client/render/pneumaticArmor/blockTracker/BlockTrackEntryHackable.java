...
public class BlockTrackEntryHackable implements IBlockTrackEntry {
    ...
    public boolean shouldTrackWithThisEntry(IBlockAccess world, int x, int y, int z, Block block, TileEntity te) {
        return HackUpgradeRenderHandler.enabledForPlayer(PneumaticCraft.proxy.getPlayer())
            && HackableHandler.getHackableForCoord(world, x, y, z, PneumaticCraft.proxy.getPlayer()) != null;
    }
    ...
    public void addInformation(World world, int x, int y, int z, TileEntity te, List<String> infoList) {
        IHackableBlock hackableBlock = HackableHandler
            .getHackableForCoord(world, x, y, z, PneumaticCraft.proxy.getPlayer());
        int hackTime = HUDHandler.instance()
