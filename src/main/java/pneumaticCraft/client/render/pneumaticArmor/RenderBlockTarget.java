...
public class RenderBlockTarget {
    ...
    public RenderBlockTarget(World world, EntityPlayer player, int x, int y, int z, TileEntity te,
        ...
        String title = world.getBlock(x, y, z)
            .getLocalizedName();
    ...
    public void update() {
        ...
        if (hackTime > 0) {
            ...
            if (hackableBlock != null) {
                hackTime++;// = Math.min(hackTime + 1, hackableBlock.getHackTime(world, blockX, blockY, blockZ,
                           // player));
            } else {
                hackTime = 0;
            }
    ...
    public void render(float partialTicks) {
        ...
        if (!getBlock().isAir(world, blockX, blockY, blockZ))
            arrowRenderer.render(world, blockX, blockY, blockZ, partialTicks);
    ...
    private boolean isPlayerLookingAtTarget() {
        ...
        Vec3 vec31 = Vec3.createVectorHelper(
            blockX + 0.5D - player.posX,
            blockY + 0.5D - player.posY + player.getEyeHeight(),
            blockZ + 0.5D - player.posZ);
    ...
    public void hack() {
        if (isInitialized() && isPlayerLookingAtTarget()) {
            ...
            if (block != null && (hackTime == 0 || hackTime > block.getHackTime(world, blockX, blockY, blockZ, player)))
                NetworkHandler.sendToServer(new PacketHackingBlockStart(blockX, blockY, blockZ));
        }
    }
