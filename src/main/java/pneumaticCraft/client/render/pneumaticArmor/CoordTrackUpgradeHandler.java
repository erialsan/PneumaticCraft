...
public class CoordTrackUpgradeHandler implements IUpgradeRenderHandler {
    ...
    private int noPathCooldown; // timer used to delay the client recalculating a path when it didn't last time. This
                                // prevents
    ...
    public void initConfig(Configuration config) {
        pathEnabled = config
            .get("Helmet_Options" + Configuration.CATEGORY_SPLITTER + "Coordinate_Tracker", "Path Enabled", true)
            .getBoolean(true);
        wirePath = config
            .get("Helmet_Options" + Configuration.CATEGORY_SPLITTER + "Coordinate_Tracker", "Wire Path", true)
            .getBoolean(true);
        xRayEnabled = config
            .get("Helmet_Options" + Configuration.CATEGORY_SPLITTER + "Coordinate_Tracker", "X-Ray", false)
            .getBoolean(true);
        pathUpdateSetting = config
            .get("Helmet_Options" + Configuration.CATEGORY_SPLITTER + "Coordinate_Tracker", "Path Update Rate", 1)
            .getInt();
    }
    ...
    public void saveToConfig() {
        ...
        config.load();
        config.get("Helmet_Options" + Configuration.CATEGORY_SPLITTER + "Coordinate_Tracker", "Path Enabled", true)
            .set(pathEnabled);
        config.get("Helmet_Options" + Configuration.CATEGORY_SPLITTER + "Coordinate_Tracker", "Wire Path", true)
            .set(wirePath);
        config.get("Helmet_Options" + Configuration.CATEGORY_SPLITTER + "Coordinate_Tracker", "X-Ray", true)
            .set(xRayEnabled);
        config.get("Helmet_Options" + Configuration.CATEGORY_SPLITTER + "Coordinate_Tracker", "Path Update Rate", true)
            .set(pathUpdateSetting);
        config.save();
    }
    ...
    public void update(EntityPlayer player, int rangeUpgrades) {
        if (coordTracker != null) coordTracker.ticksExisted++;
            ...
            if (coordTracker != null)
                navigator = new RenderNavigator(coordTracker.worldObj, coordTracker.x, coordTracker.y, coordTracker.z);
        }
    ...
    public void render3D(float partialTicks) {
        if (coordTracker != null) {
            if (FMLClientHandler.instance()
                .getClient().thePlayer.worldObj.provider.dimensionId != coordTracker.worldObj.provider.dimensionId)
                return;
    ...
    public boolean isEnabled(ItemStack[] upgradeStacks) {
        for (ItemStack stack : upgradeStacks) {
            if (stack != null && stack.getItem() == Itemss.machineUpgrade
                && stack.getItemDamage() == ItemMachineUpgrade.UPGRADE_COORDINATE_TRACKER_DAMAGE) return true;
        }
        return false;
    }
    ...
    public EnumNavigationResult navigateToSurface(EntityPlayer player) {
        ...
        PathEntity path = worldObj
            .getEntityPathToXYZ(player, (int) player.posX, y, (int) player.posZ, SEARCH_RANGE, true, true, false, true);
        ...
        if (path != null) {
            for (int i = 0; i < path.getCurrentPathLength(); i++) {
                ...
                if (worldObj.canBlockSeeTheSky(pathPoint.xCoord, pathPoint.yCoord, pathPoint.zCoord)) {
                    coordTracker = new RenderCoordWireframe(
                        worldObj,
                        pathPoint.xCoord,
                        pathPoint.yCoord,
                        pathPoint.zCoord);
    ...
    public static PathEntity getDronePath(EntityPlayer player, int x, int y, int z) {
        ...
        return new EntityPathNavigateDrone(drone, worldObj)
            .getEntityPathToXYZ(drone, x, y, z, SEARCH_RANGE, true, true, false, true);
    }
