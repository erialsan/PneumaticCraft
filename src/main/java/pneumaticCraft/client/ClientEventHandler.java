...
public class ClientEventHandler {
    ...
    public void onPlayerJoin(TickEvent.PlayerTickEvent event) {
        if (Config.shouldDisplayChangeNotification && firstTick
            && event.player.worldObj.isRemote
            && event.player == FMLClientHandler.instance()
                .getClientPlayerEntity()) {
            event.player.addChatComponentMessage(
                new ChatComponentText(
                    EnumChatFormatting.GREEN
                        + "[PneumaticCraft] Disabled world generation of plants and plant mob drops in your config automatically, oil is turned on as replacement. This is only done once, you can change it as you wish now."));
            firstTick = false;
        }
    }
            ...
            if (programmable.canProgram(event.itemStack) && programmable.showProgramTooltip()) {
                ...
                for (Map.Entry<String, Integer> entry : widgetMap.entrySet()) {
                    ...
                    if (curScreen instanceof IGuiDrone) {
                        if (!((IGuiDrone) curScreen).getDrone()
                            .isProgramApplicable(widget)) {
                    ...
                    addedEntries.add(
                        prefix + "-"
                            + entry.getValue()
                            + "x "
                            + I18n.format("programmingPuzzle." + entry.getKey() + ".name"));
                }
    ...
    private void setRenderHead(EntityLivingBase entity, boolean setRender) {
        if (entity.getEquipmentInSlot(4) != null && entity.getEquipmentInSlot(4)
            .getItem() == Itemss.pneumaticHelmet && (Config.useHelmetModel || DateEventHandler.isIronManEvent())) {
    ...
    public void onPlayerRender(RenderPlayerEvent.Pre event) {
        ...
        if (!Config.useHelmetModel && !DateEventHandler.isIronManEvent()
            || event.entityPlayer.getCurrentArmor(3) == null
            || event.entityPlayer.getCurrentArmor(3)
                .getItem() != Itemss.pneumaticHelmet)
            return;
    ...
    public void tickEnd(TickEvent.RenderTickEvent event) {
        if (event.phase == TickEvent.Phase.END && FMLClientHandler.instance()
            .getClient().inGameHasFocus
            && PneumaticCraft.proxy.getPlayer().worldObj != null
            && (ModuleRegulatorTube.inverted || !ModuleRegulatorTube.inLine)) {
            Minecraft mc = FMLClientHandler.instance()
                .getClient();
            ScaledResolution sr = new ScaledResolution(mc, mc.displayWidth, mc.displayHeight);
            FontRenderer fontRenderer = FMLClientHandler.instance()
                .getClient().fontRenderer;
            String warning = EnumChatFormatting.RED + I18n
                .format("gui.regulatorTube.hudMessage." + (ModuleRegulatorTube.inverted ? "inverted" : "notInLine"));
            fontRenderer.drawStringWithShadow(
                warning,
                sr.getScaledWidth() / 2 - fontRenderer.getStringWidth(warning) / 2,
            ...
            if (curItem != null && curItem.getItem() == Itemss.minigun) {
                ...
                if (minigun.isMinigunActivated() && minigun.getMinigunSpeed() == Minigun.MAX_GUN_SPEED) {
                    ...
                    for (int i = 0; i < 5; i++) {
                        ...
                        Vec3 directionVec = player.getLookVec()
                            .normalize();
                        Vec3 vec = Vec3.createVectorHelper(directionVec.xCoord, 0, directionVec.zCoord)
                            .normalize();
                        vec.rotateAroundY(
                            (float) Math.toRadians(-15 + (player.rotationYawHead - player.renderYawOffset)));
                        ...
                        minigunFire.endX = directionVec.xCoord * 20 + player.getRNG()
                            .nextDouble() - 0.5;
                        minigunFire.endY = directionVec.yCoord * 20 + player.getRNG()
                            .nextDouble() - 0.5;
                        minigunFire.endZ = directionVec.zCoord * 20 + player.getRNG()
                            .nextDouble() - 0.5;
                        minigunFire.render();
                    }
