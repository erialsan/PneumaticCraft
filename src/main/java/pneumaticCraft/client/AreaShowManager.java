...
public class AreaShowManager {
    ...
    public void renderWorldLastEvent(RenderWorldLastEvent event) {
        ...
        if (helmet != null && helmet.getItem() == Itemss.pneumaticHelmet) {
            if (droneDebugger == null) droneDebugger = HUDHandler.instance()
                .getSpecificRenderer(DroneDebugUpgradeHandler.class);
        ...
        if (player != null) {
            if (player.worldObj != world) {
                ...
                if (event.phase == TickEvent.Phase.END) {
                    Iterator<ChunkPosition> iterator = showHandlers.keySet()
                        .iterator();
                    while (iterator.hasNext()) {
                        ChunkPosition pos = iterator.next();
                        if (PneumaticCraftUtils.distBetween(pos, player.posX, player.posY, player.posZ) < 32
                            && world.isAirBlock(pos.chunkPosX, pos.chunkPosY, pos.chunkPosZ)) iterator.remove();
                    }
                }
            }
        }
